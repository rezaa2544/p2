-- Payesh migration 001: initial national-scale PostgreSQL schema
-- Immutable forward migration. Rollback: 001_initial.down.sql
BEGIN;

  /* ── Wave 3 (live-DB verification) ─────────────────────────────────────
     مدارس، دروس و کاربران پیش از همه ساخته می‌شوند. ۴۹ ارجاعِ FOREIGN KEY در
     این فایل به schools/subjects/users اشاره می‌کند، ولی این سه جدول پایین‌تر
     تعریف شده بودند و چون کلِ فایل در یک تراکنش است، PostgreSQL با
     «relation "schools" does not exist» کلِ migration را رد می‌کرد — یعنی
     001_initial.sql روی یک دیتابیسِ خالی هرگز اجرا نشده بود.
     ترتیبِ وابستگی: schools ← subjects ← users ← بقیه. ───────────────── */
