...
CREATE INDEX IF NOT EXISTS idx_enrollments_student_year ON enrollments (student_id, year) WHERE deleted_at IS NULL;   /* real column is `year` (academic_year_id does not exist — broke the clean chain) */
CREATE INDEX IF NOT EXISTS idx_enrollments_school_year ON enrollments (school_id, year) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_enrollments_student_year_active ON enrollments (student_id, year) WHERE deleted_at IS NULL;
