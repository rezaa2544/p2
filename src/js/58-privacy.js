...
function doDeleteAccount(){
  ...
  /* F-CSRF-01: احرازِ مجدّد — کدِ پیامکی به شمارهٔ نشست، سپس حذف با همان کد */
  var myPhone = (typeof S !== 'undefined' && S.user && S.user.phone) ? String(S.user.phone) : '';
  if(!myPhone){
    if(typeof toast === 'function') toast('برای حذف حساب، ابتدا دوباره وارد شوید', 'err');
    return;
  }
  Api.request('/api/auth/send-code', { method: 'POST', body: { phone: myPhone }, raw: true }).then(function(r0){
    if(!(r0 && r0.status === 200)){
      if(typeof toast === 'function') toast('ارسال کد تأیید ناموفق بود', 'err');
      return;
    }
    askDeleteCode(function(code){
      Api.request('/api/auth/delete-account', { method: 'POST', body: { code: code }, raw: true }).then(function(r){
...
/* مودالِ ورودِ کدِ حذفِ حساب — عمداً بدونِ data-act (سیم‌کشی با id) تا
   نقشهٔ مجوزهایِ اکشن‌ها دست‌نخورده بماند. */
function askDeleteCode(fn){
  if(typeof openModal !== 'function') return;
  openModal('<div class="card-head"><h3>تأیید نهایی حذف حساب</h3><button class="icon-btn" id="delacc_x">✕</button></div>'
    + '<div class="card-body"><p style="margin:0 0 10px;line-height:2">کد تأییدی که هم‌اکنون پیامک شد را وارد کنید تا حساب برای همیشه حذف شود.</p>'
    + '<input class="input" id="delacc_code" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="کد ۶ رقمی" style="text-align:center;letter-spacing:6px" /></div>'
    + '<div class="card-head" style="border-bottom:none;border-top:1px solid var(--border);justify-content:flex-end">'
    + '<button class="btn ghost" id="delacc_cancel">انصراف</button> '
    + '<button class="btn danger" id="delacc_ok">حذف برای همیشه</button></div>');
  var done = false;
  var close = function(){ if(typeof closeModal === 'function') closeModal(); };
  var cancel = function(){ if(done) return; done = true; close(); };
  document.getElementById('delacc_x').addEventListener('click', cancel);
  document.getElementById('delacc_cancel').addEventListener('click', cancel);
  document.getElementById('delacc_ok').addEventListener('click', function(){
    if(done) return;
    var el = document.getElementById('delacc_code');
    var code = el ? (el.value || '').trim() : '';
    if(!/^\d{4,8}$/.test(code)){
      if(typeof toast === 'function') toast('کد تأیید معتبر نیست', 'err');
      return;
    }
    done = true; close(); fn(code);
  });
}
