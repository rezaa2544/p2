/* ═══════════════════════════════════════════════════════════════════
   ...
   ── دور ۱۰۲: بهینه‌سازیِ کاراییِ ثبت (هدفِ زیر ۶۰۰ مس) ──
   هر تیکِ حاضر/غایب پیش از این یک بازسازیِ کاملِ پوسته را
   فرامی‌خواند (۱۳ رندر کامل برای ۱۳ دانش‌آموز ≈ ۷۰٪ زمانِ ثبت).
   حالا «به‌روزرسانیِ جزئی» فقط ردیفِ همان دانش‌آموز + شمارنده‌ها
   + نوارِ پیش‌نویس را از نو می‌سازد — با همان قالب‌هایِ مشترکِ
   رندرِ کامل (بایت‌به‌بایت یکسان). هر تردیدی پیش بیاید، سوپاپِ
   اطمینان به رندرِ کامل برمی‌گردد؛ رفتار هرگز از وضعِ قبل بدتر
   نمی‌شود.
   ═══════════════════════════════════════════════════════════════════ */
...
/* ─────────── دور ۱۰۲: اجزایِ مشترکِ رندرِ کامل و به‌روزرسانیِ جزئی ──
   این توابع تنها منبعِ حقیقتِ نشانه‌گذاریِ حضور هستند؛ رندرِ کامل
   (viewAttendance) و به‌روزرسانیِ جزئی (attPartialSync) هر دو از
   همین‌ها خروجی می‌گیرند تا هیچ‌وقت از هم جدا نیفتند. */

/** زمینهٔ مشترک: وضعیتِ پیش‌نویس/رکورد/رویداد/تایمر برای یک کلاس و روز */
function attCtx(cid, date){
  ...
  return {cid,date,rec,draft,dev,canExempt,_timers,baseStatus,evOf,_tfa};
}

/** نشانه‌گذاریِ یک ردیفِ دانش‌آموز — دقیقاً همان قالبِ رندرِ کامل */
function attRowHTML(s, i, ctx){
  const rec=ctx.rec, draft=ctx.draft, cid=ctx.cid, date=ctx.date,
        canExempt=ctx.canExempt, _timers=ctx._timers, baseStatus=ctx.baseStatus,
        evOf=ctx.evOf, _tfa=ctx._tfa;
...
/** شمارنده‌هایِ خلاصه — دقیقاً همان قالبِ رندرِ کامل */
function attSummaryInner(roster, ctx){
  const all=roster.map(ctx.baseStatus);
  const cnt=k=>all.filter(x=>x===k).length;
  const unset=all.filter(x=>!x).length;
  const nLateEv=roster.filter(s=>{const e=ctx.evOf(s);return e.late&&!(e.late.late_excused);}).length;
  const nExitEv=roster.filter(s=>{const e=ctx.evOf(s);return e.exit&&!(e.exit.exit_excused);}).length;
  const _nTimers=Object.keys(ctx._timers).length;
  const nDraft=Object.keys(ctx.draft).length;
  return `
    ...
    <span class="small muted">${nDraft||Object.keys(ctx.dev).length?'تغییرات هنوز ذخیره نشده‌اند':'همه‌چیز ذخیره شده است'}</span>`;
}

/** نوارِ پیش‌نویس — دقیقاً همان قالبِ رندرِ کامل */
function attDraftBarHTML(nChange){
  return `<div class="att-draft-bar">
...
/** پابرگِ کارت (دکمه‌های ثبت/دورریختن) — دقیقاً همان قالبِ رندرِ کامل */
function attCardFootHTML(nChange){
  return `<div class="card-foot row">
...
/**
 * دور ۱۰۲: به‌روزرسانیِ جزئی پس از هر تیکِ حاضر/غایب.
 * فقط ردیفِ همان دانش‌آموز + شمارنده‌ها + نوارِ پیش‌نویس و پابرگ
 * بازسازی می‌شوند — پوسته و بقیهٔ صفحه دست نمی‌خورند.
 * خروجی: true = انجام شد؛ false = هر تردیدی (سوپاپِ اطمینان:
 * فراخوان باید به رندرِ کامل برگردد؛ رفتار هرگز بدتر نمی‌شود).
 */
function attPartialSync(sid){
  try{
    if(typeof attDraftGet!=='function'||typeof attDraftDiff!=='function') return false;
    /* هم‌رفتار با رندرِ کامل: ارسالِ خودکارِ پیامک (محافظِ ۳۰ ثانیه‌ای درونِ تابع) */
    if(typeof notifyAutoTick==='function'){ try{ notifyAutoTick(); }catch(e){} }
    ...
    const cid=Number(S.filters.class||(_autoOk&&_auto.classId)||cls[0].id), date=S.filters.date||todayISO();
    const btn=document.querySelector('[data-act="att-set"][data-id="'+sid+'"]');
    const tr=btn&&btn.closest?btn.closest('tr'):null;
    const card=tr&&tr.closest?tr.closest('.card'):null;
    if(!tr||!card) return false;
    const roster=studentsOfClass(cid);
    const q=(S.filters.q||'').trim();
    const studs=q?roster.filter(s=>s.full_name.includes(q)):roster;
    let idx=-1;
    for(let i=0;i<studs.length;i++){ if(studs[i].id===Number(sid)){ idx=i; break; } }
    if(idx<0) return false;
    const ctx=attCtx(cid,date);
    /* ۱) ردیفِ همان دانش‌آموز — بایت‌به‌بایت همان قالبِ رندرِ کامل */
    tr.outerHTML=attRowHTML(studs[idx],idx,ctx);
    /* ۲) شمارنده‌هایِ خلاصه */
    const sumRow=card.querySelector('.card-body.row');
    if(sumRow) sumRow.innerHTML=attSummaryInner(roster,ctx);
    /* ۳) نوارِ پیش‌نویس (بیرونِ کارت، پیش از آن) و پابرگِ کارت */
    const nDraft=Object.keys(ctx.draft).length, hasEv=Object.keys(ctx.dev).length>0;
    const need=nDraft>0||hasEv;
    const diff=attDraftDiff(cid,date), nChange=(diff&&diff.changes)?diff.changes.length:0;
    const bar=document.querySelector('.att-draft-bar');
    if(need){
      if(bar) bar.outerHTML=attDraftBarHTML(nChange);
      else card.insertAdjacentHTML('beforebegin',attDraftBarHTML(nChange));
    } else if(bar&&bar.parentNode) bar.parentNode.removeChild(bar);
    const foot=card.querySelector('.card-foot');
    if(need){
      if(foot) foot.outerHTML=attCardFootHTML(nChange);
      else card.insertAdjacentHTML('beforeend',attCardFootHTML(nChange));
    } else if(foot&&foot.parentNode) foot.parentNode.removeChild(foot);
    return true;
...
function viewAttendance(){
  ...
  /* دور ۱۰۲: زمینۀ مشترک با به‌روزرسانیِ جزئی (attPartialSync) */
  const ctx=attCtx(cid,date);
  const dev=ctx.dev, _timers=ctx._timers;
  ...
  const bannerDraft = nDraft||Object.keys(dev).length ? attDraftBarHTML(nChange) : '';
  ...
  return `${_smBanner}${_nudge}${attChangeTip()}${bannerDraft}<div class="card"><div class="card-head">
    <div class="row"><select class="select" style="width:180px" data-f="class" aria-label="انتخاب کلاس">${cls.map(c=>`<option value="${escAttr(c.id)}" ${c.id===cid?'selected':''}>${esc(c.name)}</option>`).join('')}</select>
     ...
     <input class="input" style="width:160px" type="date" data-f="date" aria-label="تاریخ حضور و غیاب" value="${escAttr(date)}" /><span class="badge b-gray">${jalali(date)}</span></div>
   ...
   <div class="card-body row" style="border-bottom:1px solid var(--border)">${attSummaryInner(roster,ctx)}</div>
   ${studs.length?`<div class="table-wrap"><table><thead><tr><th>#</th><th>نام دانش‌آموز</th><th>ثبت وضعیت</th><th>وضعیت فعلی</th>${ctx.canExempt?'<th>اقدام</th>':''}</tr></thead><tbody>
    ${studs.map((s,i)=>attRowHTML(s,i,ctx)).join('')}
   </tbody></table></div>`:empty('🔍','دانش‌آموزی یافت نشد','این کلاس دانش‌آموزی ندارد یا جستجو نتیجه‌ای نداشت.')}
   ${nDraft||Object.keys(dev).length?attCardFootHTML(nChange):''}
   </div>${repHtml}`;
}
