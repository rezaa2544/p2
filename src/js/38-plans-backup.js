...
function buildBackup(){
  return {
    ...
    ops: (typeof log !== 'undefined') ? log.map(function(op){
      /* دور ۱۰۰ (نقصِ ۵): اسنپ‌شات هم رکوردِ کاملِ کاربر (با رمزِ متن‌ساده)
         دارد — همان قاعدهٔ امنیتِ پنتست: رمز هرگز در فایلِ قابلِ دانلود. */
      if(op && op.t==='snap' && op.db && Array.isArray(op.db.users)){
        var db2={};
        Object.keys(op.db).forEach(function(c){
          db2[c]=(c==='users')?op.db[c].map(function(u){
            if(!u||!Object.prototype.hasOwnProperty.call(u,'password'))return u;
            var r={};Object.keys(u).forEach(function(k){r[k]=(k==='password')?'[redacted]':u[k];});return r;
          }):op.db[c];
        });
        var s={};Object.keys(op).forEach(function(k){s[k]=(k==='db')?db2:op[k];});return s;
      }
...
function validateBackup(obj){
  ...
  if(obj.version > 3) return { ok:false, error:'نسخهٔ فایل جدیدتر از این برنامه است' };
  /* هر عملیات باید ساختار درست داشته باشد */
  for(var i = 0; i < obj.ops.length; i++){
    var o = obj.ops[i];
    /* دور ۱۰۰ (نقصِ ۵): اسنپ‌شاتِ فشرده‌سازی (c ندارد — db/ids دارد) */
    if(o && o.t==='snap'){
      if(!o.db || typeof o.db !== 'object')
        return { ok:false, error:'اسنپ‌شاتِ ردیف ' + (i+1) + ' ناقص است' };
      if(o.ids !== undefined && (typeof o.ids !== 'object' || o.ids === null))
        return { ok:false, error:'اسنپ‌شاتِ ردیف ' + (i+1) + ' ناقص است' };
      continue;
    }
...
function restoreBackup(obj){
  ...
  try{
    ...
    /* دور ۱۰۰ (نقصِ ۵): بازپخش با همان applyLogِ بوت (اسنپ‌شات + دمِ __aدار
       + شمارنده‌هایِ تازه) — به‌جایِ applyOpِ خام که اسنپ‌شات را نادیده
       می‌گرفت و وضعیت را ناقص برمی‌گرداند. */
    applyLog();
    if(typeof idxReset === 'function') idxReset();
    saveLog();
    return { ok:true, ops: obj.ops.length };
