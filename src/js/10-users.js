...
function viewUsers(){
  ...
  return `<div class="card"><div class="card-head">
   ...
   ${filterPanel('users',`
     <select class="select" style="width:135px" data-f="role"><option value="">همه نقش‌ها</option>${['manager','teacher','student','parent','counselor','driver'].map(r=>`<option value="${escAttr(r)}" ${role===r?'selected':''}>${ROLE_FA[r]}</option>`).join('')}</select>
   ...
   ${slice.length?`<div class="table-wrap"><table><thead><tr><th>نام و نام خانوادگی</th><th>نقش</th><th>نام کاربری</th><th>مدرسه</th><th>کلاس</th><th>تلفن</th><th>وضعیت</th>${canEdit?'<th></th>':''}</tr></thead><tbody>
    ${slice.map(x=>`<tr><td><div class="row" style="flex-wrap:nowrap;gap:8px"><div class="avatar" style="width:30px;height:30px;font-size:12px;background:var(--primary-soft);color:var(--primary)">${esc(x.full_name[0])}</div>
      ...
      <td><span class="badge ${x.active?'b-green':'b-gray'}" ${canEdit?`data-act="user-toggle" data-id="${escAttr(x.id)}" style="cursor:pointer"`:''}>${x.active?'فعال':'غیرفعال'}</span></td>
      ${canEdit?`<td><button class="icon-btn" title="بازنشانی رمز" data-act="pass-reset" data-id="${escAttr(x.id)}">🔑</button> <button class="icon-btn" data-act="user-edit" data-id="${escAttr(x.id)}">✏️</button> <button class="icon-btn danger" data-act="user-del" data-id="${escAttr(x.id)}">🗑️</button></td>`:''}</tr>`).join('')}
...
function pager(page,pages){ if(pages<2)return '';
  ...
  return `<div class="pager"><button data-act="page" data-p="${escAttr(page-1)}" ${page<=1?'disabled':''}>›</button>
   ${nums.map(n=>`<button class="${n===page?'active':''}" data-act="page" data-p="${escAttr(n)}">${fa(n)}</button>`).join('')}
   <button data-act="page" data-p="${escAttr(page+1)}" ${page>=pages?'disabled':''}>‹</button><span class="muted small">از ${fa(pages)} صفحه</span></div>`;}
