...
function tomorrowChecklistItems(sid){
  ...
  var out={date:tomorrow, day:day, school:day<=4, rows:[], exams:[]};
  if(out.school){
  ...
  db.exams.filter(function(e){return e.class_id===cls.id&&e.date>=today&&e.date<=horizon;})
    .forEach(function(e){
      L.push('BEGIN:VEVENT','UID:payesh-exam-'+e.id+'@payesh',
        ...
        'SUMMARY:امتحان '+((byId('subjects',e.subject_id)||{}).name||'امتحان')+(e.start_time?' '+e.start_time:''),
        e.room?('LOCATION:'+e.room):'','END:VEVENT');
    });
  db.calendar.filter(function(c){return c.school_id===cls.school_id&&c.date>=today&&c.date<=horizon;})
    .forEach(function(c){
      L.push('BEGIN:VEVENT','UID:payesh-cal-'+c.id+'@payesh',
        ...
        'SUMMARY:'+(c.title||'رویداد مدرسه'),'END:VEVENT');
    });
...
function clientFeaturesCard(sid){
  ...
  var h='<div class="card" style="margin-bottom:12px"><div class="card-head"><h3>🗓️ فردا و امتحانات</h3><div class="spacer"></div>'
    + '<button class="btn ghost sm" data-act="ics-export" data-sid="'+escAttr(sid)+'" title="دانلودِ تقویمِ امتحانات و رویدادهای مدرسه (فایل ICS — با هر تقویمی باز می‌شود)">📥 تقویم (ICS)</button></div>'
...
const CF_ACTIONS = {
  /* ویژگی ۳: خروجی ICS */
  'ics-export'(el,id){
    var sid=id?Number(id):((typeof recordTargetId==='function')?recordTargetId():null);
  ...
  'quick-excuse'(el,id){
    ...
    if(!rec) return;
    var u=S.user;
    var ok = u.role==='parent' ? db.parent_links.some(function(p){return p.parent_id===u.id&&p.student_id===rec.student_id;})
           : (u.role==='student'&&u.id===rec.student_id);
    if(!ok){ toast('این دسترسی را ندارید','err'); return; }
