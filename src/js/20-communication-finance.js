...
function tuitionReminders(){
  var made=0, skipped=0, today=todayISO();
  db.installments.forEach(function(i){
    if(i.status!=='pending' && i.status!=='partial') return;
    ...
    targets.forEach(function(uid){
      insert('notifications',{user_id:uid, school_id:i.school_id, type:'tuition_due', ref:'tr_'+i.id,
        title:'⏳ یادآوری قسطِ سررسیدِ گذشته',
        body:'قسط '+fa(i.seq)+' به مبلغ '+rial(i.amount-i.paid_amount)+' ریال — سررسید '+jalali(i.due_date)+' گذشته است.',
