...
function installmentNeedsReminder(i,now){
  ...
  const due=i.due_date; if(!due) return null;
...
function printTuitionReceipt(tId){
  ...
  const plan=byId('tuition_plans',t.plan_id)||{};
  const ins=db.installments.filter(i=>i.tuition_id===t.id).sort((a,b)=>a.seq-b.seq);
...
<title>رسید تسویه کامل — ${esc(st.full_name||'')}</title>
<style>body{font-family:Tahoma,'Segoe UI',sans-serif;padding:32px;color:#111}
h1{font-size:20px;margin:0 0 4px} h2{font-size:14px;color:#555;margin:0}
table{width:100%;border-collapse:collapse;margin-top:18px} td,th{border:1px solid #bbb;padding:8px 10px;font-size:13px}
th{background:#f2f4f7}.tot{font-weight:bold;background:#f7f9fb}
.meta{font-size:12px;color:#444;line-height:2;margin-top:14px}
.stamp{margin-top:26px;padding:14px;border:2px solid #1a7f37;border-radius:10px;color:#1a7f37;font-weight:bold;text-align:center}</style></head>
<body>
<h1>رسید تسویه کامل شهریه</h1>
<h2>${esc(sc.name||'')} — ${esc(sc.code||'')}</h2>
<div class="meta">
دانش‌آموز: <b>${esc(st.full_name||'—')}</b> · کلاس: ${esc((byId('classes',t.class_id)||{}).name||'—')}<br>
طرح: ${esc(plan.title||'—')} · تاریخ صدور: ${jalali(todayISO())}
</div>
<table><thead><tr><th>قسط</th><th>سررسید</th><th>مبلغ</th><th>پرداختی</th><th>روش</th><th>کد رهگیری</th><th>تاریخ پرداخت</th></tr></thead>
<tbody>${ins.map(i=>`<tr><td>${fa(i.seq)}</td><td>${jalali(i.due_date)}</td><td>${rial(i.amount)}</td>
<td>${rial(i.paid_amount)}</td><td>${PAY_FA[i.method]||'—'}</td><td>${esc(i.ref_id||'—')}</td><td>${i.paid_at?jalali(i.paid_at):'—'}</td></tr>`).join('')}
<tr class="tot"><td colspan="2">جمع کل</td><td>${rial(t.payable)}</td><td>${rial(t.paid)}</td><td colspan="3">تخفیف: ${rial(t.discount||0)} ریال</td></tr>
</tbody></table>
${t.status==='settled'?'<div class="stamp">✅ شهریه به‌کلی تسویه شد</div>':''}
</body></html>`);
  w.document.close();
  w.focus();
}
