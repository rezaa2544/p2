 ...
 */
function reportCardCert(sid, term, tpl){
  ...
  /* قالبِ دوم (۴.۳): همان داده، چیدمانِ فشردهٔ دوستونه — بدون جدول،
     مناسبِ پرینتِ سریع / کاغذِ کوچک. همهٔ ورودی‌ها از همان مقادیرِ
     محاسبه‌شده (rows/gpa/rank/pres/absn) — هیچ محاسبهٔ جداگانه‌ای ندارد. */
  var bodyCompact =
    '<div class="meta"><span>نام: <b>' + esc(st.full_name) + '</b></span>'
    + '<span>کد ملی: <b>' + esc(st.national_id || '—') + '</b></span>'
    + '<span>کلاس: <b>' + esc(cls ? cls.name : '—') + '</b></span>'
