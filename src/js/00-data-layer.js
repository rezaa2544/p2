 ...
 *
 * خروجی (دور ۸۵, P1-2) — همیشه resolve، هرگز reject نمی‌کند:
 *   { ok, status, code, serverTime, data, error, networkError?, timedOut? }
 *   ok        ← res.ok (HTTP < 400) — تمایز 4xx/5xx از راه status
 *   status    ← کد HTTP؛ 0 برای شکستِ سطحِ شبکه/مهلت
 *   code      ← همان status (نگه‌داشتنی: مصرف‌کننده‌های قدیمی مثل
 *               42-self-diagnostics از r.code استفاده می‌کنند)
 *   data      ← بدنهٔ JSON؛ null اگر بدنه نبوده یا خراب باشد
 *   error     ← null در پاسخِ HTTP (هر status)؛ در غیر این‌ها
 *               'bad_json' یا 'network: ...' / 'timeout: ...'
 *   serverTime← سرآیندهٔ Date برای سنجش اختلاف ساعت
 * مصرف‌کننده‌ها (46-bell-now, detectServer) بدون تغییر عمل می‌کنند:
 * r.ok/r.data همان معنای پیشین دارند و مسیرِ catchشان بی‌ضرر می‌ماند.
 */
function httpGetJson(url, timeoutMs){
  return new Promise(function(resolve){
    if(typeof fetch !== 'function'){
      resolve({ ok:false, status:0, code:0, serverTime:null, data:null,
                error:'fetch در دسترس نیست', networkError:true });
      return;
    }
    var ctl = null, to = null, timedOut = false;
    try{ ctl = new AbortController(); }catch(e){}
    if(ctl) to = setTimeout(function(){ timedOut = true; try{ ctl.abort(); }catch(e){} }, timeoutMs || 6000);
    fetch(url, { method:'GET', signal: ctl ? ctl.signal : undefined })
      .then(function(res){
        ...
        return res.json()
          .then(function(j){ return { ok:res.ok, status:res.status, code:res.status, serverTime:serverTime, data:j, error:null }; })
          .catch(function(){ return { ok:res.ok, status:res.status, code:res.status, serverTime:serverTime, data:null, error:'bad_json' }; });
      })
      .then(function(r){ resolve(r); }, function(err){
        if(to) clearTimeout(to);
        var msg = String((err && err.message) || err);
        resolve({ ok:false, status:0, code:0, serverTime:null, data:null,
                  error: (timedOut ? 'timeout: ' : 'network: ') + msg,
                  networkError: !timedOut, timedOut: !!timedOut });
      });
  });
}
