...
DIAG_CHECKS = DIAG_CHECKS.concat([
...

  {
    ...
    check: function(){
      ...
      log.forEach(function(op, i){
        ...
        if(op.t==='snap'){
          ...
          return; /* اسنپ‌شات عملیات نیست (AD ۸۵.۱) */
        }
    ...
    fix: function(){
      ...
      var clean = log.filter(function(op){
        ...
        if(op.t==='snap') return !!op.db; /* اسنپ‌شات سالم بماند (AD ۸۵.۱) */
...

  {
    ...
    check: function(){
      ...
      var miss = need.filter(function(n){
        // eslint-disable-next-line security/detect-eval-with-expression -- شناسه‌های ثابتِ سخت‌کدشده (نه ورودی کاربر) برای resolve کردنِ اسکوپِ lexical؛ مجوزِ X2a
...

  {
    ...
    check: function(){
      ...
      var miss = need.filter(function(n){
        // eslint-disable-next-line security/detect-eval-with-expression -- همان مجوزِ X2a: شناسهٔ ثابتِ سخت‌کدشده، فقط resolve اسکوپ
