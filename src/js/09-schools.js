...
function viewSchools(){
  ...
  const fp=S.filters.sp||'', fc=S.filters.sc||'', fd=S.filters.sd||'', fl=S.filters.slevel||'', fg=S.filters.sgender||'', fa_=S.filters.sactive||'';
  ...
  const anyFilter=fp||fc||fd||fl||fg||fa_||q;
  return `<div class="card"><div class="card-head">
   ...
   ${isAdmin?`<div class="card-body" style="padding-bottom:8px">
     <div class="row" style="gap:8px">
       ...
       ${anyFilter?`<button class="btn ghost sm" data-act="school-clearf">✖️ پاک کردن فیلترها</button>`:''}
     </div></div>`:''}
   ${rows.length?`<div class="table-wrap"><table><thead><tr><th>نام مدرسه</th><th>کد</th><th>مکان</th><th>مقطع</th><th>مدیر</th><th>تلفن ثابت</th><th>دانش‌آموز</th><th>دبیر</th><th>کلاس</th><th>وضعیت</th>${isAdmin?'<th></th>':''}</tr></thead><tbody>
    ${rows.map(s=>{const us=db.users.filter(u=>u.school_id===s.id);const mg=us.find(u=>u.role==='manager');
     return `<tr><td><b>${esc(s.name)}</b><div class="small muted">${esc(s.gender||'')}</div></td><td class="muted">${esc(s.code)}</td>
      ...
      <td><span class="badge ${s.active?'b-green':'b-gray'}" ${isAdmin?`data-act="school-toggle" data-id="${s.id}" style="cursor:pointer"`:''}>${s.active?'فعال':'غیرفعال'}</span></td>
      ${isAdmin?`<td><div class="row" style="gap:5px;flex-wrap:nowrap">
        ...
        <button class="btn ghost sm" data-act="school-toggle" data-id="${s.id}" title="فعال یا غیرفعال کردن مدرسه">${s.active?'⏸️ غیرفعال کن':'▶️ فعال کن'}</button>
