...
/* ─────────────── دور ۷۹ بند ۶ (۲.۳): شاخصِ صریحِ کم‌برخوردار ───────────────
   کم‌برخوردار = خاصیتِ ساختاریِ مدرسه (روستایی/دورافتاده) — جدا از
   عملکرد (سیگنال‌هایِ «نیازمندِ عتِمَام»): مدرسهٔ روستاییِ پرفورمنسِ بالا
   هم کم‌برخوردار است، و مدرسهٔ شهریِ خوش‌برخورد با میانگینِ پایین
   «نیازمندِ عتِمَام» است، نه کم‌برخوردار.
...
function viewOfficeSchools(){
  ...
  /* دور ۷۹ بند ۶ (۲.۳): شاخصِ ساختاریِ کم‌برخوردار — مستقل از عملکرد */
  ...
  return `<div class="card"><div class="card-head"><h3>🏫 مدارس تحت پوشش</h3>
    ...
    ${upN?`<span class="badge b-purple" title="شاخصِ ساختاری: مدرسهٔ روستایی/دورافتاده — جدا از سیگنال‌هایِ عملکردی">🟤 ${fa(upN)} کم‌برخوردار</span>`:''}</div></div>
...
<select class="select" style="width:150px" data-f="ounder"><option value="" ${!onder?'selected':''}>همهٔ مناطق</option><option value="1" ${onder?'selected':''}>🟤 فقط کم‌برخوردار</option></select>`)}
   ${shown.length?`<div class="card-body" style="display:grid;gap:10px">
    ${shown.map(r=>`<div class="row" style="background:var(--surface-2);padding:12px 14px;border-radius:12px">
      ...
      <div class="row" style="gap:14px">
        ...
        ${_up[rows.indexOf(r)]?'<span class="badge b-purple" title="مدرسهٔ روستایی/دورافتاده — شاخصِ ساختاریِ کم‌برخورداری (جدا از عملکرد)">🟤 کم‌برخوردار</span>':''}
...
const P8_ACTIONS = {
  ...
  'exam-save'(){
    ...
    /* منشأ جلسه (بند ۳): درسی / نهایی / جبرانی */
    const src=(V('ex_source')||'internal');
    /* امتحان نهایی فقط پایه‌های پایانی (نهم/دوازدهم) — قاعدهٔ
       تک‌منبع در finalGradeOk (26-curriculum) */
    const cls=byId('classes',cid);
    if(src==='national_final'&&!(typeof finalGradeOk==='function'&&finalGradeOk(cls&&cls.grade))){
    ...
    const data={school_id:S.user.school_id,term_id:term.id,class_id:cid,subject_id:Number(V('ex_subject')),date,
      start_time:st,duration:dur,room:V('ex_room'),max_score:Number(V('ex_max'))||20,source:src};
...
function examModal(e,term){
  ...
  e=e||{class_id:(classes[0]||{}).id,subject_id:(subs[0]||{}).id,date:term.start_date,start_time:'08:00',duration:90,room:'',max_score:20,source:'internal'};
  openModal(modalTpl(e.id?'ویرایش جلسه امتحان':'افزودن جلسه امتحان',
    `<div class="grid g2">
      ...
      ${f('منشأ جلسه',sel('ex_source',(typeof EXAM_SOURCES!=='undefined'?EXAM_SOURCES:[['internal','درسی'],['national_final','نهایی'],['makeup','جبرانی']]),(typeof examSource==='function')?examSource(e):'internal'))}
