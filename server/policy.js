...
/* P0-18 (همزمانی خوش‌بینانه): `base_version` فیلدِ پروتکلیِ لایهٔ
   همزمانی است، نه دادهٔ رکورد — مصرفش با checkOcc است و در allowlist
   فیلدهای مدل نمی‌آید. بدونِ این استثنا، هر PATCH با پایهٔ نسخه
   «فیلدِ ناشناخته» می‌گرفت (تعارضِ ادغامِ policy مرکزی با P0-18). */
const TRANSPORT_FIELDS = ['base_version'];
...
function createPolicy(ctx){
  ...
  function validate(op, coll, payload){
    ...
    for(const k of Object.keys(payload)){
      if(TRANSPORT_FIELDS.indexOf(k) > -1) continue; /* P0-18: فیلدِ پروتکلی */
...
module.exports = { createPolicy, SELF_ALLOWED_FIELDS, REST_ALIASES, KNOWN_ROLES, TRANSPORT_FIELDS };
