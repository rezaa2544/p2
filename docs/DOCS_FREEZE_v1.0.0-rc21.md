# 🔒 قفل مستندات — DOCS_FREEZE_v1.0.0-rc21 (تاریخی — جایگزین‌شده)

> **⚠️ جایگزین شد:** این قفل با `docs/DOCS_FREEZE_v1.0.0-rc22.md` بامپ شد (2026-09-11، پس از پلی‌بوک ارزیابی و آنبوردینگ وندور). این سند صرفاً تاریخچهٔ فهرست ۲۵۶ سند ریشه در قفل بیست‌ویکم است.

**نسخهٔ قفل:** ۱.۰.۰-rc21 | **تاریخ قفل:** ۲۰۲۶-۰۹-۱۱ | **مالک:** چت ۶ (مستندات و انتشار)
**وضعیت:** این سند نقطهٔ انجماد رسمی کتابخانهٔ مستندات است و جایگزین
`DOCS_FREEZE_v1.0.0-rc1.md` تا `rc20` (تاریخی) می‌شود. فهرست زیر **همهٔ ۲۵۶ سند ریشهٔ**
`docs/*.md` را در لحظهٔ قفل با هش دقیق ثبت می‌کند؛ خود این سند عمداً بیرون فهرست
است (خودارجاعی ممکن نیست) و شمارهٔ ریشه با آن به ۲۵۷ می‌رسد.

**دلیل بامپ:** بستن شکاف «راهنمای عملیاتی روز-به-روز پایلوت وجود ندارد» —
`docs/PILOT_OPERATIONS_PLAYBOOK.md` (دوازده بخش: روال روزانه/هفتگی، تیم و آن‌کال،
کی‌پی‌آی‌ها، تریاژ تیکت، ارتباط با مدارس، آنبوردینگ، الگوهای پاسخ، قالب گزارش،
محرک‌های بازگشت و معیارهای خروج) + تست پوشش آن — مأموریت ۳۸.
قفل قبلی (`rc20`) به فهرست اسناد عادی پیوست.

**زیرپوشه‌های بیرون مانیفست ریشه** (۱۹ سند؛ پوشش ساختاری‌شان را تست‌های پوشش
می‌پایند و فهرست قطعی‌شان در نمایه ثبت شده است):

| زیرپوشه | شمار | محتوا |
|---|---|---|
| `RUNBOOK_CARDS/` | ۱۱ | ۱۰ کارت A4 ران‌بورد (RC-001 تا RC-010) + فهرست/درخت تصمیم |
| `user-guides/` | ۵ | راهنمای نقش‌محور: مدیر، دبیر، ولی، دانش‌آموز، کارکنان |
| `pilot/` | ۳ | اسناد عملیاتی پایلوت: معرفی مدرسه، فرم رضایت والد، راهنما |

---

## ۱) قاعدهٔ قفل

1. از این لحظه، **هر تغییر در هر سندِ فهرست‌شده** نیازمند **بامپ نسخه** است
   (rc21 ← rc22؛ نسخهٔ بعد با همین ساختار و فهرست هش تازه ساخته می‌شود و این
   سند را «قبلی» اعلام می‌کند).
2. اسناد «زنده» از قاعدهٔ بامپ و از تضمین اثرِ این فهرست مستثنا‌اند — ماهیت آن‌ها
   به‌روزرسانی ماشینی/رخدادی است: `DOCS_HEALTH_REPORT.md` و `DOCS_CONSISTENCY_REPORT.md`
   (بلوک ماشینی) و `SECURITY_INCIDENT_LOG.md` (رخدادهای تازه). این‌ها در فهرست هستند
   ولی اثرشان تضمین نمی‌شود.
3. افزودن سند تازه آزاد است؛ **ویرایش/حذف** سایر اسناد فهرست‌شده بدون بامپ ممنوع است.
4. راستی‌آزمایی هرزمانه: `node tests/docs-freeze-marker.js` فهرست و اثرها را می‌سنجد.

## ۲) امضا

امضای دیجیتال مستقل (جی‌پی‌جی) در این سندباکس در دسترس نیست (کلید خصوصی پیکربندی نشده —
قید صادقانه، مانند نسخه‌های پیشین). جایگزین زنجیرهٔ اعتماد:
- همین سند با **کامیت گیت** روی شاخهٔ نشست ثبت می‌شود (دست‌نخورده‌بودن با تاریخچهٔ کامیت
  و باندل تحویل `/home/user/pilot-playbook.bundle` قابل اثبات است)؛
- فهرست هش‌های زیر، اثرِ هر سند را به‌تنهایی قابل راستی‌آزمایی می‌کند؛
- یکپارچگی کامل کتابخانه در `docs/POST_RESET_INTEGRITY_REPORT.md` ممیزی شده است.

## ۳) فهرست اسناد ریشه با هش (شمار: ۲۵۶)

| سند | اثر (SHA-256) |
|---|---|
| `A01_PULL_BOOTSTRAP_REPORT.md` | `sha256:007f070f72e2ddba6a452289045747f640d9b9ebd66049f4d4b57318ffd32c99` |
| `ACADEMIC_YEARS_GUIDE.md` | `sha256:a358026e4e558309f90a0745cda27eccb09cd425919f2a21a1ef157c1d073b7c` |
| `AI_PROMPT.md` | `sha256:5a45db245c8817de1d13f2e95f9d808db98ed9c7fe1ebe0187162b1cf2ee7a89` |
| `ANDROID_BUILD_PLAN.md` | `sha256:eab3ea553e26dbd35b0ea45804950f2d0ef182515cda234ab0765108ac9deeb0` |
| `ANDROID_BUILD_REPORT.md` | `sha256:adf4d461fe4ba65fc94a9d7e8d7c22b28a059b11391e227e3266a871304554d0` |
| `API_CHANGELOG.md` | `sha256:9144ab8d486f1493cbe0e3921a946f9a57b9ff0f390c0e6249419766d9285fe3` |
| `API_REFERENCE.md` | `sha256:43a31a7a52d387f4c19ce234c455cabdec8591333de7383e1c9ca9a52f9630f0` |
| `ARCHITECTURE.md` | `sha256:a1f6e140f2a84aa1616e79769708be096b38fc02ee8eae8b03b20d2c974f5a9f` |
| `ARCHITECTURE_DECISIONS.md` | `sha256:bd66bcaa2e87748ed93f0a8f7b099a7451f516a076efdb612f79ddf79d6ad2d4` |
| `ARCHITECTURE_REVIEW.md` | `sha256:e40c46cd9dccb9e1108b7f8f89e569302d316462191a10ca0513aa001c5328da` |
| `ARENA4_PERFORMANCE_INFRA.md` | `sha256:1d9165d0eab6d6d02de494c56afc0766b83086e6bfcdaf9a0005d4b520292881` |
| `ARENA5_QA_RELIABILITY.md` | `sha256:0545b420c46f649cfeb2966be2c5bbc60982bb4f976bcf059aa6a18aef0435ee` |
| `ASYNC_ARCHITECTURE.md` | `sha256:39ae87912dedfebd04ee5afe34516378bf391b8a3c8a5edcdb4d381a5250f27d` |
| `AUTHORIZATION_MODEL.md` | `sha256:5f489da3c9e20231067bead51f35763cb8a18072af1dc118e90dd7c9163da2c3` |
| `AUTH_FLOW.md` | `sha256:2507804bf4595b8b32d179104ad03eb60a1a43275a4eec15078e1b0071e6e03d` |
| `BACKEND_PRODUCTION_ARCHITECTURE.md` | `sha256:c3f387a827f772ce6a2a4d311866d6b26987d130d36f38addad48cbbb2631e9c` |
| `BEHAVIOR_GAMIFICATION.md` | `sha256:9c53d8e73346af73a1934b3fc5f23776895cdf016a4bd878d4b29c4e129deaa8` |
| `BOTTLENECK_MAP.md` | `sha256:5c6be2301e932bc963ef9e18750343babf19d65af4e2acdf111cf0e76ad34c1d` |
| `BUG_HUNT_REPORT.md` | `sha256:f7159920d1ee593e4dbdfba5d4cf05dc15b3060f7c6bb1426dde923694e53396` |
| `CACHE_STRATEGY_DESIGN.md` | `sha256:9186d05e39cb8095593cfbe9cbcbd699bab519e41994255ebe3708102a3cca20` |
| `CAPACITY.md` | `sha256:f99be9d2fd2b4cbcb4e0d6c043071625b6473c26ddaed777d7248e8d30989738` |
| `CAPACITY_MODEL.md` | `sha256:6f939aced476d7b0e8e9f0155ec5c86ce348d7f4bc99a3cd49f5c87a3234ed74` |
| `CAPACITY_SIM.md` | `sha256:f98ec7cfde8b25a306c765c03cdc78afc092c21cf777de11d93605762ab961db` |
| `CDN_INTEGRATION_SETUP.md` | `sha256:b2290486612d534b5c8c2fa709be6a32721890852069228eb24c5e9908e8a9ff` |
| `CERT_REPORTS_VERIFICATION.md` | `sha256:3a09923997d49c6163449068f78d13c69eeae4cc8802232fa8b7064b3d3b307f` |
| `CHAT3_SUMMARY.md` | `sha256:d43eb1bc0d3595f7d3d8258d4c64595678755fd26d191a974375b97c561471d0` |
| `CLIENT_CAPACITY.md` | `sha256:52ed0463a717c893dc1a74dccc45df6c725e5045bfd2868b4368d0e5ae9b3abf` |
| `CLIENT_OFFLINE_ARCHITECTURE.md` | `sha256:e93b1c66a61d645912722e77ed18f19eea0b51dc9e6989f53e7555b771e5fee8` |
| `COMPLETE_REPORT_FOR_CLOUD.md` | `sha256:40831a86e47d1d516b18853d15e78909003a4ef9cf1a3be3864d282f317976e8` |
| `CONFIGURATION_REFERENCE.md` | `sha256:38d1d7f120a917495fdec3171facd7f9362f854d243f6bc3bda635d22213b488` |
| `CONFLICT_MODEL_DESIGN.md` | `sha256:be53b0a4086abd9477b6fc5ef98df04b8b850707ba13726f33f358e96d6f4ff1` |
| `D2_DRAFT.md` | `sha256:f62d2eff0772ce15393ed679e56ccfc3af174f702990109f479a0a81f3eb5349` |
| `D3_DRAFT.md` | `sha256:a4bdb239a1dd6610c8319c842c3274929acd203e854585b7bbf6806ee0a6fa92` |
| `D4_DRAFT.md` | `sha256:9537ea48b0740c56ede54faa37a557bddeb584bbae008d72d1bec1375b081ac2` |
| `DATABASE_ARCHITECTURE.md` | `sha256:f2dbd45af7bae8136004b4a1335fcf700a74a864f178c336f0f5dce29b17caaa` |
| `DATABASE_PERFORMANCE_OPTIMIZATION.md` | `sha256:2ac6df7ad07d0911a96c23e4e68043932d3183fae0851f1686c93971c9ae1a12` |
| `DATA_DICTIONARY.md` | `sha256:ffa198708a28618eca312e870f39507a7a9764da9d186fd72af98c30838580aa` |
| `DATA_FLOW.md` | `sha256:829de2c3e12ee96974d084524a3579823b534941eb678b5aca33a4de03d40fd8` |
| `DATA_SAFETY.md` | `sha256:97b3e3a14fca7a33a946ce680241a97c375a561ddd620688f97119bb5f878ddc` |
| `DEPENDENCY_GRAPH.md` | `sha256:2893954372f97d6cd0def200cbbe91319656e896c5f1dc4fb1253cf746a0bebc` |
| `DEPLOY.md` | `sha256:f501bc8a827c136394a6431d7e86665fc25e30478db736ea83e687b60e6e91c4` |
| `DEPLOYMENT_GUIDE.md` | `sha256:145dfd6cf24f85d2923e9824ec8a1ec9b016623d14aacaadf726d5cd2f279a05` |
| `DEVELOPMENT.md` | `sha256:36583814876f4be82e6963dcf025160618576ed55a342296687ff942890af12e` |
| `DISASTER_RECOVERY.md` | `sha256:cb63210eab050d3ed3adcff527d31d6d2b44f2ad4ae7937761863d51a3e100b9` |
| `DISCOVERABILITY_R43.md` | `sha256:b0be575e99a75fadc310ede35eae24a63fd4e11a525d85e1947fb11d8b76c5ca` |
| `DOCS_CONSISTENCY_REPORT.md` | `sha256:34f8315f37b496e3d6029c964d39ca4506f8c8417fc36bdfa9dde4277da1fe91` |
| `DOCS_EXPORT_GUIDE.md` | `sha256:c5064687de82757e6bbbe10d35dfd29f2d37effe960f146511aefdd86c456fb5` |
| `DOCS_FREEZE_v1.0.0-rc1.md` | `sha256:54a86e2f181e88a1c7062d0ecd7303ea4ed0fe080c627b272b977f2eb7709254` |
| `DOCS_FREEZE_v1.0.0-rc10.md` | `sha256:0ae335dbeee27185dad172fb75ec395d5e7e0b56791313212a13692dbee8a7ad` |
| `DOCS_FREEZE_v1.0.0-rc11.md` | `sha256:1511dd1ec228d8f33d6eb2d960a575d63ef968f06db00e040648f9490645bf5f` |
| `DOCS_FREEZE_v1.0.0-rc12.md` | `sha256:15adef2d4b3af023b5b9e45fe40503f34e4f9f7168ef0cd68e81ee683f9c5221` |
| `DOCS_FREEZE_v1.0.0-rc13.md` | `sha256:4922094b86b8e456b73a78705d673668e7ad0b37598e70be5d611f70d62e026c` |
| `DOCS_FREEZE_v1.0.0-rc14.md` | `sha256:693e639d4a5d1c053acb8a276d7e860fe89c71688bf1ea09d2e5a1637a652cd7` |
| `DOCS_FREEZE_v1.0.0-rc15.md` | `sha256:12e182ac6f44cff6b2ce08aaad34c4f448bdea0a402e837550ea80a91d4ca5c7` |
| `DOCS_FREEZE_v1.0.0-rc16.md` | `sha256:bc7f667140a1c931adf58cff1cb164125e35747cca8a00fbf3a9db9a66836c21` |
| `DOCS_FREEZE_v1.0.0-rc17.md` | `sha256:22e5e3835fe950129c36d6bda837885bbb449d2214f6828fb74fb256cfeabf7a` |
| `DOCS_FREEZE_v1.0.0-rc18.md` | `sha256:e8d6fdebcd621827b33a0d35ac4434c02dc5a2baaa82b9b2caf0fc73f51661e5` |
| `DOCS_FREEZE_v1.0.0-rc19.md` | `sha256:149510c891692a98c5fb4c21c375cfffbe9ba540810b3a3b958aec46e8086f95` |
| `DOCS_FREEZE_v1.0.0-rc2.md` | `sha256:f103652075f3ffd52cf8dbdf7a47df5d4f1a09f112b032627822ce667df7d953` |
| `DOCS_FREEZE_v1.0.0-rc20.md` | `sha256:0a1714c8a8689f37cf92dbbf4f6f7dc9d283d950c51627b01777f4b362e1b103` |
| `DOCS_FREEZE_v1.0.0-rc3.md` | `sha256:dde6513623ab09ee2d0a52ef608118999669c145f562e6288e58340894628e77` |
| `DOCS_FREEZE_v1.0.0-rc4.md` | `sha256:c33ea913bef9630a24e34ab0348980e291570cad81d906fcf57908df96fdff73` |
| `DOCS_FREEZE_v1.0.0-rc5.md` | `sha256:df97a36b50b8e4398fd8ac3090b1a2bd7ed1a113780ec8841c8b44e490dce79e` |
| `DOCS_FREEZE_v1.0.0-rc6.md` | `sha256:ded449054c3464e7c34fc913cf3f71dc5745ffe24e34888cc1aceb94ee93782e` |
| `DOCS_FREEZE_v1.0.0-rc7.md` | `sha256:f0bbd20a82f5d700283703a265d3fadbadc49198d442ebf10950441bf27c55c4` |
| `DOCS_FREEZE_v1.0.0-rc8.md` | `sha256:361dfe21c653239ddad6873c394c454a5b2bdc1deb067f32de0686f08d96a205` |
| `DOCS_FREEZE_v1.0.0-rc9.md` | `sha256:854b8da74ff9cb7a746499afe532610ee4adb569736f07a2eb8f5eb133f378ab` |
| `DOCS_HEALTH_REPORT.md` | `sha256:e86212387610e74040fdb99c697ee3750d697ed2518286bcaf8fa9ff673d523a` |
| `DOCS_INDEX.md` | `sha256:6ae8f95c31c6b9a3ac6a89a024afaa0148f244af41457a15272c3c9925178071` |
| `DOCS_METRICS.md` | `sha256:719b5ffafdc6751f90e395e01d7776f88da1efa7c341d7b41b9f219baab5f1b1` |
| `DOCUMENTATION_HANDOVER.md` | `sha256:ce1cac98fa55fdc36a05d4e6f633ca3bc1aa3ab53a513006183cdb8f322c852f` |
| `DOCUMENTATION_MAINTENANCE.md` | `sha256:bbca0ce31bc63dabaacf2f2eab15f2ac2de60528a53de9a34985ef1ea41ee091` |
| `DOCUMENTATION_MAP.md` | `sha256:73ec09b788461a8108fbd1fafa32d7b73fd4713d92f0207f5493ea25006674a6` |
| `DR_RUNBOOK.md` | `sha256:d41181a05ddde65f0ee0db4c3203509184ccc181b409e62afd6e6bb00b4af035` |
| `D_MAPPING_CLARIFICATION.md` | `sha256:3a46eb50802790fc057c021a1f3012dc1e5b30395e0fef7a864a0a0ecece8b7b` |
| `D_OFFICE_LEVEL_ANALYSIS.md` | `sha256:339579559535bbd57556f6efae8ccdb4ff601984f01457a2a04a97b6ce6db826` |
| `EXAM_TYPES_GUIDE.md` | `sha256:bfc9a35481de6bd64c48f31bca4396951fc41ea1aedbc6070260fef9ea026354` |
| `EXECUTIVE_BRIEFING.md` | `sha256:bdde779c2514d44ffd40fb058cb9f0188403684b1343ae082b48d405d9b07d72` |
| `EXECUTIVE_SLIDES_OUTLINE.md` | `sha256:48432a8aba0966d0604cb3e1a82d4b29b2089ca9e1024825ed0ce3d5fb8a1ab6` |
| `FAQ.md` | `sha256:b8609cf3ec063a343fa9bf50a708df5a8c298c675df8024302bbd20f49c1547b` |
| `FARNAZ_PHASE1_FINAL_REPORT.md` | `sha256:7c370d0cc8bc69c85bb29f3af78857e0c99eb8ff5ecf17ccdfd78b0d03524a51` |
| `FIXES_ACTION_PLAN.md` | `sha256:456eb5dfbe0b907be425e4fc24255de935b48d50f89e77135738d413b85baf53` |
| `G1_HEALTH_INDEX_DESIGN.md` | `sha256:9e4f0ad92cd70a9b4fe68a31459f99676b36e111d27e5dcb59b5a4ba49f58b27` |
| `G3_USER_GUIDE_REVIEW.md` | `sha256:bf8ac3acff733ee58119a8c08f6dfb7854b448df3ae92145d4dee82affd729b1` |
| `GAP_ANALYSIS_R43.md` | `sha256:e9877a3d16e9c33d66f2cf89296794c295d81f3fba0dbfe80b1feb036625ddfa` |
| `GITHUB_COMPARISON.md` | `sha256:3a57d084d3f419f6799ef4ad7d49894b78418a61784ce9bbb3a21b7fe9cdc381` |
| `GLOSSARY.md` | `sha256:6c57f358053b05223da135eeb892487a8921594978b320d37046290cda145e26` |
| `GO_LIVE_PACKAGE.md` | `sha256:6b95a9ba516ff20327a51d348c103866983560b6c41b5dc9f3a467e755a76946` |
| `HANDOFF_ARCHIVE.md` | `sha256:812efbb71396089dafa22af9a47570bc250c902cf8c5ce5a4807f8301ca4a763` |
| `HA_POSTGRES.md` | `sha256:8a73c7c0d783b0c8139e518bfdfbd709109adb585170e313102f1a4c3bbca677` |
| `HA_REDIS.md` | `sha256:aaabbaf59318616ef520a002de1e096e37b500e4c27592f67ac0627ca7854a34` |
| `HEALTH_INDEX_MODULE.md` | `sha256:060947c6021b4db1332c15360a6bd2068ab77b134f2263cf19d2c014130f186d` |
| `INCIDENT_RESPONSE.md` | `sha256:681ff5faea8aed823ec35d2dd760e5c63cfd9f9be652b06082a5dbfed61ac3fa` |
| `INSCOPE_OPTIMIZATION_PLAN.md` | `sha256:85e3f0260e008e7c71ae9a4ed3a943f94546ff33405750d176dd4b6fcadd9f06` |
| `INTERNSHIP_MODULE.md` | `sha256:bfaef4d22a634fd68bde60ad2517e42ca2cf0f7e6ed259baffbd5b8f7af5be99` |
| `IRAN_COMPLIANCE_PACKAGE.md` | `sha256:2f318f6f7047763c1f4316a8ada62fbdd1565839d6be62acbf7967fd476cd832` |
| `LESSONS_LEARNED.md` | `sha256:b88d8c5fa00c75641996ffb5fbccdb6e2c27152ae20591f89276c5423f3bb8c7` |
| `LOAD_TESTING_PLAN.md` | `sha256:45e33479294382c36fa1d468268ee4e6841629a25b7c2b6b49875018d0ee89dc` |
| `LOAD_TEST_PLAN.md` | `sha256:07ff36bfd025f7acf1eed95823ff0385ea95f28a36cb30f43145715e247339f1` |
| `LOAD_TEST_RESULTS.md` | `sha256:efb483e293b8a7ce2bd6433f11db3952a6334654d2a0f1fae74fb603dd2f4a1a` |
| `MIGRATION_GUIDE.md` | `sha256:69f9e640c217126d1e359ff74e325fda8cdec09f0d46a53217b04a533974c4fa` |
| `MULTI_INSTANCE_ARCHITECTURE.md` | `sha256:67e0b9447275483f25159fc8034546c8eeacf925d5b7ea594d929453a4dc1146` |
| `MULTI_INSTANCE_AUDIT.md` | `sha256:767bda832dc565cb74577d58eb2708ae3245be8fc52713ef6a75f7b08652f8d8` |
| `MULTI_INSTANCE_READINESS.md` | `sha256:76b91801a731da1f5c2501dccb0d90200347463945dcb1f32ce52fb2f93b91f9` |
| `NATIONAL_ARCHITECTURE.md` | `sha256:15351fba08418f4cbfabcf68920c2411e84394b83b152e10372f28c00f576ed9` |
| `NATIONAL_BASELINE.md` | `sha256:6eab76a129383f4290015e76b07e657c7bba0144fe20f2999462ce0e3c23cadb` |
| `NATIONAL_BASELINE_PART2.md` | `sha256:be89b97781a5f3bc2876d60fe7c6a54d261fafdee653148df66f75d5354f34ff` |
| `NATIONAL_BASELINE_PART3.md` | `sha256:66b2bf5e2df32e471a0b0b5e44a733764d9c01e2557aecb94a58b2f58b054d76` |
| `NATIONAL_BASELINE_PART4.md` | `sha256:68d5446b6a2f2334ae8748412923431c6a3ad0e952dc70387dcc804d85bb5e01` |
| `NATIONAL_ROADMAP_ARCHITECTURE_ADDENDUM.md` | `sha256:911f99cda41d415193f9179f5bb6183a6242c769f28a3471af38abb5bb81d228` |
| `NATIONAL_ROADMAP_PROGRESS.md` | `sha256:813618514ab9ae9d6b4883651270ad939411566a8426c639a94c9861884b9ad1` |
| `OBSERVABILITY.md` | `sha256:a1cc14e91c53927c4cb114eb67270bf089fbbe5f7ab1189b08846b66bc649d1e` |
| `OBSERVABILITY_DEPLOYMENT.md` | `sha256:efaf413b9130c57a5473984bf24e0652f21fa7fa57099a0bf68ba191473822a9` |
| `OBSERVABILITY_LIVE_SETUP.md` | `sha256:a99b046779fe46883733858080f338ee014c79c438db827da2d605037320d34c` |
| `ONBOARDING_CHECKLIST.md` | `sha256:138a99b9bb129b9a0ea31db4d7565e24e5399d022b164b4b4ca85119f77f6f0c` |
| `ONBOARDING_NEW_DEVELOPER.md` | `sha256:4dbad99994690b6bfcd0ab61e3f906497d4c982efb903dbe6b9d05866dac44cb` |
| `OPEN_ITEMS.md` | `sha256:af2bf5b99d4aecf6b0193d457a8add545ecefe38d9098ad78403bfd11477dec3` |
| `OPEN_WORK.md` | `sha256:d4ea868a8162d00a962a06e812d1d6dc5488317fbf9d977e682240294b6bd924` |
| `OPERATIONAL_HANDOVER.md` | `sha256:c15d9f91817649d542684635fddf963d070c4489373a0b5801bfe596f6906987` |
| `OPINION_2026-09-05_ROADMAP_REVIEW.md` | `sha256:ae4998223e48d65fe3094895bfdc44a70013c8150b7c0389d7bffb6c14ba89bc` |
| `P0_BLOCKER_TRACKER.md` | `sha256:c6d35faa52b70f7a11b52264f4b6fd03570cfed5d682b986743f7c74274996bf` |
| `PEN_TEST_CHECKLIST.md` | `sha256:775c62ef672aa56cb1ae3a6461215e3e183cbe7f208648d3fe0dad9134a85c58` |
| `PERFORMANCE_BENCHMARKS.md` | `sha256:0b38bacf5e44f941b5f668fa8efc8a804a8b5dd06505587ad2d7888bd2f7e15e` |
| `PERFORMANCE_TESTING_PLAN.md` | `sha256:a0650a9a2957a0c18a8e77af72e09cecd050cce74c98a0a38ecaff52377af7cc` |
| `PHASE2_SUMMARY.md` | `sha256:8ad69b7618890eb7ae22d1daaedd64790ecc1aa28bd67fe2ce81525b6454a312` |
| `PHASE3_SUMMARY.md` | `sha256:11bb4d68dee1aa12713710dcec676901b83c20c47c28a2b6fc864b14e257e5a3` |
| `PHASE4_COMPLETION_REPORT.md` | `sha256:ff994f2ed7d88dd81bb1d81fd52b5d41faa3a282f11ec463eb4f8b7582bc8389` |
| `PHASE_2_MIGRATION_REPORT.md` | `sha256:a9d1e99320a5e6c4905c89d782d433b753125ab03bfdd2156573d6ed9057cf93` |
| `PHASE_3_PRODUCTION_API_REPORT.md` | `sha256:94da5039f637d09dbf10c42261e9166c7f46769544ae84fa1779c43b4cf5e898` |
| `PHASE_4_REDIS_CACHING_REPORT.md` | `sha256:66072a67f3aac2a82882e12d7e36717a0271823b6712a06b3c6d3a75c171a8a1` |
| `PILOT_KICKOFF.md` | `sha256:634d6f6210b202c115fd8740203096428b8bd0db7d9e4955a0ba0bd2ab63f9d3` |
| `PILOT_ONBOARDING.md` | `sha256:42ed47771f0d84700f62ebfc1798bd6f75004f69e444da79e5f50be46945a361` |
| `PILOT_OPERATIONS_PLAYBOOK.md` | `sha256:68b7177ca7c9657fe08793875c383aa9ba84e8e2b0e85cf8e591f8cf80b12e9b` |
| `PILOT_ROLLOUT_PLAN.md` | `sha256:2368297ffb363eb6f58c4f8d53de82626cfa953f30a873814fd41cdc5a094f9d` |
| `PLACEMENT_PLAN.md` | `sha256:591f0969eda04ea3b3701ebe9d83e0a18111bbeff9f56e78a99f61510ddb58a6` |
| `PLAN_ATTENDANCE_STATES.md` | `sha256:07e2ddc2fb4b1a13a7b9e6b1b56b3a05e2fc0fc86d335939857f13cc57e297bc` |
| `PLAN_BELL_AUTOCLASS.md` | `sha256:525dc949a65c88f1e28d61994fdd95daa091cfc3ae010fc820c1cd3ca9a48c4f` |
| `PLAN_CALENDAR_TIME.md` | `sha256:94d7537d4e276ce284a8b4850f2f237cce11d57d68586f4b568f31b592dba8d6` |
| `PLAN_LIVE_SCHEDULE.md` | `sha256:d7878e9892422a12fc5621f9dbaf76a49bd3efdcc830d897d3366d4fce6361e5` |
| `PLAN_MULTICHILD.md` | `sha256:4be4098a88ece3a1fa26d0a3f4083786bebb4d09f46f77e9174368996af0649a` |
| `PLAN_PHONE_AUTH.md` | `sha256:2828b36deaebb4fe0ab7c283e6bd3dc9117743ad094dfc1d83366b123fa22450` |
| `PLAN_SMS_GATEWAY.md` | `sha256:3758aac7113ddc69f0cc434988edba2093be5cf50678fa03f1932015dd878b7d` |
| `PLAN_VIRTUAL_CLASS.md` | `sha256:3dba8eea54a8ca8630c82a519c0f536a0fa920d83c698a1f7f98523f902d6c5e` |
| `PLAY_STORE_CHECKLIST.md` | `sha256:281839aec3cc835fe0a6b76dc6be9d9f19cc35128053906cd972aa37c0f2d060` |
| `POSTGRESQL_MIGRATION_PLAN.md` | `sha256:3595d8f2aa4bbe997af1add7305a7afcdcd44402d5182e61fb50b6e49f70b533` |
| `POST_RESET_INTEGRITY_REPORT.md` | `sha256:e7561e1d66f1d0cd0d72b2973532fcfb63a1ebd0897dff014a03d12b610f9c8a` |
| `PRIVACY_POLICY.md` | `sha256:94651d72ec3ef03777f007cca01ca34aad138350472bbf495beb288b5fb08757` |
| `PRODUCTION_READINESS_CHECKLIST.md` | `sha256:ef3afeae83020d58a1edf88ba77bab2c6afe42246dff71773597f52511e5258a` |
| `PRODUCTION_RUNBOOK.md` | `sha256:c0ace61e14145b2bc00fb2fb5817199066eaa5f3b9023fc18741d10fb8de326e` |
| `PR_MERGE_PLAN.md` | `sha256:a5108641807592670de84c52d6adbb453a4a79ffaac3aa0d055b8ef65ede27ba` |
| `PULL_BOOTSTRAP_DESIGN.md` | `sha256:b2266cdf6bb3433d7ad322f43809f7d300c6875f5591ecb1d59aa63b26b2927f` |
| `RATE_LIMITING_DESIGN.md` | `sha256:e8ea50174a94f8af8fe7a002b8c59e20cc0d825fc05115776a87735756a55866` |
| `README.md` | `sha256:fec77da4d3f6406d00c04d1be92f5913ba66869eb71b5ab24ae3e4cc9fd27e42` |
| `READY_STATE_2026-09-06.md` | `sha256:250fe21f2892a8d18fdd02971f6d4570f518e0045d762f465f9b8dddf1a79869` |
| `REDIS_CLUSTER_SETUP.md` | `sha256:b554055c75f300dcc96469ffd2542c5bfca61039d188bd703842831aad799203` |
| `REDIS_KEY_OPTIMIZATION.md` | `sha256:7e4c484d0e85bdc55366642c079473cbbd25eb3ff48e72b9e84aa7d48b62f330` |
| `REDIS_RESTORE_PROCEDURE.md` | `sha256:60f84845549135e8270ac2732d20bc538a481dfd5c321a182a1e6311fa04cf5b` |
| `RELEASE_GATE_CHECKLIST.md` | `sha256:6959441852029e2aecf5bae8ad28de3c8c753a34c1f5cbd37d80924933462676` |
| `RELEASE_GATE_EVIDENCE.md` | `sha256:c1b52cb02299401a23e8fed0ff31c90ea5f7200bf3f0acb3d89e1c7e61ce6384` |
| `RELEASE_NOTES.md` | `sha256:bf521966a3978c64f446c7d59d4fca1c9b646b694f8843befaa6b0653c10b928` |
| `RELIABILITY_DR_PLAN.md` | `sha256:797d8cb0ec5c4542bf849045936505d80d59a1ee92f8eded7bec965fd37caad2` |
| `REPORT_2026-09-05.md` | `sha256:d0059ac9a148be5445ec2f5c93f00f48c36e6c75a3f1fa65c92fc41bdc664d7f` |
| `REPORT_2026-09-05_CONSOLIDATED.md` | `sha256:361b157620a0e4ffa824bc71b6ba1eee8b7c1619c8818ce38b6a85c83bcfa6dd` |
| `REPORT_2026-09-05_DIAG2.md` | `sha256:3bf7d83dca7da390318ecb4bf834dd0bdd1ae3cbd4201df55abeec31d645b6f2` |
| `REPORT_2026-09-05_DIAG_GUIDE.md` | `sha256:42837767feccf9270f7980e14c738a68d8bf6783f3970e9809c5255f64ae30f5` |
| `REPORT_2026-09-05_HEALTH_GAUGES.md` | `sha256:382b9a00f96d5cb671adc6d303f7a744616ee0e979859a82420a7fddc6bb9dc6` |
| `REPORT_2026-09-05_HEALTH_GAUGES_V2.md` | `sha256:003ecce09846df895377dcfe63dd898c3377cd876770bc462f3ed9c8fcb06deb` |
| `REPORT_2026-09-05_LOGIN.md` | `sha256:7c0515a3c206be652a6bd45e9a56ae70bd78ed76dbc9c3525c6df62e9ccbc715` |
| `REPORT_2026-09-05_ROUND2.md` | `sha256:31f850e7e832635aeb2f63a002d95df1748d3f9264952d4616c78479368c01d6` |
| `REPORT_2026-09-05_ROUND65.md` | `sha256:84a9aad5f9f45685fa722d50876f5dbfaeb16c6ba0835362ee730672d0175f77` |
| `REPORT_2026-09-05_SECURITY.md` | `sha256:0b4b2e713d80f9c3a15ce751c537baf61d7bf77d7f08081a3cc399bc1b23cbeb` |
| `REPORT_2026-09-05_SIMULATION.md` | `sha256:69659820ba5029ada2e0be693d9324417d2a3aee2263b20a2db202fd39dadea0` |
| `REPORT_2026-09-05_SKILLS.md` | `sha256:4a1e0466717d374f4190f053408eb9f431a72e16f7f454d6094af0a8f5375659` |
| `REPORT_2026-09-05_STRATEGIC_BATCH.md` | `sha256:e3ae3fa4cd25fe61d4327471f3a42d3b95c9612bd726bd55706b26c1e838073a` |
| `REPORT_2026-09-06_ACCOUNT_DELETION.md` | `sha256:37bdb3070e8d385b7e0c01ba6405c9514f3cda1d201145144aa146b745469fe9` |
| `REPORT_2026-09-06_AUTO_BACKUP.md` | `sha256:d33f1143d034445cd99f6c19555fd4729a309f0708feccccd7d17913dfd1d351` |
| `REPORT_2026-09-06_BACKUP_RESTORE.md` | `sha256:1069091a4073e6790ac7f73751c9cac8959610776de7f83611d76b9ee38f14c3` |
| `REPORT_2026-09-06_FINAL.md` | `sha256:7edfc2ad9d16fd31ff854d4a786009f3bd64a9f6844a3c6451171c865e96311a` |
| `REPORT_2026-09-06_PLAY_PRIVACY.md` | `sha256:899d378f26e5b931b18f148eb1f24e42078d8cbceb25e49d561c7539dc579895` |
| `REPORT_2026-09-06_ROADMAP_AUDIT.md` | `sha256:700aecd68c175fbc51e6ff99b2a75bac786096df21a878cdaec3d91fa23f0c43` |
| `REPORT_2026-09-06_ROUND69.md` | `sha256:c2e3b2c3569b4773430d5899a1e02d8b7a2e16a6666f6a8dbba9fb412b9cac54` |
| `REPORT_2026-09-06_ROUND70.md` | `sha256:c44de3f3f1ab7ce0483116dc42fa545e72f4bbe55ad90597647b038623f95ce5` |
| `REPORT_2026-09-06_ROUND71.md` | `sha256:af83d02a2d764e9409751f08314686549efd4085bb04567fd016362f0b1dd0e8` |
| `REPORT_2026-09-06_ROUND72_SIMULATION.md` | `sha256:d7d845572e5c6beff24b7d1b7a218872420776f043692febb6a4f0b535f45eb1` |
| `REPORT_2026-09-06_ROUND73_SECURITY.md` | `sha256:0f60f3202ea425867813ab829ae9d5bc325b7499dbdd1eeac4e197dce43f9b9d` |
| `REPORT_2026-09-06_ROUND74_INTEGRATION_PRIVACY.md` | `sha256:d64dc1d6061789ea98a2b9e1f6860bb3ba4de3849c1f515d1463d34bc0866adf` |
| `REPORT_2026-09-06_ROUND75_EXIT_TIMER_LATE.md` | `sha256:4c9ae4477ef54d35c9e6117d1ee21ebabf7e14b2e0a6555cc3b4f3961d8bcbb3` |
| `REPORT_2026-09-06_ROUND76_DROPOUT_READINESS.md` | `sha256:e927969590e2ec218e859dea1b1f291e322b80684a05b611236b483346a65cfb` |
| `REPORT_2026-09-06_ROUND77-2_SMOKE_CRASH.md` | `sha256:bcc417c08ece24a2166dc23118b3e89a20bf9f13901a7f9e8bdfbb55be2f2f72` |
| `REPORT_2026-09-06_ROUND77_ATTENDANCE_EVENTS.md` | `sha256:6ccc550c539295e79c69efd73b99c6fa92470315feb6ae7793ce4ab40f0f30b7` |
| `REPORT_2026-09-06_ROUND79.md` | `sha256:e7f86be268e8b2dd88095ad967153afe78094490ae60ea32410c31b6675fe715` |
| `REPORT_2026-09-06_ROUND80.md` | `sha256:32ea55d1651f3b9d8babd5da4ba02ef688196737a868029a351962d2ad131b1d` |
| `REPORT_2026-09-06_ROUND81.md` | `sha256:f2cb155019108daac34db206f1c7c4437cdffe33b8fbf6032992a53a6d7537d0` |
| `REPORT_2026-09-06_ROUND82.md` | `sha256:6302fd65b2e34b19b1ee71265b058b91223e4e0b6f532efeea3792293f1e4f10` |
| `REPORT_2026-09-06_SERVER1.md` | `sha256:3f8b76bd637d4ea8f7473ec813a4b61694aaad1572c6634dd9992c059d7137d4` |
| `REPORT_2026-09-06_STAGE2_TLS_BELL.md` | `sha256:b3b69ddb315744aeea1da71fb30fc2c54be7a866041b09f6fdb9e57b460a34a8` |
| `REPORT_2026-09-06_TIME_BASED_ABSENCE.md` | `sha256:303b002b081144f4d990c72503bc73a4e3e510695bcfa9f1a6ade851f5677b8f` |
| `REPORT_2026-09-06_VIRTUAL_DAY_SERVER.md` | `sha256:e47e44d6731287f687d6e9eaf64d4888ffaf1df45635be95b88df056e96428c8` |
| `REPORT_2026-09-07_ROUND83.md` | `sha256:18bde4fd02e600c886d9538cea677394f4ab3d2aa4e97cc068c7bc61cb79f382` |
| `REPORT_2026-09-07_ROUND85.md` | `sha256:8ab54d3c6ef7558d04ec324d32f8791c24e408701a97d5d79c4a86aea7df3e50` |
| `REPORT_86.md` | `sha256:8ba9e5d13786d7f1f3f8cb30deaf7022d3b3c256ca776d4775a938dbc46b4793` |
| `REPORT_87.md` | `sha256:dbbd8d2822cec9fbf2daa51be76e2a4bb4c852778f263df30c99ff911f78ea33` |
| `REPORT_AUDIT_HARDENING.md` | `sha256:38e350686df3308d8eaeb5f8b7ac6b63b983c6e1897efbfbbca5fea84e775ffb` |
| `REPORT_BATCH11-13.md` | `sha256:a4d2e2f0a66de6ae9fc8235a673eb5e91698516978fd9f3c0a93efab63b4cf75` |
| `REPORT_BATCH8-10.md` | `sha256:ded217bd8d051610f21e76778ff926891152e69dcd62b451667280357ddd9a19` |
| `REPORT_FULL_PERIOD_2026-09-05.md` | `sha256:02b7ee41d3270f1684b52277ed8dc5221620412736b221b2d92482df941fc8a5` |
| `REPORT_OPEN_2026-09-05.md` | `sha256:bed47e123b34e7d8cc14c18ff335493d79261aae2e54607946d522f9ccbacec9` |
| `REPORT_ROUND89_FINAL.md` | `sha256:2a8e639ecaff7fe96764fcd7a21a0af0bf40f4fdfb37afef78b08b5217afc805` |
| `REPORT_SCROLL_FIX_2026-09-05.md` | `sha256:6e9ea3a604aed1146ae0577c7ffc8c32d93389cee469c0a7f310836a8ed70505` |
| `REPORT_UI_REVIEW_2026-09-05.md` | `sha256:7870b4de011dd12dd4cbce88baa1b86005d036a059ddf6877310ea252a1986b0` |
| `RESEARCH_2026-09-05_UNIFIED_EDU_ROADMAP.md` | `sha256:95367f41cee304112aa091f10401669596a3c38edfd39a24b0bf6a1f18248c86` |
| `REVIEW_PR2_FOR_SUPERVISOR.md` | `sha256:b64f2e6b188affb37f3762a69bd73fb940696d152bc7e9a7dbb52b124e1d66c1` |
| `RISK_REGISTER.md` | `sha256:c87cf044e3bc29982566b0b9dbdd8df53b885fd64a168c7a15b8b7e270a2cffe` |
| `ROADMAP.md` | `sha256:b67586dd16d5f3a3d29c8a222f226b6a9597bd74d1f30342b250560344381fa6` |
| `ROADMAP_FINAL_SCAN_2026-09-08.md` | `sha256:c99e4429df6c87800e6adc135039a69cdf96731bf76c913b625624df3bd1a86d` |
| `SCALE_10M.md` | `sha256:a7c09ae15eefc43526fdc2764b17b072dfef1c0101bcde50efe5030b079c8cc4` |
| `SCHEDULE_GENERATOR.md` | `sha256:5dfb1e5d2da3d3e2c4a5e7a8266dcd955afd53342f3dbbd3d27bc157d12c37a5` |
| `SCHOOL_TYPE_GUIDE.md` | `sha256:ba313c058be76ec641609d4ea5e3fa59f92c4359f4c0569ba444d4ea0c6f7994` |
| `SECRETS_MANAGEMENT.md` | `sha256:3a452fde27d1a9266e5d2e68cf12621cff8695a11d7e8a0ce6d702cecc5e6413` |
| `SECURITY_FINDINGS_REGISTER.md` | `sha256:e749b370c66136c30841ea7ebf000f0617dda1e52d7cebd7d6f5754b510219ec` |
| `SECURITY_INCIDENT_LOG.md` | `sha256:0c8f201c70ba2361b3add0ad8cbd9f06cff93b794cd80242115ae5d3ada5a917` |
| `SECURITY_MODEL.md` | `sha256:d827c99a150a91f8601f20c3a272655c3c8348c2fbb5bef8ad26e076de57cac1` |
| `SERVER_SECURITY_CONTRACT.md` | `sha256:4c6488a33222662c4a2b2a830c3d3ddff087e1348338f41658e943b871f01a59` |
| `SERVER_TIMING.md` | `sha256:d28f2d80749e29093172f04bc562d1d170788b927f55e1f370d7ad480fadb8ed` |
| `SMS_NOTIFY_PLAN.md` | `sha256:8eca9d921c1278e572504202105915ffc79717d4acc79d843a3135900ce0532c` |
| `STORAGE_OPTIONS_2026-09-06.md` | `sha256:1f78f4fdcc759fb014a65831ace199a6e1909d50e0190af3de5983af7a2c128a` |
| `SUPERADMIN_GAPS.md` | `sha256:0f3da49a3cd88f39b5053179c5c932efafe10aab1f843cd4068d740b43362721` |
| `SUPERVISOR_BRIEF.md` | `sha256:5535818ea7580ae11e2681741574d008bb8614fa800ec1eca8411907bc57eacd` |
| `SYNC_FLOW.md` | `sha256:0d768e1fa6bcbdf0f69108408ba5733a787cbb881bf60c8b3b57cf5acc54db0a` |
| `SYNC_PROTOCOL.md` | `sha256:13ca6de184b934b4465522a7aee75b37cfead71be6a8e25703511f24423c1004` |
| `TEST_COVERAGE_REPORT.md` | `sha256:e7ee1ea60967663bd0f860fcf48afa014bc60a6403b62cc0e3a1744a6760e864` |
| `THREAT_MODEL.md` | `sha256:2d93966da760fec407778e27e9ef85033f95efffe71d1135dd849f6780d966f0` |
| `TRACING_SETUP.md` | `sha256:6d17e75b8cc28763db6d7d7c9310d587fb3419c15a2d1dbf2f9e938881040099` |
| `TROUBLESHOOTING.md` | `sha256:195898a386cd084aacc2ea3b553be0750eb9821a1508877e3497d6cd43d2bb72` |
| `VISITORS_MODULE.md` | `sha256:6882e68880cc068fbb32d0b0f7da4eaafa477b27535dbf869f560ffe98f2825e` |
| `VOCATIONAL_GRADES.md` | `sha256:ddc79151225d8b6cf605b97c4bec7d5a1bfc4800b71dedd13db3f97bf1b717bb` |
| `WAF_DDOS_SETUP.md` | `sha256:0aa30a600e0c07b15e6f2b39a347a921e043aca5240ab0321c88a80c559ed6d2` |
| `WAVE10_DB_SCALE.md` | `sha256:ba5ef9b7edeb38b361d6d51959b66d300a3cf25f8f77d1c588e48ff47d4f439f` |
| `WAVE11_CACHE_STRATEGY.md` | `sha256:9c0e8db61ea642001525a1f4c41b46830d8f3be2cd90ee88252a7fb4442fec99` |
| `WAVE12_NETWORK_EDGE.md` | `sha256:ab98a49fb41cfab607d2f77777bd13323439255ecb889871be5c63618cc9b103` |
| `WAVE13_ASVS_AUDIT.md` | `sha256:50de6c4eafbe96486c98643658549f41e85042af0ec7f043dfdfeb82fd7864cc` |
| `WAVE14_OBSERVABILITY.md` | `sha256:97357797c730215b6002999685c572c49afaefe3de544b19741dd1dcc0dc0b1f` |
| `WAVE18_LOAD_TEST_PLAN.md` | `sha256:a2df124b003624a1f5abd4c795d5f4bf78bdc54c0b2a7772402da04219d58960` |
| `WAVE19_CHAOS_PLAN.md` | `sha256:492fb30c52f5c666dd3ae5950c5cfa37e2d2bb17ef2f277c7ec428b439e04f71` |
| `WAVE1_READS_INVENTORY.md` | `sha256:6e29c2f59cce63eb1add329dd09d130b01f7216e9a205123a3eda766386218c3` |
| `WAVE1_WRITES_INVENTORY.md` | `sha256:c72b4b881c6ee051d0ca95d68e42fa18e079d1569bac67fa40a43e617cd67c9a` |
| `WAVE3_QUERY_PERFORMANCE.md` | `sha256:e2fa62cdb6aa3ea4c001f51a377640f4c194f9ffc06bff82a5e2dcf818f83d7b` |
| `WAVE5_AUTHZ.md` | `sha256:09adacd599dbdb74d9e72afcfa2de6faa23e0bd4f5ea91fbc1c2f07c4f727894` |
| `WAVE6_REDIS_AUDIT.md` | `sha256:24fea84c7281358277e9d4f88231aff26f7f1d8c214910b1f0c5e2e2f89868b3` |
| `WAVE7_OFFLINE_QUEUE.md` | `sha256:7b93c7f20769f0640912ee0e4f36ef9427ee1db5f431f5d36dd206bd601382c6` |
| `WAVE9_PERFORMANCE.md` | `sha256:69722d9f5a2b4525d0341929ac5904bf74832d6ccd13ae6123fa3271a9cc56eb` |
| `ZERO_TRUST_ARCHITECTURE.md` | `sha256:0df302eab2c29a9ccff1280058b87eeb7811106932e9004e4ec0867c1402ac5c` |
| `توضیح_کامل_برنامه.md` | `sha256:c7b8199918aba64f02114df01226f22464beee19f63db4b89ef80d4cfcad9d2a` |
| `راهنمای_برنامه_نویس.md` | `sha256:19ba18ccf6b3bdb4b9974249c9a2493f7f209850510e7a175d546e81835bf84b` |
| `راهنمای_سوپر_ادمین.md` | `sha256:a4cc813e1ea302aebf72bf5c298695b1afcfb2c411d60972112cf55c1867f17f` |
| `راهنمای_کاربر.md` | `sha256:e7061c3b64353b6b04c0a17551eb990df1805f27a861dbb17996c9d6e62b26be` |

---
_قفل توسط چت ۶ (مستندات و انتشار) در مأموریت ۳۸ بسته شد. راستی‌آزمایی: `node tests/docs-freeze-marker.js`._
