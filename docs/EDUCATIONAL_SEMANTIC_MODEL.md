...
### ۳.۱۴. داشبورد سلامت آموزشی مدرسه و مرکز اقدام روزانه (`School Educational Health Dashboard & Action Center`)
- **ماژول:** `server/analytics/school-health-dashboard.js`
- **اندپوینت REST:** مسیر `GET /api/v1/analytics/school-health`
- **مؤلفه‌های کلیدی:**
  1. **نیمرخ چندبعدی سلامت مدرسه:** تجمیع ۴ بعد اصلی:
     - بعد حضور و غیاب (نرخ خالص، غیبت مزمن، وقت‌شناسی).
     - بعد آموزشی و تحصیلی (میانگین نمرات، نرخ قبولی، نرخ افت).
     - بعد عملیات سنجش (پوشش آزمون، زمان‌بندی ثبت).
     - بعد کیفیت داده (DQS ترکیبی حضور و نمرات).
  2. **تضمین اصل عدم پنهان‌سازی (No-Masking Principle):** هیچ‌گاه معدل کل بالا نباید نقایص حاد (نظیر غیبت مزمن بالای ۲۰٪ یا افت تحصیلی متوالی) را مخفی کند؛ در صورت بروز هر پرچم بحرانی، شاخص سلامت بلافاصله به وضعیت نیازمند مداخله تنزل یافته و در گزارش تصریح می‌شود (`no_masking_applied: true`).
  3. **مرکز اقدام روزانه (Daily Command Center):** تولید وظایف اولویت‌بندی‌شده، شفاف و توضیح‌پذیر به همراه گام پیشنهادی بعدی (`recommended_action`) برای مدیر، ناظم و مشاوران مدرسه شامل:
     - اقدامات فوری غیبت و تماس با اولیا.
     - هشدارهای توالی غیبت غیرموجه (Early Warning).
     - تشکیل کلاس‌های جبرانی برای دروس با نرخ افت بالا.
     - رفع نقص‌های پوشش ارزشیابی و نمرات ثبت‌نشده.
  4. **تفکیک کیفی عملکرد:** نمایش جداگانه «قوت‌های پایدار» (`stable_strengths`)، «مسائل نوظهور» (`emerging_issues`) و «مسائل بحرانی» (`critical_issues`).
  5. **ایزولاسیون سازمانی و مستأجران:** کنترل دسترسی نقش‌محور با توابع `policy.schoolInOfficeScope` و `filterReadable` جهت ممانعت قاطع از نشت اطلاعات مدارس.
