...
### ۳.۱۲. پایه تحلیل‌های جامع سنجش و ارزشیابی (`Assessment Analytics Foundation`)
- **ماژول:** `server/analytics/assessment.js`
- **اندپوینت REST:** مسیر `GET /api/v1/grades/analytics`
- **مؤلفه‌های کلیدی:**
  1. **پوشش ارزشیابی (`calculateAssessmentCoverage`):** سنجش نسبت دانش‌آموزان ارزشیابی‌شده به کل دانش‌آموزان ثبت‌نامی (`total_eligible`)، کشف لیست دانش‌آموزان فاقد نمره (`missing_student_ids`)، و طبقه‌بندی سطح پوشش (`FULL`, `ADEQUATE`, `PARTIAL`, `LOW`).
  2. **تحلیل تأخیر و به‌موقع بودن (`calculateTimelinessMetrics`):** محاسبه فاصله زمانی برگزاری آزمون تا ثبت در سامانه بر حسب روز، و دسته‌بندی سوابق به `ON_TIME` ($\le ۳$ روز)، `DELAYED` (۴ تا ۷ روز) و `LATE` ($> ۷$ روز).
  3. **تشخیص نمرات پرت (`detectGradeOutliers`):** پیاده‌سازی متد آماری حصارهای توکی (Tukey's Fences) بر مبنای دامنه میان‌چارکی ($IQR = Q_3 - Q_1$)، با حصارهای پایین و بالای خفیف ($1.5 \times IQR$) و شدید ($3.0 \times IQR$) برای کشف انحرافات خاص کلاسی.
  4. **امتیاز کیفیت داده نمرات (`calculateAssessmentDataQuality`):** محاسبه شاخص چهاربعدی DQS شامل:
     - کامل بودن (Completeness - ۳۵٪)
     - اعتبار داده و ریاضی (Validity - ۳۵٪)
     - سازگاری پیوندها با ثبت‌نام و ساختار مدرسه (Consistency - ۱۵٪)
     - تازگی و سرعت ثبت (Freshness - ۱۵٪)
     - خروجی: نمره ترکیبی ۰ تا ۱۰۰ و تعیین سطح اطمینان (`HIGH`, `MEDIUM`, `LOW`).
  5. **گزارش تجمیعی (`buildAssessmentAnalyticsReport`):** ارکستراسیون یکپارچه توزیع نمرات، کیفیت روان‌سنجی (دشواری و تمایز)، پوشش، زمان‌بندی، داده‌های پرت و کیفیت داده در قالب یک شیء غیرقابل دستکاری و قابل ممیزی.
  6. **کوئری‌ساز هرس پارتیشن (`buildAssessmentAnalyticsQuery`):** کوئری‌ساز SQL پارامتری و ایمن برای جدول پارتیشن‌بندی‌شده `grades` با اعمال شروط بازه `created_at` و گارد Fail-Closed روی `school_id`.

### ۳.۱۳. پایه تحلیل‌های جامع حضور و غیاب (`Attendance Analytics Foundation`)
- **ماژول:** `server/analytics/attendance.js`
- **اندپوینت REST:** مسیر `GET /api/v1/attendance/analytics`
- **مؤلفه‌های کلیدی:**
  1. **شاخص وقت‌شناسی و تاخیرها (`calculatePunctualityMetrics`):** محاسبه تعداد جلسات با تأخیر، دقایق کل تأخیر، میانگین دیرکرد به ازای هر ورود با تاخیر، و نرخ خالص وقت‌شناسی.
  2. **کشف توالی‌های غیبت متوالی (`detectAbsenceStreaks`):** شناسایی فوری الگوهای ترک تحصیل زودهنگام با رصد غیبت‌های متوالی (آستانه پیش‌فرض ۳ جلسه) و صدور هشدارهای با شدت `WARNING` و `CRITICAL`.
  3. **الگوهای روزهای هفته (`calculateDayOfWeekPatterns`):** تجزیه و تحلیل توزیع زمانی غیبت در روزهای شنبه تا چهارشنبه، تعیین روز دارای بیشترین افت حضور (`highest_absence_day`) برای برنامه‌ریزی مداخله‌ای مدرسه.
  4. **امتیاز کیفیت داده حضور و غیاب (`calculateAttendanceDataQuality`):** ارزیابی ۴ ستون کیفیت داده (اعتبار وضعیت‌ها ۳۰٪، کامل بودن پوشش ۳۵٪، سازگاری ارجاعات ۲۰٪، و تازگی ثبت ۱۵٪) همراه با سطح اطمینان آماری (`HIGH`, `MEDIUM`, `LOW`).
  5. **گزارش تجمیعی (`buildAttendanceAnalyticsReport`):** اتصال یکپارچه نرخ حضور تقویمی و خالص، وضعیت غیبت مزمن، شاخص‌های وقت‌شناسی، هشدارهای توالی و کیفیت داده در یک گزارش غیرقابل تحریف.
  6. **کوئری‌ساز هرس پارتیشن (`buildAttendanceAnalyticsQuery`):** کوئری‌ساز SQL بر مبنای جدول پارتیشن‌شده `attendance` با محدودسازی بازه `created_at` و الزامی بودن `school_id`.
