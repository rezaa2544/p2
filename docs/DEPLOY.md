...
| `PAYESH_REVOKE_NEG_CACHE_MS` | `5000` | ابطالِ نشست (P4): عمرِ کِشِ «این توکن باطل نیست» — هرچه کمتر، ابطال زودتر دیده می‌شود و هرچه بیشتر، فشار روی Redis کمتر. `0` = بی‌کِش (هر درخواست یک پرسش) |
| `PAYESH_REVOKE_REQUIRE_REDIS` | `0` | ابطالِ نشست: با `1`، **قطع شدنِ** Redis هم به معنای ۴۰۱ است (امنیت بر در دسترس بودن). پیش‌فرض `0` یعنی قطعِ کوتاهِ Redis همه را بیرون نمی‌اندازد |
| `PAYESH_REVOKE_ALL_ON_LOGIN` | `0` | با `1`، هر ورود همهٔ نشست‌هایِ پیشینِ همان کاربر را باطل می‌کند (خروجِ اجباریِ دستگاه‌های دیگر) |
| `REDIS_URL` | `(خالی)` | با تنظیم شدن، ابطالِ نشست **توزیع‌شده** می‌شود (الزامی برای استقرارِ چندنمونه‌ای — مستندِ معماری §۲.۱) |
