# 🔒 قفل مستندات — DOCS_FREEZE_v1.0.0-rc19 (تاریخی — جایگزین‌شده)

> **⚠️ جایگزین شد:** این قفل با `docs/DOCS_FREEZE_v1.0.0-rc20.md` بامپ شد (2026-09-11، پس از مرجع پیکربندی). این سند صرفاً تاریخچهٔ فهرست ۲۵۲ سند ریشه در قفل نوزدهم است.

**نسخهٔ قفل:** ۱.۰.۰-rc19 | **تاریخ قفل:** ۲۰۲۶-۰۹-۱۱ | **مالک:** چت ۶ (مستندات و انتشار)
**وضعیت:** این سند نقطهٔ انجماد رسمی کتابخانهٔ مستندات است و جایگزین
`DOCS_FREEZE_v1.0.0-rc1.md` تا `rc18` (تاریخی) می‌شود. فهرست زیر **همهٔ ۲۵۲ سند ریشهٔ**
`docs/*.md` را در لحظهٔ قفل با هش دقیق ثبت می‌کند؛ خود این سند عمداً بیرون فهرست
است (خودارجاعی ممکن نیست) و شمارهٔ ریشه با آن به ۲۵۱ می‌رسد.

**دلیل بامپ:** بستن شکاف «اسپک اوپن‌ای‌پی‌آی وجود ندارد» —
`docs/openapi.yaml` (OpenAPI 3.0.3 — ۲۶ مسیر / ۳۱ عملیات منطبق با کد،
شامل دو مسیر کشف‌شده هنگام تدوین: `/api/health-index` و `/metrics`)،
`docs/API_REFERENCE.md` (مرجع انسانی)، سه ابزار (اعتبارسنجی/لینت/دریفت)،
دو تست (۱۰۳ + ۹ چک) و به‌روزرسانی چنج‌لاگ و چک‌لیست پنتست — مأموریت ۳۶.
قفل قبلی (`rc18`) به فهرست اسناد عادی پیوست.
...
1. از این لحظه، **هر تغییر در هر سندِ فهرست‌شده** نیازمند **بامپ نسخه** است
   (rc18 ← rc19؛ نسخهٔ بعد با همین ساختار و فهرست هش تازه ساخته می‌شود و این
...
- همین سند با **کامیت گیت** روی شاخهٔ نشست ثبت می‌شود (دست‌نخورده‌بودن با تاریخچهٔ کامیت
  و باندل تحویل `/home/user/openapi-spec.bundle` قابل اثبات است)؛
...
## ۳) فهرست اسناد ریشه با هش (شمار: ۲۵۲)
...
| `DOCS_HEALTH_REPORT.md` | `sha256:4c5d0bfb58f6c49cbbe9e9c9794deac86f20c2d85410eab213d000f3916bff65` |
| `DOCS_INDEX.md` | `sha256:29aa0f90b6518521e10d4c6cce0807a5b637f587f4b6d1a985df2dd36b0a1f7f` |
| `DOCS_METRICS.md` | `sha256:b4d607bbd32d3802980cbdbab4588d78e1523fa128cfa84e6614e6b7aa552e2a` |
...
| `DOCUMENTATION_MAP.md` | `sha256:36982f4aa6d615a1347b8025e1e3cff6d4ef53dfea2a37d3ab9983b72b5d630f` |
...
| `TEST_COVERAGE_REPORT.md` | `sha256:4ee5e55f2daf59333e8c8e5f90029c0fba3d3e7134c6d911afda059968e2de73` |
...
---
_قفل توسط چت ۶ (مستندات و انتشار) در مأموریت ۳۶ بسته شد. راستی‌آزمایی: `node tests/docs-freeze-marker.js`._
