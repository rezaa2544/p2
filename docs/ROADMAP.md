...
>
> **این سند همچنان مرجع «الزامات مهندسی، ناوردا‌ها، Anti-patternها و Production
> Readiness Gate» است، اما دیگر مرجع «برنامهٔ زمانی و ترتیب اجرا» نیست.**
>
> مرجع جدید ترتیب و زمان‌بندی کل پروژه از فاز ۱ تا آخرین قابلیت پیش‌بینی‌شده:
>
> ### 📋 [`docs/ROADMAP_MASTER_EXECUTION_SCHEDULE.md`](./ROADMAP_MASTER_EXECUTION_SCHEDULE.md)
>
> آن سند شامل: Historical Execution Timeline (بازسازی‌شده از Git، فاز ۱ → ۸.۱)،
> Current State، Findings Register (R1–R25 + یافته‌های جدید)، Future Phase
> Schedule (Phase 8.2 → Phase 15)، Sprint Plan (Seq 01 → 67، ۱۴۰ Sprint)،
...
> پیش‌نیاز و با چه نوع موازی‌سازی** اجرا می‌شوند (بخش‌های ۷، ۷.۱ و ۸ آن سند).
>
> **سه تصحیح مهم که در آن سند ثبت شد و بر خواندن سند حاضر اثر می‌گذارد:**
> 1. ادعاهای «۱۰۰٪ کامل» فاز ۵ (گام‌های ۰۸–۱۲) و فاز ۶ (cutover ملی) توسط
>    حکم Red Team زیروتراست (`PHASE6_FINAL_ZERO_TRUST_RED_TEAM_VERDICT.md`
>    @ کامیت `046dafd4`) **رد شدند**؛ وضعیت واقعی‌شان در سند جدید ثبت است.
> 2. یافتهٔ جدید **F-EI-01**: هشت موتور آموزشی در `server/analytics/` هیچ
>    ارجاع رانتایمی ندارند → گیت اجباری **Phase 9.0 Wiring Gate**.
> 3. آیتم‌های Wave 21 دیگر معلق نیستند؛ سه مورد (W21-05/06/07) بر پایهٔ
>    وابستگی واقعی به فاز ۸.۲/۸.۴ **جلو کشیده شدند**.
>
> واژگان `100% complete` / `Production Ready` / `National Ready` /
> `Fully Verified` / `GO` بدون Evidence در برنامه‌ریزی ممنوع‌اند.

> این سند مکمل الزامات معماری ملی است و اجرای Waves بدون رعایت آن مجاز نیست.
**تاریخ:** ۱۸ شهریور ۱۴۰۵ (۲۰۲۶-۰۹-۰۹) · **آخرین هم‌راستاسازی:** ۲۰۲۶-۰۹-۲۰
...
- **برنامهٔ اجرایی مرجع (Historical Timeline + Sprint Plan + Gates + Dependency Graph):
  `docs/ROADMAP_MASTER_EXECUTION_SCHEDULE.md`** ← ترتیب و زمان‌بندی از اینجا خوانده می‌شود
