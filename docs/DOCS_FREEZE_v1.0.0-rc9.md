# 🔒 قفل مستندات — DOCS_FREEZE_v1.0.0-rc9 (تاریخی — جایگزین‌شده)

> **⚠️ جایگزین شد:** این قفل با `docs/DOCS_FREEZE_v1.0.0-rc10.md` بامپ شد (2026-09-11، پس از واژه‌نامهٔ جامع). این سند صرفاً تاریخچهٔ فهرست ۲۳۱ سند ریشه در قفل نهم است.

**نسخهٔ قفل:** ۱.۰.۰-rc9 | **تاریخ قفل:** ۲۰۲۶-۰۹-۱۱ | **مالک:** چت ۶ (مستندات و انتشار)
**وضعیت:** این سند نقطهٔ انجماد رسمی کتابخانهٔ مستندات است و جایگزین
`DOCS_FREEZE_v1.0.0-rc1.md` تا `rc8` (تاریخی) می‌شود. فهرست زیر **همهٔ ۲۳۱ سند ریشهٔ**
`docs/*.md` را در لحظهٔ قفل با هش دقیق ثبت می‌کند؛ خود این سند عمداً بیرون فهرست
است (خودارجاعی ممکن نیست) و شمارهٔ ریشه با آن به ۲۳۲ می‌رسد.

**دلیل بامپ:** تدوین پنج راهنمای نقش‌محور کاربران نهایی در `docs/user-guides/`
(مأموریت ۲۶) + ارجاع آن‌ها در نمایه، برنامهٔ پایلوت و بستهٔ تحویل؛ قفل قبلی
(`rc8`) به فهرست اسناد عادی پیوست.

**زیرپوشه‌های بیرون مانیفست ریشه** (۱۶ سند؛ پوشش ساختاری‌شان را تست‌های پوشش
می‌پایند و فهرست قطعی‌شان در `README` هر پوشه و در نمایه ثبت شده است):
...
1. از این لحظه، **هر تغییر در هر سندِ فهرست‌شده** نیازمند **بامپ نسخه** است
   (rc9 ← rc10؛ نسخهٔ بعد با همین ساختار و فهرست هش تازه ساخته می‌شود و این
...
- همین سند با **کامیت گیت** روی شاخهٔ نشست ثبت می‌شود (دست‌نخورده‌بودن با تاریخچهٔ کامیت
  و باندل تحویل `/home/user/user-guides.bundle` قابل اثبات است)؛
...
## ۳) فهرست اسناد ریشه با هش (شمار: ۲۳۱)
...
| `DOCS_HEALTH_REPORT.md` | `sha256:3fd2bcf7e364a2b1acd20cfc6906c6b516cd1f316c8072a8b9197f4c637ba21f` |
| `DOCS_INDEX.md` | `sha256:f08b371c916dba56aa84dd4003e6d5224a6b2d289ce9cd401dfc5ea9469d1185` |
| `DOCS_METRICS.md` | `sha256:8e66f439336db8aa72539881ddb4176a3e1535238939c0136cbea6b65f8998a5` |
...
---
_قفل توسط چت ۶ (مستندات و انتشار) در مأموریت ۲۶ بسته شد. راستی‌آزمایی: `node tests/docs-freeze-marker.js`._
