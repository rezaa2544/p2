...
| `node build.js --check` | ✅ |
| `tests/docs-consistency.js` · `security-findings-register-coverage.js` | ۲۲/۲۲ ✅ · ۵۱/۵۱ ✅ |
...
### پیوستِ پس از مرج — بازآدیت روی `main@e52b50b` (۲۰۲۶-۰۹-۱۱)

در جریانِ همین نشست دو PRِ بازِ دیگر هم به `main` مرج شدند، پس هر یافته باید روی
درختِ **تازه** دوباره سنجیده می‌شد. نتیجه (سنجش، نه فرض):

| یافته | روی `main@e52b50b` | چه شد |
|---|---|---|
| S9-4 مودالِ تودرتو | رفع نشده | #77 مرج شد ⇒ شاخه روی `main` rebase و PR #80 به `main` نشانه رفت |
| S9-5 موجیبِکِ `HANDOFF.md` | **مستقل رفع شد** | مرجِ #78 دفترچه را بازنویسی کرد؛ سنجش: ۰ خطِ آلوده. گاردِ `tests/handoff-integrity.js` (H1–H4) افزوده شد تا تکرار نشود |
| S9-7 TDZ در `--skip-live` | **مستقل رفع شد** | حالا سالم تمام می‌شود (۹ ایستا + ۵ NOT-RUN · exit 2) |
| S9-6 bootstrapِ کهنه | **باقی مانده** | فهرستِ hardcode هنوز `004` را دو بار دارد و `005`..`007` را بی‌صدا skip می‌کند؛ رفع در PR #81 روی شاخهٔ rebase‌شده به `main` |

- **قفل:** مانیفستِ `rc29` پس از این مرج‌ها **بازتولید** شد (۲۹۸ سندِ ریشه، شاملِ
  `WAVE19_WAL_DRILL_REPORT.md`ِ تازه)؛ وگرنه همان لحظهٔ مرج کهنه می‌شد. روی `main`
  پیش از این کار نشانگر **۱۱/۱۴** بود (این عدد، جدا از ۱۳/۱۴ پیشین، پس از مرجِ #۷۸ است).
- **قلمِ بازِ خودِ مانورِ Wave 19 بسته شد:** `docs/WAVE19_WAL_DRILL_REPORT.md` §۴
  بند ۱ گفته بود آلارمِ پرشدگیِ WAL غایب است و باید **بیرونی** باشد؛
  `docs/OBSERVABILITY.md` اکنون `WALDiskHigh` (۷۰٪) و `WALDiskCritical` (۸۵٪) را با
  عبارتِ نمونه، جایگزینِ بدونِ node_exporter و قاعدهٔ «اول فضا، بعد restart» مستند
  می‌کند. این آلارم عمداً در `infra/observability/alert-rules.yml` نیست تا قفلِ
  سه‌جانبهٔ متریک‌های خودِ برنامه دست‌نخورده بماند (`observability-config` ۵۹/۵۹ ✅).
- **ساختارِ دفترچه:** مرجِ #78 ورودیِ خود را **بالای** سرتیترِ `HANDOFF.md` جا داده
  بود؛ سرتیتر به خطِ اول برگشت و گاردِ H4 همین را pin می‌کند.
