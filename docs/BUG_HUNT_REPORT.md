...
| `tests/docs-freeze-marker.js` | **۱۴/۱۴ ✅** — روی `main` یک خطای پیش‌موجود داشت (سه سندِ ناهم‌خوان از مرجِ PR #77) و ویرایشِ اسنادِ همین نشست هم طبقِ §۱ بند ۱ قاعدهٔ قفل نیازمندِ بامپ بود؛ هر دو با **بامپِ `rc29`** و همگام‌سازیِ مانیفست با دیسک رفع شد (پیش از بامپ: ۱۳/۱۴) |
...
- باز: PR #74 (نشست ۷) هنوز open است؛ #76 (واریانتِ دیگرِ مانورِ WAL) بررسی نشد؛ PR #80 و #81 منتظرِ پذیرشِ نویسندگان‌اند.
- **قفلِ مستندات:** ویرایشِ دو سندِ یخ‌زده (`BUG_HUNT_REPORT.md` و `SECURITY_FINDINGS_REGISTER.md`) طبقِ §۱ بند ۱ نیازمندِ بامپ بود؛ `DOCS_FREEZE_v1.0.0-rc29.md` ساخته شد (۲۹۷ ردیفِ هش، همگام با دیسک)، `rc28` بنرِ تاریخی گرفت و `tests/docs-freeze-marker.js` به قفلِ تازه اشاره می‌کند. سه سندِ ناهم‌خوانِ باقی‌مانده از مرجِ PR #77 هم در همان بامپ با دیسک هم‌خوان شد.
- **PR #77 در میانهٔ نشست به `main` مرج شد** (merge commit `2c7097d`): کدِ a11y حالا روی `main` است و یافتهٔ S9-4 هم روی `main` وجود دارد؛ شاخهٔ `bug-hunt-session9` روی `main` تازه rebase شد و رفعِ S9-4 در PR #80 به `main` نشانه می‌رود.
