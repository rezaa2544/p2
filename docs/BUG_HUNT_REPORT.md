...
# گزارش باگ‌هانت — نشست ۷ (چت ۵: مهندس ادغام و رفع خطا)

> ادامهٔ مستقیمِ نشست‌های ۱–۴ همین سند. پایه: `main` روی `b872f44` (مرج PR #63: red-team + supply-chain).
> شاخهٔ این نشست: `bug-hunt-session7` — **۷ کامیت** (هر رفع = یک کامیت؛ هیچ تستی حذف/تضعیف نشد).

## ۱. گیت‌های اولیه روی درخت تمیز (پیش از هر تغییر)

- `tests/smoke.js` **۵۴۷/۵۴۷** · `tools/check-authz.js` **exit 0** · `tests/secret-scan.js` **۱۱/۱۱** · `build.js --check` ✅ · `tests/api/runner.js` **۷/۷ سوئیت**.

## ۲. رگرسیون کامل (`scripts/run-all-tests.sh`)

- اجرای پایه روی `b872f44`: **۳۸۰ سوئیت — ۳۴۸ سبز / ۳۲ سرخ** (فهرست سرخ‌ها: `/tmp/red-list-session7.txt`).
- ⚠ حادثهٔ ابزارِ این نشست: ویرایشِ میانیِ خودِ `scripts/run-all-tests.sh` (رفعِ S7-2) در حینِ اجرا — bash فایلِ در حالِ اجرا را با آفستِ بایتی می‌خواند، پس لغزشِ آفست اجرا را چند سوئیت پیش از پایان قطع کرد (خطای نحوی در میانهٔ فایل؛ خودِ اسکریپت سالم است و `bash -n` تأیید می‌کند). `api/runner` تنها سوئیتی بود که اجرا نشد و جداگانه **۷/۷** سبز شد. درس ثبت شد: **هیچ ویرایشی روی اسکریپتِ در حالِ اجرا**.
- رگرسیونِ نهاییِ همین نشست روی درختِ رفع‌شده دوباره اجرا شد (بخش ۷).

## ۳. جدول باگ‌ها (۶ رفع + ۱ پاک‌سازی)

| # | باگ | شدت | ریشه | شاهدِ قرمز | رفع | کامیت |
|---|---|---|---|---|---|---|
| **S7-1** | **DoS بدونِ احراز هویت با request-target بدشکل** (`GET //[ HTTP/1.1`) | **بحرانی** | `new URL(req.url, 'http://localhost')` بدونِ گارد در نخستین دستورِ `onRequest` (پیش از WAF/CSRF)؛ `wrappedRequest` هم بدونِ catch ⇒ `ERR_INVALID_URL` → unhandledRejection → `exit 1` | بازتولیدِ زندهٔ مرگِ سرور روی پورت واقعی با سوکتِ خام (پیش از رفع)؛ سوئیتِ تازه `tests/route-url-guard.js` **۰/۵ قرمز** | گاردِ تجزیه ⇒ `400 bad_request`؛ catchِ ایمن در wrapper ⇒ `500 internal_error` (هیچ‌گاه مرگِ فرآیند) | `542f4fd` |
| S7-2 | REDِ کاذب: `wave15-child` به‌عنوان سوئیتِ مستقل | متوسط (گیت) | دونده فقط `server11-child.js` را کنار می‌گذاشت؛ اسکریپتِ کارگر با `<root> <mode>` صدا زده می‌شود | `ERR_INVALID_ARG_TYPE` در `tests/wave15-child.js:23` | هر دو فهرست، کلِ خانوادهٔ `*-child.js` را کنار می‌گذارند (۲۸۲ سوئیتِ مستقل) | `2481409` |
| S7-3 | پینِ کهنهٔ زنجیرهٔ مهاجرت‌ها (۵ فایل/۴ شماره در برابرِ ۶ فایل/۵ شماره) | متوسط (گیتِ اسناد) | دو گیت روی وضعِ پیش از `005_delta_sync_updated_at_indexes` مانده بودند؛ تکرارِ ۰۰۴ قلمِ بازِ به‌ارث‌رسیدهٔ main است | `docs-consistency` و `db-engineering` سرخ | پین‌ها به زنجیرهٔ واقعی به‌روز شد؛ پینِ «تکرارِ ۰۰۴» **حفظ** شد؛ ابزار **۴۷/۱ → ۴۸/۰** | `6f4705d` |
| S7-4 | اسکنرِ authz فراخوانی‌های رجیستری متریک را «نوشتنِ داده» می‌خواند | کم (گیت) | هم‌نامیِ `add` در `metrics.createRegistry()` با APIِ داده | `authz-model` سرخ روی `wave14-observability.js:54` (`r.add('t_depth', …)`) | تشخیصِ alias از خودِ فایل و عبور؛ پوششِ فراخوانی‌های بی‌واسطه **حفظ** شد (probeِ منفی تأیید کرد) | `a521c59` |
| **S7-5** | **گم‌شدنِ بی‌صدای صفِ کهنهٔ همگام‌سازی در مهاجرت به IndexedDB** | **بالا (ازدست‌رفتنِ داده)** | `Store.get(a) ‖ Store.get(b) ‖ Store.get(c)` — «اولی-برنده»: با وجودِ کلیدِ جاریِ برنامه، صفِ کهنه هرگز خوانده نمی‌شد | `idb-persistence` تستِ ۱۴ «صف قدیمی به IndexedDB مهاجرت نکرده است» — **۱۵/۱۶** | هر سه کلید ادغام می‌شوند؛ `addToQueue` روی `uid` یکتا می‌کند ⇒ بدونِ تکرار | `41e7436` |
| S7-6 | رگرسیونِ کهنهٔ `wave3-keyset` (الگوی پیش از چت ۲) | کم (تست) | SQLِ فعلی `AND ((date < $2) OR (date = $3 AND id > $4))`؛ الگو پرانتزِ بیرونی را نداشت و فیکِ DB هم predicate را اعمال نمی‌کرد | A1/A4 سرخ — ۱۱/۱۳ | الگو به شکلِ واقعی و **سخت‌گیرانه‌تر** (بایندِ هر سه جایگاه)؛ فیکِ DB همان predicateِ PG را اجرا می‌کند | `3be0fcb` |
| — | پاک‌سازی: اسنپ‌شاتِ `DOCS_HEALTH_REPORT` (۲۷۸ → ۲۸۷ سند) | — | خروجیِ ابزار کهنه مانده بود | — | کامیتِ جدا | `0899df9` |

## ۴. سرخ‌های طبقه‌بندی‌شده (رفع نشد — نیازمندِ تصمیمِ مالک)

| رده | سوئیت‌ها | تشخیص |
|---|---|---|
| پین‌های کهنهٔ اسناد | `docs-freeze-marker` (ردیف ۲۷۷ در برابر ۲۸۶ + هشِ ۴ سند)، `docs-metrics` (۲۷۷ در برابر ۲۸۷)، `docs-metadata` (سه سندِ یتیم)، `documentation-map-coverage` (۲۰/۲۳)، `test-coverage-report-coverage` (۳۱/۳۳) | ادعاهای عددیِ اسناد از دیسک عقب مانده‌اند؛ هش‌های سندِ قفل بدونِ تصمیمِ مالکِ main به‌روز نمی‌شوند |
| قاعدهٔ ۳۰٪ حضور | `att2/att3/att4` (+ سه سوئیتِ جهش) | تست انتظارِ «رکورد غایب» دارد، کد «present» می‌سازد؛ تصمیمِ سیاست (کد یا تست) با مالکِ Wave |
| دامنهٔ REST↔sync | `wave5-authz` T18/T20/T21 (+ جهش‌ها) | تفاوتِ معناییِ دامنهٔ دبیر/دانش‌آموز بین REST و مدل |
| هارنسِ جهش | `server-mutations` M2، `wave8-outbox` M3، `dropout` M6، `libserial2` M2 (الگو پیدا نشد)؛ `server14-gc` M4، `libserial2` M3، `sync-atomic-batch` ۴/۵، `vocational-grades` ۳/۵، `server15` ۵/۶، `urgent-ann` ۵/۶، `wave17-testing` ۹/۱۰، `staff-gap` | بخشِ بزرگ = الگوی جهش با کدِ بازآرایی‌شده هم‌خوان نیست (تست/هارنس)؛ چند زنده‌ماندهٔ واقعی (شکافِ پوشش) |
| محیطی | `arena5-recovery` (FATAL: fetch failed)، `config-audit` (۵/۷) | وابسته به شبکهٔ بیرونی/متغیرهای محیطیِ سندباکس |

## ۵. راستی‌آزمایی پس از رفع (تک‌تک، محیطِ آرام — بدونِ اجرای موازی)

- S7-1: `route-url-guard` **۹/۹** (پیش از رفع: ۰/۵) · S7-5: `idb-persistence` **۱۶/۱۶** · S7-6: `wave3-keyset` **۱۴/۱۴** · S7-4: `authz-model` **۲۵۲/۲۵۲** · S7-3: ابزار **۴۸/۰**، `docs-consistency` **۲۲/۲۲**، `db-engineering` **۱۳/۱۳**.
- گیت‌ها روی درختِ رفع‌شده: دودی **۵۴۷/۵۴۷** · مجوز **exit 0** · نشت‌یاب **۱۱/۱۱** · `build.js --check` ✅ (باندلِ مرورگر هم بازسازی شد: `index.html`/`USER_GUIDE.html`).

## ۶. باندل و رجیستری

- باندل تازه از رأسِ همین گزارش ساخته شد: `/home/user/bandle/chat5-session7.bundle` (+ `chat5-session7.patch`، `chat5-worktree-backup.tar.gz`، `chat5-sha256.txt`).
- رأسِ باندل و sha256 در `docs/BUNDLE_REGISTRY.md` §۲ ثبت شده است (ردیفِ رجیستری یک کامیت بعد از رأسِ باندل است — باندل نمی‌تواند هشِ خودش را درونِ خود داشته باشد).

## ۷. رگرسیونِ پایانی و پوش

- رگرسیونِ کاملِ پس از رفع‌ها (۲۴۰۶ ثانیه · **۳۸۰ سوئیت**): **۳۵۲ سبز / ۲۸ سرخ**. هر شش رفعِ این نشست در همین اجرا سبز شدند — از جمله `route-url-guard` که تازه به فهرست اضافه شده و ۶ ثانیه‌ای سبز است.
- سه سرخِ تازهٔ همین اجرا بررسی شدند: `waf-mutations` (۴/۴ ایستاده سبز) و `wave18-load` (۶۲/۶۲ ایستاده سبز) = فلیکِ بارِ موازی؛ و `wave20-arena5` که **پینِ کهنهٔ SCR-3** بود («فقط `server11-child.js`») و با **S7-7** به قاعدهٔ `*-child.js` به‌روز شد ⇒ **۲۲/۲۲**.
- ۲۸ سرخِ باقی‌مانده دقیقاً همان رده‌های بخش ۴ است (پین‌های کهنهٔ اسناد، قاعدهٔ ۳۰٪، دامنهٔ REST↔sync، هارنسِ جهش، محیطی) و هیچ‌کدام رگرسیونِ این نشست نیستند.
- پوش: شاخهٔ `bug-hunt-session7` با توکنِ درون‌خطی به `origin` رفت و `git ls-remote` رأس را تأیید کرد (هشِ رأسِ نهایی در `git log` همین شاخه / پیامِ تحویل ثبت است).

---

## ۸. ادامهٔ نشست ۷ — سه یافتهٔ تازه (S7-8 · S7-9 · بستنِ شکافِ پوششِ کرسر)

> همان شاخه و همان قاعده: یک رفع = یک کامیت · تست‌اول‌قرمز · بدونِ تضعیفِ تست.

### S7-8 — حذف/بازگردانیِ قلمِ صف، آینهٔ Background Sync را هم‌گام نمی‌کرد (بالا)

- **ریشه:** `sync-del` قلم را از `SYNC.queue`/`SYNC.dlq` می‌ریخت و `save` می‌کرد، اما هیچ مسیری به `bgMirrorQueue()` نداشت — و برخلافِ `sync-retry` هیچ چرخه‌ای هم برنامه‌ریزی نمی‌کرد. شاخهٔ حذفِ خودِ `bgMirrorQueue` («قلم‌هایی که دیگر در صفِ زنده نیستند … از آینه پاک شوند») دقیقاً همین را می‌خواست؛ فقط صدا زده نمی‌شد. `sync-retry` هم فقط به چرخهٔ زمان‌بندی‌شده تکیه داشت (که بی‌شبکه شکست می‌خورد).
- **اثر:** قلمِ حذف‌شده با `status=pending/failed` در آینه می‌ماند و رویدادِ بعدیِ sync (تبِ بسته) آن را می‌فرستد ⇒ **کاربر چیزی را که صریحاً حذف کرده بود، سرور اعمال می‌کرد**.
- **رفع:** `bgMirrorQueue()` پس از `save` در هر دو اکشن (ایدمپوتنت و دِبونس‌شده).
- **شاهد:** سوئیتِ تازهٔ `tests/sync-del-mirror.js` (jsdom + IDB جعلی) — پیش از رفع **۴/۷** (D3 آینه قلمِ حذف‌شده را نگه می‌داشت، D6 مسیرِ retry هرگز به آینه نمی‌رسید) ⇒ پس از رفع **۷/۷**. همسایه‌ها: `bgsync` ۱۴/۱۴ · `sync-dlq-retry` ۷/۷ · `sync-queue-caps` ۳۶/۳۶ · `sync-atomic-batch` ۲۲/۲۲ · جهش‌های `deadletter`/`sync-chunk`/`sync-queue-caps` همه کشته.
- **کامیت:** `10bb224`

### S7-9 — توکنِ زندهٔ گیت‌هاب در فایلِ track‌شده + کوریِ گیتِ نشت‌یاب (بحرانی)

- **یافته:** `setup-deps.ps1` (اسکریپتِ کمکیِ ویندوز) در خطِ ۱ یک **PAT زندهٔ گیت‌هاب** داشت و روی `origin/main` در کامیتِ `0654313` کامیت شده بود.
- **چرا کسی ندید:** `tests/secret-scan.js` فقط یک فهرستِ بستهٔ پسوند را راه می‌رفت (`.js/.json/.html/.md/.env/.sh/.yml/.yaml`) — `.ps1` بیرون از دامنه بود؛ گیت ۱۱/۱۱ سبز گزارش می‌داد.
- **رفع (دو کامیت، تست‌اول):** (الف) دامنهٔ اسکن به «هر فایلِ متنی» گسترد (۱۱۳۹ فایل در برابر ۸۸۹؛ فقط باینریِ شناخته‌شده و فایل‌های >۲MB کنار) + دو خودسنجیِ دامنه تا عقب‌نشینی بی‌صدا ناممکن شود + allowlistِ باریکِ `.env.example` برای «نمونهٔ» راز — اجرا **قرمز** شد و خودِ توکن را گرفت؛ (ب) توکن از فایل حذف شد و اسکریپت از `$env:GH_TOKEN` می‌خواند (نبود ⇒ خطای صریح). ⇒ **۱۳/۱۳ سبز**.
- **باقی‌مانده با مالکِ مخزن (عمداً یک‌طرفه انجام نشد):** توکن در تاریخچه (`0654313`) می‌ماند ⇒ تصمیمِ چرخش/ابطال + بازنویسیِ تاریخچه با مالک است. T1/T2 در فایل‌های track‌شده و تاریخچه **نیستند** (بررسی شد).
- **کامیت‌ها:** `2e7ac0d` (گیت) · `b3d4dcb` (رفع) — هش‌های نهایی در `git log`.

### پوششِ کرسر (PR #60) — شکافِ تست بسته شد

- `server/cursor.js` بدونِ سوئیتِ اختصاصی بود. سوئیتِ تازهٔ `tests/cursor-token.js` قرارداد را با **۳۲ سنجه** پین می‌کند: fail-closed بی‌کلید، قالبِ `pc1.<b64>.<b64>`، roundtrip، دست‌کاریِ payload/امضا، مرزِ انقضا، «منقضیِ جعلی ⇒ invalid نه expired»، سقفِ skew، ورودی‌های بدشکل/بزرگ، کلمپِ TTL + env، جداسازیِ دامنه (کلیدِ HMAC = `sha256('payesh.cursor.v1|secret')` و هرگز خودِ راز)، ردِ رازِ کوتاه، ردِ نسخهٔ ناشناخته، هم‌ارزیِ کلید بینِ منابع (چند-نمونه‌ای) و سیم‌کشیِ `pull.js` (سه کدِ شکست + `cursor_renewal:'full_pull'` + `next_cursor`/`cursor_ttl_s`).
- **باگی یافت نشد** (رفتار مطابقِ مستند است) — خروجی: بستنِ شکافِ پوششِ یک ماژولِ امنیتی. ۳۲/۳۲. کامیت: `3cfeb5d`.

### وضعیتِ گیت‌ها پس از ادامهٔ نشست

دود **۵۴۷/۵۴۷** · مجوز **۰** · نشت‌یاب **۱۳/۱۳** (دامنهٔ گسترده) · `build --check` ✅ · `route-url-guard` **۹/۹** · `idb-persistence` **۱۶/۱۶** · `sync-del-mirror` **۷/۷** · `cursor-token` **۳۲/۳۲** · `wave3-keyset` **۱۴/۱۴**.
