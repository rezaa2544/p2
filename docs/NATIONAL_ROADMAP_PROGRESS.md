...
| 21 | پشتیبانی از مدارس روستایی و چندپایه | چت ۳ | ✅ | Medium | مدلِ موجودِ has_multigrade/c.multigrade + لایهٔ پیش‌نویسِ حضور (دور ۴۲/۷۷) | شاخهٔ `feat/wave21-multigrade`: نمای ماتریسی `multigrade` (سطر=دانش‌آموز، ستون=پایه) برای حضور (حاضر/غایب/مرخصی + ثبتِ گروهیِ هر پایه با یک ضربه + فیلتر پایه + وضعیت فعلی) و نمرهٔ گروهی؛ Offline-First حفظ شد (پیش‌نویسِ Store + att-review→att-commit؛ نمره از insert→صف)؛ `src/js/76-multigrade.js` + `docs/MULTI_GRADE_GUIDE.md`؛ `tests/multi-grade.js` ‏34/34 + mutation ×۳؛ گیت‌ها: smoke ‏547/547، check-authz تطبیق کامل، secret-scan ‏11/11، a11y-keyboard ‏92/92، build --check سبز |
