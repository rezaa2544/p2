...
| 10 | Database Scale | Arena 1 + Arena 4 + چت ۳ | 🟡 | High | Wave 1/2/3 و benchmark | `server/db.js`: pool رپلیکایِ خواندنی (`READ_DATABASE_URL`) + `queryRead()` + reprobeِ خودکارِ S3-1؛ `tests/wave10-db-scale.js` ‏26/26، `wave10-query-audit` ‏14/14، `db-replica-recovery` ‏11/11؛ **قیدِ «replica/پارتیشن هرگز رویِ PG زنده» بسته شد (2026-09-12 چت ۳)**: گیتِ `tests/wave10-pg-live.js` ‏19/19 رویِ PostgreSQL 18 واقعی با streaming replica واقعی (P1..P4 رپلیکیشن، D1..D4 مسیریابی/fallback/بازگشتِ خودکار، T1..T8 DDL §۳.۲ پارتیشن‌بندی + مهاجرت از heap + pruning) + جهش ‏5/5 (`wave10-pg-live-mutations.js`)؛ سند: `docs/WAVE10_DB_SCALE.md`. باقی: PgBouncer (لایهٔ استقرار) و اعمالِ تولیدیِ پارتیشن‌بندی |
