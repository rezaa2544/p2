...
| 18 | National Load Testing | Arena 5 + Arena 4 | 🟡 در حال اجرا | High | Capacity Model (✅ `docs/CAPACITY_MODEL.md` — چت ۶)، Wave 3/10/14 | **نقطهٔ شکست یافت شد: OOM در ۶۰ rps** — اجرای استیجینگ (۲ هسته/۱۹۸۴ مگابایت، مولد+API+PG روی همان ۲ هسته) پنج سناریوی load/peak/stress/spike/soak را با k6 v2.2.0 انجام داد؛ peak سرور را از کار انداخت (`http_req_failed=19.07%`، `p95=147,345ms`، `dropped_iterations=2346`، `vus_max=1099`) و OOM Killer پروسهٔ Node را کشت (`anon-rss≈911MB`)؛ `write_errors=0%` در هر پنج سناریو. **ظرفیت ملی (۲۰٬۰۰۰ rps) روی این سخت‌افزار قابل سنجش نیست** — اعداد اعتبارسنجیِ هارنس‌اند، نه ظرفیت. دو «سبز کاذب» در هارنس رفع شد (`SCENARIO=peak` هیچ سناریویی نمی‌ساخت؛ `spike` با `maxVUs<preAllocatedVUs` اجرا نمی‌شد). گزارش کامل: [`docs/WAVE18_LOAD_TEST_REPORT.md`](WAVE18_LOAD_TEST_REPORT.md). گیت‌ها: `wave18-load-test.js` 38/38 · `secret-scan.js` 12/12 · `check-authz.js` 6/6 · `build.js --check` 394 اکشن · `smoke.js` 547/547. پیش‌تر: **پی‌آر #۴۶ مرج شد (2026-09-10 @ cd484c3)** — مولد دیتاست ملی + `tests/wave18-load-test.js` روی `main`. ⚠️ بازمانده: سنجش رسمی ظرفیت روی استیجینگ واقعی؛ رفع واگرایی `enrollments` در خودِ مولد؛ واگرایی ابعاد مولد با §۲.۱ (قلم ۲ `docs/DOCS_CONSISTENCY_REPORT.md`) |
