...
ROOT=pathlib.Path('/home/user/p2'); OUT=pathlib.Path('/home/user/arena8-dr/evidence'); OUT.mkdir(exist_ok=True)
...
def pg_suite(n):
 ...
 try:
  ...
  sql('commit-before-target',"INSERT INTO users VALUES(11,'keep')")
  sql('before-target',"SELECT pg_create_restore_point('arena_target')")
  ...
  run(f'p{n}-target-original-start',['pg_ctl','-D',target,'-o',f'-c config_file={rd}/etc/postgresql.conf','-l',rd/'original.log','-w','start'],env)
  for lf in rd.rglob('*.log'):(OUT/f'p{n}-original-{lf.name}').write_text(lf.read_text())
  # Correct only disposable restored config for independent engine control; repository unchanged.
  with (target/'postgresql.auto.conf').open('a') as f:f.write(f"\nrestore_command = 'pgbackrest --pg1-path={target} --stanza=payesh archive-get %f %p'\n")
  run(f'p{n}-target-start',['pg_ctl','-D',target,'-o',f'-c config_file={rd}/etc/postgresql.conf -c recovery_target_action=promote -c max_connections=100','-l',rd/'target.log','-w','start'],env,True);started.append(target)
  ...
  # Independent physical standby: same-host real replication/promote, not E4.
  standby=base/'standby';ssock=base/'standby-socket';ssock.mkdir()
  run(f'p{n}-basebackup',['pg_basebackup','-h',sock,'-p',port,'-U','user','-D',standby,'-R','-X','stream','-c','fast'],env,True)
  run(f'p{n}-standby-start',['pg_ctl','-D',standby,'-o',f'-p {port+4} -k {ssock} -c archive_mode=off','-l',base/'standby.log','-w','start'],env,True);started.append(standby)
  senv={'PGHOST':str(ssock),'PGPORT':str(port+4),'OLD_PGHOST':str(sock),'OLD_PGPORT':str(port)}
  sql('standby-before',"SELECT pg_is_in_recovery(),count(*),md5(string_agg(users::text,'|' ORDER BY id)) FROM users",senv)
  stop(data);started.remove(data)
  run(f'p{n}-promote',['bash','tools/failover-postgres.sh'],env|senv,True)
  sql('standby-after',"SELECT pg_is_in_recovery(),count(*),md5(string_agg(users::text,'|' ORDER BY id)) FROM users",senv)
  ...
  if backup_files and n < 9:
   shutil.make_archive(str(OUT/f'p{n}-clean-repository'),'gztar',repo)
   damaged=max(backup_files,key=lambda p:p.stat().st_size);prior=hashlib.sha256(damaged.read_bytes()).hexdigest();damaged.write_bytes(b'CORRUPT-BACKUP')
   ...
   run(f'p{n}-verify-corrupt',['pgbackrest','--stanza=payesh','verify'],env)
  ...
  for lf in base.rglob('*.log'):
   if 'repo' not in lf.parts:(OUT/f'p{n}-final-{lf.parent.name}-{lf.name}').write_text(lf.read_text())
...
try:
 ...
 for n in [9,10]:
  for name,fn in [('pg',pg_suite)]:
   try:fn(n)
   except Exception as e:(OUT/f'{name}{n}-exception.txt').write_text(traceback.format_exc());print(name,n,'ERROR',str(e),flush=True)
finally:
 (OUT/'ledger-committed-control.json').write_text(json.dumps(ledger,indent=2));print('LAB_COMPLETE',flush=True)
