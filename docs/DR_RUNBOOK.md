...
| 2026-09-13 | Queue-Outage (§۶ سناریوهای صف) — دریل `tests/queue-outage-drill.js` روی outbox/worker واقعی + PG ‏17.10 | **~43ms** (replay پس از بهبود) | **۰** | سبز **26/26** (۲ اجرای پایدار): قطعی→بافر(۱۰)→replay با exactly-once؛ slow-consumer: lag=20، جدا بودنِ تولید (append p95≈13ms)؛ dedupe دو لایه ۱۰۰٪؛ backlog ‏۱۲۰→drain ‏86ms. یافتهٔ ثبت‌شده: ترتیبِ بین-retry تضمین نمی‌شود (قراردادِ کارگر؛ هندلرها idempotent). شاهد: `docs/daily-reports/2026-09-13.md` §چت۴-دور۴ | چت ۴ |
