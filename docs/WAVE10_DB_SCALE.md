...
- **تست‌های این موج:**
  ...
  - `tests/partitioning.js` → **۵۵/۵۵** (نوبتِ چهارم: U1–U7 واحد + L1–L9 زنده بر PG 17.11؛ نوبتِ سوم ۴۲)
...
- **Phase C (دو تراکنش — یافتهٔ استیجینگ §۹):** تراکنشِ بلندِ کپی هیچ قفلِ
  انحصاری نمی‌گیرد؛ پنجرهٔ کوتاهِ swap = `ANALYZE *_p` + `LOCK … ACCESS
  EXCLUSIVE` + **کچ‌آپِ ضدالحاقِ مبتنی بر chg_id** (سطرهای نوشته‌شده حینِ
  کپی — درج و به‌روزرسانی — آپسرت می‌شوند؛ شرطِ تازگی `p.chg_id >= o.chg_id`
  یعنی last-writer-wins) + رقصِ rename. سپس **فاز D (دو پاسِ idempotent)**:
  نویسندهٔ بلاک‌شدهٔ لحظهٔ swap که بعد از کامیت روی OID قدیمی (`*_old`)
  می‌نشیند، ادغام می‌شود. نام‌های ایندکس/سکوئنس که `002/005/007/008`
  ساخته‌اند عیناً حفظ می‌شوند؛ `*_old` نگه داشته می‌شود (rollback بی‌درز).
...
### ۷.۳ تأییدِ زنده (`tests/partitioning.js` — ۵۵/۵۵؛ نوبتِ سوم ۴۲ → نوبتِ چهارم ۵۵)
...
`*_old` بازنویسیِ معکوس نمی‌شود (شرطِ تازگی — رگرسیونِ L9e)** · وارون‌سازیِ
کامل + بازیابیِ سطرِ پس از swap در `grades_recovered`.
...
**تأییدِ استیجینگِ ۱.۸M سطری (§۹):** کپیِ 1.8M (1.2M+600k) با ترافیکِ
هم‌زمان ۵۷–۶۵s ⇒ نرخِ ~۳۱k سطر/s — هم‌راستا با برون‌یابیِ بالا.
...
### ۹.۲ سه یافتهٔ مهاجرت که فقط اجرای استیجینگ آشکار کرد (هر سه فیکس شد)
...
- **فشار WAL روی نوشتن‌ها حینِ کپی** (یافتهٔ عملکردی، محدود به دورهٔ کپی):
  کامیتِ نویسنده‌ها پشتِ فشارِ WAL صف می‌شود و بدترین حالت تا ~۴۷s طول
  می‌کشد — خطا رخ نداد و همهٔ نوشته‌ها رسیدند (verify همه صفر) ولی برایِ
  تولید توصیه می‌شود کپی در بازهٔ کم‌ترافیک اجرا شود یا باِ throttle. خواندن‌ها
  عملاً دست‌نخورده ماندند.
...
3. **۰۰9 را اجرا کنید** (`tools/migrate-helper.js`) — تک‌فایل، دو تراکنش،
   ANALYZE و فاز D داخلِ خودش.
