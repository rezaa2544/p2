...
| 2 | **رگرسیون SG11 + server15 C15c/C16/C17c** | ‏`resolve⇒del` فوری در `05c3136` (باگ ۲ بازبین #124) قرارداد Gap-3 ‏(#59: دلتا ردیف resolved را از updated_at می‌بیند) و 409 already_resolved را می‌شکست | **PR #143**: resolve⇒keep + هرس سقف‌دار جداگانهٔ resolvedها (`PAYESH_RESOLVED_CONFLICTS_MAX`، پیش‌فرض ۵۰۰)؛ جهش ۲/۲ کشته | باز — منتظر مرج |
| 3 | **رگرسیون wave1-writes W4c** | برش post-commit آینه (‏`430e1b3`، ‏#124) نوتیف مشتق را عمداً از store می‌بُرد؛ قرارداد تست کهنه + شکاف پوشش memory-mode | **PR #144**: ‏W4c به قرارداد جدید + تست W4m جدید (memory-mode)؛ جهش ۲/۲ کشته | باز — منتظر مرج |
| 4 | rot سوئیت جهش server15-mutations ‏(M1/M6 «الگو پیدا نشد» = جهش اعمال‌نشده) | بازآرایی کد در #124 و delta-hardening | **PR #150** (استک روی #143): ری‌تارگت M1/M6؛ ۶/۶ کشته | باز — منتظر مرج |
...
## توصیه‌ها
1. مرج زنجیرهٔ #143 → #150 (استک) و #144 — تنها رگرسیون‌های واقعی main را می‌بندند.
