...
**??:** AZ partition detection → promote standby → readiness 200 → data integrity check.
...
**??:** Fill WAL disk → PG PANIC → restart → crash recovery → PITR → readiness=200 → verify data integrity → measure RTO.
