...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc44`) | **۵۴۲** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۴۷۴ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `ARCHITECTURE/` + `GOVERNANCE/` + `audit/` + `roadmaps/`) | ۶۸ |
...
| ممیزی فاز ۷ و ارزیابی دروازه انتشار | «`ARCHITECTURE/PHASE_7_RELEASE_GATE_AND_READINESS_AUDIT.md`» · «`RELEASE_GATE_CHECKLIST.md`» · «`RELEASE_GATE_EVIDENCE.md`» |
