...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc33`) | **۳۳۷** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۱۲ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-reports/`) | ۲۵ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۲۸۱ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۳۳ |
