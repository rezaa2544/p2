...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc36`) | **۳۵۱** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۲۰ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-reports/`) | ۳۱ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۲۹۲ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۳۶ |
