...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc45`) | **۴۲۸** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۶۱ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-audits/` + `daily-reports/` + `roadmaps/`) | ۶۷ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۳۶۰ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۴۵ |
