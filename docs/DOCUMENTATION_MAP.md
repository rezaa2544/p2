...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc41`) | **۳۸۳** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۲۹ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-reports/`) | ۵۴ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۳۱۹ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۴۱ |
