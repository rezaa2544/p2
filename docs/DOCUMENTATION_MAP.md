...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc26`) | **۲۸۶** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۲۶۷ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/`) | ۱۹ |
| توکن در نمایهٔ جستجو | ۲۲۲۸ |

**به تفکیک وضعیت:**

| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۲۳۴ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۲۹ |
