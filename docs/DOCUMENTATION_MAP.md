...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc43`) | **۳۹۴** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۳۷ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-reports/`) | ۵۷ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۳۲۸ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۴۳ |
