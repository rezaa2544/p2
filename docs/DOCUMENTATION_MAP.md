...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc40`) | **۳۶۹** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۲۳ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-reports/`) | ۴۶ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۳۰۶ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۴۰ |
