...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc39`) | **۳۵۶** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۲۲ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-reports/`) | ۳۴ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۲۹۴ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۳۹ |
