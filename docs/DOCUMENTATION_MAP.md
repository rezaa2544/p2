...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc34`) | **۳۴۳** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۱۴ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-reports/`) | ۲۹ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۲۸۶ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۳۴ |
