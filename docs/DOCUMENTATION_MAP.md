...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc44`) | **۴۲۵** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۵۹ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/` + `daily-audits/` + `daily-reports/` + `roadmaps/`) | ۶۶ |
