...
| سنجه | مقدار |
|---|---:|
| کل اسناد (لحظهٔ قفل `rc31`) | **۳۲۶** |
| اسناد ریشهٔ `docs/*.md` (با خود نقشه و قفل) | ۳۰۶ |
| اسناد زیرپوشه‌ها (`RUNBOOK_CARDS/` + `user-guides/` + `pilot/`) | ۲۰ |
...
| وضعیت | شمار |
|---|---:|
| فعال (در حال استفاده) | ۲۷۲ |
| زنده (ماشینی/رخدادی، مستثنی از بامپ) | ۳ |
| منجمد (قفل‌های تاریخی) | ۳۱ |
