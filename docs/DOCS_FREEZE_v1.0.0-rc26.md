# 🔒 قفل مستندات — DOCS_FREEZE_v1.0.0-rc26

> ## ⚪ سند تاریخی — جایگزین‌شده با `DOCS_FREEZE_v1.0.0-rc27.md` (۲۰۲۶-۰۹-۱۱)
>
> این قفل در لحظهٔ بامپ `rc27` به فهرست اسناد **تاریخی/منجمد** پیوست و دیگر نقطهٔ
> انجماد فعال کتابخانه نیست. قفل فعال: `DOCS_FREEZE_v1.0.0-rc27.md`.
>
> **دلیل بامپ به `rc27`:** مرج `origin/main` در شاخهٔ `feat/chat6-recovery` ده سند ریشهٔ
> تازه از main وارد `docs/` کرد (`ASSETS_MODULE`، `LIBRARY_MODULE`، `OPENCODE_USER_GUIDE`،
> `POST_MERGE_AUDIT_2026-09-10`، `RUNBOOK_DISASTER_RECOVERY`، `WAVE16_DISASTER_RECOVERY`،
> `WAVE17_TESTING`، `WAVE17_TESTING_PYRAMID`، `WAVE18_LOAD_TESTING`، `WAVE5_RED_ANALYSIS`)
> و دو سند فهرست‌شده را تغییر داد (`WAVE14_OBSERVABILITY`، `WAVE3_QUERY_PERFORMANCE`).
> طبق §۱ بند ۱ همین سند، هر دو مورد نیازمند بامپ نسخه‌اند؛ بنابراین مانیفست زیر
> **دیگر با دیسک یکی نیست و عمداً دست‌نخورده نگه داشته شده** تا وضعیت لحظهٔ `rc26`
> قابل اثبات بماند. راستی‌آزمایی زندهٔ کنونی: `node tests/docs-freeze-marker.js` (روی `rc27`).
>
> شمار مانیفست این سند ۲۶۶ ردیف است؛ شمار دیسک در لحظهٔ بامپ ۲۷۷ ردیف شد.

**نسخهٔ قفل:** ۱.۰.۰-rc26 | **تاریخ قفل:** ۲۰۲۶-۰۹-۱۱ | **مالک:** چت ۶ (مستندات و انتشار)
**وضعیت:** ⚪ **تاریخی** — در لحظهٔ انتشار، این سند نقطهٔ انجماد رسمی کتابخانهٔ مستندات بود و جایگزین
`DOCS_FREEZE_v1.0.0-rc1.md` تا `rc25` (تاریخی) می‌شود. فهرست زیر **همهٔ ۲۶۶ سند ریشهٔ**
`docs/*.md` را در لحظهٔ قفل با هش دقیق ثبت می‌کند؛ خود این سند عمداً بیرون فهرست
است (خودارجاعی ممکن نیست) و شمارهٔ ریشه با آن به ۲۶۷ می‌رسد.

**دلیل بامپ:** مأموریت نجات محتوا پس از دو ریست پیاپی سندباکس —
`docs/RECOVERY_INSTRUCTIONS.md` (پنج بخش: محتوای باندل جامع، ورود آن در نشست جدید،
ری‌بیس اگر مِین جلو رفته باشد، راستی‌آزمایی پس از مرج و بازیابی از پشتیبان فشرده)
+ تست پوشش آن + ردیف باندل جامع در رجیستری — مأموریت ۴۳.
این قفل، قفلِ همراه باندل جامع `chat6-master-recovery.bundle` است.
قفل قبلی (`rc25`) به فهرست اسناد عادی پیوست.
...
| زیرپوشه | شمار | محتوا |
|---|---|---|
| `RUNBOOK_CARDS/` | ۱۱ | ۱۰ کارت A4 ران‌بورد (RC-001 تا RC-010) + فهرست/درخت تصمیم |
| `user-guides/` | ۵ | راهنمای نقش‌محور: مدیر، دبیر، ولی، دانش‌آموز، کارکنان |
| `pilot/` | ۳ | اسناد عملیاتی پایلوت: معرفی مدرسه، فرم رضایت والد، راهنما |
...
1. از این لحظه، **هر تغییر در هر سندِ فهرست‌شده** نیازمند **بامپ نسخه** است
   (rc26 ← rc27؛ نسخهٔ بعد با همین ساختار و فهرست هش تازه ساخته می‌شود و این
...
- همین سند با **کامیت گیت** روی شاخهٔ نشست ثبت می‌شود (دست‌نخورده‌بودن با تاریخچهٔ کامیت
  و باندل تحویل `/home/user/chat6-master-recovery.bundle` قابل اثبات است)؛
...
## ۳) فهرست اسناد ریشه با هش (شمار: ۲۶۶)
...
| `DATA_DICTIONARY.md` | `sha256:ffa198708a28618eca312e870f39507a7a9764da9d186fd72af98c30838580aa` |
...
| `DOCS_CONSISTENCY_REPORT.md` | `sha256:34f8315f37b496e3d6029c964d39ca4506f8c8417fc36bdfa9dde4277da1fe91` |
...
| `DOCS_HEALTH_REPORT.md` | `sha256:58c8ce675f3898d3e6d41c7edf33f4f02d8549d7d7b55eac3346cd445da4ef58` |
| `DOCS_INDEX.md` | `sha256:0aebdcec0f03dd5ad5a49cac0c9402ec140ccca9eb8c82722713cce2c631eb01` |
| `DOCS_METRICS.md` | `sha256:b50c6cf73dea3f00ce31984285a8dcf19e6a3b89a92cbe1c0e96d0327a065c77` |
...
| `DOCUMENTATION_MAP.md` | `sha256:61101642f562ce84c4f13d36056b951a9c4eddd43386f103cbe2f1eebf082822` |
...
| `RECOVERY_INSTRUCTIONS.md` | `sha256:9e1c9baabdec9332153f9a767fa743474b74a639e6900c468b5c5c922c87d3dc` |
...
| `TEST_COVERAGE_REPORT.md` | `sha256:3910ffb1a8d16c9cdbb2580c4e9716152a2a6be6bb84ec67ece9b7505e9a318a` |
...
---
_قفل توسط چت ۶ (مستندات و انتشار) در مأموریت ۴۳ بسته شد. راستی‌آزمایی: `node tests/docs-freeze-marker.js`._
