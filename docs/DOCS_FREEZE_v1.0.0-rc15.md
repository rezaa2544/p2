# 🔒 قفل مستندات — DOCS_FREEZE_v1.0.0-rc15 (تاریخی — جایگزین‌شده)

> **⚠️ جایگزین شد:** این قفل با `docs/DOCS_FREEZE_v1.0.0-rc16.md` بامپ شد (2026-09-11، پس از بستهٔ خلاصهٔ مدیریتی). این سند صرفاً تاریخچهٔ فهرست ۲۴۴ سند ریشه در قفل پانزدهم است.

**نسخهٔ قفل:** ۱.۰.۰-rc15 | **تاریخ قفل:** ۲۰۲۶-۰۹-۱۱ | **مالک:** چت ۶ (مستندات و انتشار)
**وضعیت:** این سند نقطهٔ انجماد رسمی کتابخانهٔ مستندات است و جایگزین
`DOCS_FREEZE_v1.0.0-rc1.md` تا `rc14` (تاریخی) می‌شود. فهرست زیر **همهٔ ۲۴۴ سند ریشهٔ**
`docs/*.md` را در لحظهٔ قفل با هش دقیق ثبت می‌کند؛ خود این سند عمداً بیرون فهرست
است (خودارجاعی ممکن نیست) و شمارهٔ ریشه با آن به ۲۴۵ می‌رسد.

**دلیل بامپ:** تدوین `docs/DOCUMENTATION_MAP.md` (نقشه و کاتالوگ جستجوی کتابخانه —
هشت بخش: نمای کلی، حرارتی، کلیدواژه، نقش/موج/مأموریت، یتیم‌ها و قطب‌ها) + ابزار
`tools/docs-metadata.js` (متادیتا + نمایهٔ جستجو) + رفع پنج سند یتیم و افزودن
ارجاع اسناد پایلوت به طرح پایلوت — مأموریت ۳۲. قفل قبلی (`rc14`) به فهرست
اسناد عادی پیوست.

**زیرپوشه‌های بیرون مانیفست ریشه** (۱۹ سند؛ پوشش ساختاری‌شان را تست‌های پوشش
می‌پایند و فهرست قطعی‌شان در نمایه ثبت شده است). از این قفل، زیرپوشهٔ `pilot/`
که در قفل‌های پیشین از شمارش جا مانده بود نیز رسماً ثبت می‌شود:
...
1. از این لحظه، **هر تغییر در هر سندِ فهرست‌شده** نیازمند **بامپ نسخه** است
   (rc15 ← rc16؛ نسخهٔ بعد با همین ساختار و فهرست هش تازه ساخته می‌شود و این
...
- همین سند با **کامیت گیت** روی شاخهٔ نشست ثبت می‌شود (دست‌نخورده‌بودن با تاریخچهٔ کامیت
  و باندل تحویل `/home/user/docs-map.bundle` قابل اثبات است)؛
...
## ۳) فهرست اسناد ریشه با هش (شمار: ۲۴۴)
...
| `DOCS_HEALTH_REPORT.md` | `sha256:8243b56bd0fe482e6b2739833001f052763a2492473e132d6218565b462aed33` |
| `DOCS_INDEX.md` | `sha256:cbb5e6f160487a02e3dc23f3eb827dfc74a02f2b1509838f34c66994d20f6564` |
| `DOCS_METRICS.md` | `sha256:e1f67a518136a3ad7ce1f9a363e095a48f3e11acf0308f528333d0bc53912599` |
...
| `DOCUMENTATION_MAP.md` | `sha256:3e622fec35e6a023f3afdb893d38913db7dee43f211a15e1e24835a2a3c1ab62` |
...
| `TEST_COVERAGE_REPORT.md` | `sha256:e103deaea4a6799254c3b20b3fca3238cd6e0ade13457836834f5377790e4cb8` |
...
---
_قفل توسط چت ۶ (مستندات و انتشار) در مأموریت ۳۲ بسته شد. راستی‌آزمایی: `node tests/docs-freeze-marker.js`._
