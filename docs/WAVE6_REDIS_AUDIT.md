...
- در این ساندباکس **ردیسِ زنده‌ای نیست** — همهٔ چیزها با clientِ جعلیِ
  سازگار با قرارداد اثبات شده و رفتارِ «ردیسِ قطع» (P0-13) با
  `REDIS_URL`ِ نالایق تست شده است. اتصالِ واقعی به یک `redis-server`
  (مثلاً در CI با service) هنوز **pending** است و در گزارشِ این دور
  ثبت شده؛ منطقِ درایور (ioredis 6) از قبل در `redis-fallback.js` (10/10)
  و `session-revocation.js` (16/16، بخشِ redis-server با skip) پوشش دارد.
