...
sc(){ # sc <sentinel-addr> <args...>
  ...
  REDISCLI_AUTH="${REDIS_PASSWORD:-}" redis-cli -h "$h" -p "$p" "$@"
}
...
for s in "${LIST[@]}"; do
  if sc "$s" ping >/dev/null 2>&1; then SENT="$s"; break; fi
done
...
if [ "$FORCE" != 1 ]; then
  MH="${CUR%%,*}"; MP="${CUR##*,}"
  if REDISCLI_AUTH="${REDIS_PASSWORD:-}" redis-cli -h "$MH" -p "$MP" ping 2>/dev/null | grep -q PONG; then
...
sc "$SENT" SENTINEL FAILOVER "$MASTER_NAME"
...
NH="${NEW%%:*}"; NP="${NEW##*:}"
REDISCLI_AUTH="${REDIS_PASSWORD:-}" redis-cli -h "$NH" -p "$NP" INFO replication | grep -q 'role:master' \
