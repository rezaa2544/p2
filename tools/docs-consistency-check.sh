...
# زنجیرهٔ واقعی روی دیسک (S7-3، باگ‌هانت نشست ۷ — پینِ کهنه به‌روز شد):
#   001_initial, 002_indexes, 003_constraints, 004_wave1_version_seq,
#   004_wave3_query_indexes (تکراری — از main), 005_delta_sync_updated_at_indexes
#   ⇒ شش فایل فوروارد با پنج شمارهٔ متمایز (۰۰۱–۰۰۵) و یک تکرارِ عمدیِ ۰۰۴.
# تصادمِ شمارهٔ ۰۰۴ یک قلمِ بازِ به‌ارث‌رسیده از main است (سیاستِ شماره‌گذاریِ پیوستهٔ
# docs/MIGRATION_GUIDE.md نقض شده) — همچنان به‌عنوان یک واقعیتِ سنجیده پین می‌شود
# تا پنهان نماند؛ تصمیمِ شماره‌گذاریِ مجدد با مالکِ main است (renumber عمداً انجام نشد:
# اسناد main به نامِ فایل ارجاع دارند — HANDOFF.md).
[ "$MIG" -eq 6 ]; chk "شمار مهاجرت‌های روی دیسک: ۶ فایل (شماره‌های ۰۰۱–۰۰۵ + تکرارِ ۰۰۴)" $? "یافت‌شده: $MIG"
DUP004=$(ls migrations/004_*.sql 2>/dev/null | grep -v '\.down\.sql$' | wc -l)
[ "$DUP004" -eq 2 ]; chk "تصادمِ شمارهٔ ۰۰۴ (دو فایل) پین شده — قلمِ بازِ به‌ارث‌رسیده از main" $? "یافت‌شده: $DUP004"
has $DOCS/DOCS_INDEX.md "۰۰۱–۰۰۵"; chk "نمایه: فهرست مهاجرت ۰۰۱–۰۰۵ (+ تکرارِ ۰۰۴)" $? "در برابر دیسک"
has $DOCS/RELEASE_NOTES.md "مهاجرت‌های نسخه‌دار ۰۰۱–۰۰۵"; chk "یادداشت انتشار: مهاجرت‌های ۰۰۱–۰۰۵" $? "در برابر دیسک"
