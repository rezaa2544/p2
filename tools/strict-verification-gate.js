...
function walk(dir,out){out=out||[];for(const e of fs.readdirSync(dir,{withFileTypes:true})){if(['node_modules','.git','dist'].includes(e.name))continue;const p=path.join(dir,e.name);if(e.isDirectory())walk(p,out);else if(/\.(js|mjs|cjs|yml|yaml)$/.test(e.name)&&path.relative(ROOT,p)!=='tools/strict-verification-gate.js')out.push(p);}return out;}
const amap=new Map();
