...
const pats=[[['assert','true'].join('('),/assert\s*\(\s*true\b/gi],[['assert','constant'].join('_'),/assert\s*\.(?:ok|equal|strictEqual|deepEqual)\s*\(\s*(?:true|false)\s*(?:[,)]|$)/gi],[['chk','constant'].join('_'),/\bchk\s*\([^)]*,\s*(?:true|false)\s*\)/gi],[['process','exit','0'].join('.'),/process\.exit\(\s*0\s*\)/g],[['|','|','true'].join(' '),/\|\|\s*true\b/g],[['0','0','checks'].join('/'),/0\s*\/\s*0\s*(?:checks?|tests?)/gi],[['not-run','exit-0'].join('_'),/not[-_ ]?run.{0,120}process\.exit\(\s*0\s*\)/gis]];
...
if(items){
 for(const item of items){
  ...
  chk('item '+id+' evidence schema',Array.isArray(ev)&&ev.length>0&&ev.every(e=>e&&typeof e==='object'&&sha(e.head_sha)&&typeof e.requirement==='string'&&e.requirement.trim()&&typeof e.command==='string'&&e.command.trim()&&Number.isInteger(e.exit_code)&&typeof e.artifact==='string'&&e.artifact.trim()&&typeof e.artifact_sha256==='string'&&/^[0-9a-f]{64}$/.test(e.artifact_sha256)&&typeof e.runtime==='boolean'&&typeof e.scope==='string'&&e.scope.trim()&&typeof e.provenance==='string'&&e.provenance.trim()&&typeof e.timestamp==='string'&&e.timestamp.trim()));
...
chk('G16 zero-ranking cannot be compliant',certified===0 || (items||[]).filter(x=>x.status==='CERTIFIED').every(x=>x.certification&&Number.isInteger(x.certification.ranking_count)&&x.certification.ranking_count>0));
