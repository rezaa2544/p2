...
# T2/T6: clean PG has no identity rows by design; seed isolated verifier identities
# into the PostgreSQL authority used by auth/session checks.
...
SET role=EXCLUDED.role, full_name=EXCLUDED.full_name, username=EXCLUDED.username,
    national_id=EXCLUDED.national_id, phone=EXCLUDED.phone, active=true,
    school_id=EXCLUDED.school_id, status='active';
SQL
...
for f in $(ls "$ROOT/migrations"/[0-9][0-9][0-9]_*.sql | grep -v '\.down\.sqlPORTC=3513
