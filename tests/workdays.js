...
test('W2 — isWorkDay: پنجشنبه برای مدرسهٔ ۵ روزه خاموش، برای SH-101 روشن', () => {
  ...
  /* BUG-6 (باگ‌هانت چت ۵): نسخهٔ قبلی daysAgoISO(0) را «شنبه (امروز)»
     فرض می‌کرد و پنجشنبه/جمعه قرمز می‌شد (وابستگی به تاریخِ اجرا)؛
     حالا تاریخِ ثابت مثلِ بقیهٔ سابت + پوششِ جمعه. */
  const sat = W(`isWorkDay(4, '2026-09-05')`);
  assert(sat === true, 'شنبه باید برای مدرسهٔ ۵ روزه روز کاری باشد');
  const fri = W(`isWorkDay(4, '2026-09-11')`);
  assert(fri === false, 'جمعه برای مدرسهٔ ۵ روزه نباید روز کاری باشد');
});
