#!/usr/bin/env node
/* ═══════════════════════════════════════════════════════════════════
   tests/api/observability-health.test.js — Observability API Tests (P1-SC-03)
   ═══════════════════════════════════════════════════════════════════ */
'use strict';
...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), 'payesh-api-obs-'));
...
async function run() {
  ...
  console.log('\n🔍 Testing /api/v1/system/observability-health Endpoint');
  ...
  try {
    ...
    const anonRes = await req('GET', '/api/v1/system/observability-health?school_id=1');
    assert.strictEqual(anonRes.status, 401, 'Anonymous request must return 401');
    console.log('  ✅ OBS1: Anonymous request rejected with 401');
    pass++;

    // ۲. درخواست بدون پارامتر الزامی
    const noParamRes = await req('GET', '/api/v1/system/observability-health', { cookie: managerCookie });
    assert.strictEqual(noParamRes.status, 400, 'Missing school_id or region_id must return 400');
    console.log('  ✅ OBS2: Missing params rejected with 400');
    pass++;

    // ۳. دسترسی مدیر مدرسه به تابلوی سلامت رصدپذیری مدرسه خود
    const mgrOwnRes = await req('GET', '/api/v1/system/observability-health?school_id=1', { cookie: managerCookie });
    assert.strictEqual(mgrOwnRes.status, 200, 'Manager accessing own school observability health must return 200');
    ...
    console.log('  ✅ OBS3: Manager gets 200 with Phase 4 observability health snapshot');
    pass++;

    // ۴. مدیر مدرسه به مدرسه دیگر (Anti-IDOR)
    if (foreignManagerCookie) {
      const foreignRes = await req('GET', '/api/v1/system/observability-health?school_id=1', { cookie: foreignManagerCookie });
      assert.strictEqual(foreignRes.status, 403, 'Foreign manager must be rejected with 403');
      console.log('  ✅ OBS4: Foreign manager rejected with 403 (Tenant Isolation)');
      pass++;
    } else {
      console.log('  ⏭️ OBS4: Foreign manager skipped (no second manager in fixture)');
    }

    // ۵. نقش غیرمجاز (دانش‌آموز)
    const stuRes = await req('GET', '/api/v1/system/observability-health?school_id=1', { cookie: studentCookie });
    assert.strictEqual(stuRes.status, 403, 'Student role must be rejected with 403');
    console.log('  ✅ OBS5: Unauthorized student role rejected with 403');
    pass++;

    // ۶. نمای منطقه‌ای رصدپذیری بدون رتبه‌بندی مدارس
    const superAdmin = store.users.find(u => u.role === 'superadmin' || u.role === 'admin') || manager;
    const adminCookie = await loginAs(superAdmin);
    const regRes = await req('GET', '/api/v1/system/observability-health?region_id=1', { cookie: adminCookie });
    ...
    console.error('❌ Observability Health API Test Error:', err);
    fail++;
  } finally {
    server.close();
  }

  console.log(`\nObservability Health API Tests: ${pass} passed, ${fail} failed`);
