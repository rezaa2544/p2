...
pass('boundary-t5-excludes-authority-migrations', () => {
  ...
  assert.ok(s.includes("grep -v") && s.includes(".down.sql") && s.includes("| sort); do"), 'T5 migration loop must exclude down migrations and terminate');
});
...
pass('negative-empty-pg-auth-fixture', () => {
  ...
  assert.doesNotMatch(s, /P7V_STORE_PATH/);
});
...
console.log('Phase 7 verifier contract: 6/6 PASS');
 | sort); do"), 'T5 migration loop must exclude down migrations and terminate');
});
