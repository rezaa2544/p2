#!/usr/bin/env node
/* Mutation checks for non-blocking async audit initialization and rotation. */
'use strict';
...
const kit = session('s8aio-mut-');
const FILE = path.join(ROOT, 'server/audit.js');
const original = fs.readFileSync(FILE, 'utf8');
const mutants = [
  {
    name: 'async initialization performs sync work',
    from: 'if (asyncMode) return;',
    to: 'if (false) return;'
  },
  {
    name: 'async flush skips rotation decision',
    from: 'try { await checkRotationAsync(); } catch (e) {}',
    to: 'try { await Promise.resolve(null); } catch (e) {}'
  }
];

function run(e) {
  try {
    return { code: 0, output: execFileSync(process.execPath, ['tests/session8-audit-async-io.js'], {
...
let killed = 0;
console.log('\n▸ Session 8 audit-async-io mutation tests');
try {
...
const restored = run();
const restoredGreen = restored.code === 0 && /session8-audit-async-io: 5\/5 checks/.test(restored.output);
console.log(`\nsession8-audit-async-io mutations: ${killed}/${mutants.length} killed` + (restoredGreen ? ' — restored green ✅' : ' — restore check FAILED'));
