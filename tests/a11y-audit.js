...
const calendar = js('22-jalali-calendar.js');
...
// ── A: skip link و landmarkها (۶) ──────────────────────────────
group('A — skip link و landmarkها');
chk('A1 پوسته skip link به #main دارد', /<a[^>]*class="skip-link"[^>]*data-act="skip-to-main"[^>]*>/.test(shell));
chk('A2 صفحه ورود skip link و مقصد #main هر دو دارد', /<a[^>]*class="skip-link"[^>]*data-act="skip-to-main"[^>]*href="#main"|<a[^>]*href="#main"[^>]*class="skip-link"[^>]*data-act="skip-to-main"/.test(login) && /<main[^>]*id="main"[^>]*class="login-wrap"/.test(login));
chk('A3 متنِ skip link خالی نیست', /<a[^>]*class="skip-link"[^>]*>[^<]{2,}<\/a>/.test(shell) && /پرش به محتوای اصلی/.test(login));
chk('A4 پوسته <main id="main"> رندر می‌کند', /<main[^>]*id="main"/.test(shell));
chk('A5 سایدبار (aside) برچسبِ ناوبری دارد', /<aside[^>]*aria-label="[^"]+"/.test(shell));
chk('A6 استایلِ skip-link (نمایان با فوکوس) هست', /\.skip-link/.test(css));
...
chk('D5 تاریخِ مرکب label id و دکمهٔ قابل تعامل aria-labelledby دارد', /data-jd=/.test(modals) && /jdate-label-/.test(modals) && /aria-labelledby="jdate-label-/.test(calendar));
chk('D6 فیلترهایِ audit همگی شناسهٔ واقعی برای label دارند', ['audit_q','audit_coll','audit_op','audit_sens'].every((id) => new RegExp('id="' + id + '"').test(audit)));
