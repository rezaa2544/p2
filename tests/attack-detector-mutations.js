...
function execute() {
  try { return { code: 0, output: execFileSync(process.execPath, ['tests/attack-detector.js'], { cwd: ROOT, encoding: 'utf8', timeout: 120000, stdio: ['ignore', 'pipe', 'pipe'] }) }; }
...
try {
  for (const mutant of mutants) {
    ...
    fs.writeFileSync(file, original[mutant.file].replace(mutant.from, mutant.to));
    const result = execute();
    fs.writeFileSync(file, original[mutant.file]);
...
} finally { for (const [name, file] of Object.entries(sources)) fs.writeFileSync(file, original[name]); }
