#!/usr/bin/env node
/* ═══════════════════════════════════════════════════════════════════
   دور 85 بند W3 — کشِ classScoreContext (بند 2.7 بازبینی)
   زمینهٔ نمراتِ هر کلاس تا بعد از نخستین تغییرِ داده کش می‌شود؛
   پیش از این، هر رندر کلِ db.grades را می‌گذشت.
...
(async function main(){
  console.log('▸ دور 85 — W3: کشِ classScoreContext (2.7)');
  ...
  /* ── صحنهٔ کنترل‌شده: یک کلاس + یک درسِ جدید + ۳ دانش‌آموزِ عضو ── */
  W(wa, `
    (function(){
      SYNC_MUTED = true;
      ...
      /* دو نمره برای عضوهای ۱ و ۲ (سومِ اعضا نمره دارد اما بعداً عضو می‌شود) */
      ...
      /* نمرهٔ سوم — هنوز نباید شمرده شود (عضویتِ سوم هنوز ثبت نشده) */
      window._g3 = insert('grades', Object.assign({}, base, {student_id: studs[2].id, score: 20})).id;
      window._s3 = studs[2].id;
      SYNC_MUTED = false;
    })();
  `);
  check('K1 صحنهٔ آزمون ساخته شد (کلاس + ۳ عضو + ۳ نمره)', W(wa, '!!window._c && !!window._g1'), '');
