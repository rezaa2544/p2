...
- **باقی‌مانده (ثبت‌شده در گزارش، دست نخورد):** (۱) `icsForStudent` فیلترِ `school_id` ندارد — تداخلِ عددیِ class id میانِ مدرسه‌ها در seed → رفعِ یک‌خطی در دورِ بعد (۲) شکافِ سیستماتیک: `insert('notifications')` از همهٔ نقش‌هایِ غیرمدیر توسطِ `inScope` رد می‌شود (پیشینِ PR#2) (۳) opِ ردشدهٔ demo در ردیفِ همگام‌سازی بی‌ضرر دیده می‌شود.
