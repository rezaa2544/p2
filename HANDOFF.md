...
## تثبیت قانون استفاده از اسکیل‌ها (Mandatory Skill Usage Rule) — ✅ (2026-09-19)

- **TAKEOVER-FROM:** `origin/main@889445d`؛ در بازیابی اولیه مشخص شد یک چتِ قبلی تغییر هم‌موضوع را در کامیتِ `docs(skills): add mandatory skill usage rule to AI_PROMPT.md and HANDOFF.md` روی `origin/main` ثبت کرده است.
- **PREVIOUS-STATE:** تغییر قبلی روی `origin/main` موجود بود، اما جدِ شاخهٔ ثابت این سشن (`arena/01a0b00d-p2@bc5a038`) نبود؛ بنابراین ادغامِ کورکورانهٔ تاریخچهٔ نامرتبط انجام نشد و وضعیتِ شاخهٔ فعلی مبنای کار قرار گرفت.
- **CURRENT-STATE:** قانون به‌عنوان بند ۱۴ در بخش قوانین اصلی `docs/AI_PROMPT.md` ثبت شد: خواندن خودکار اسکیل پیش از کارهای امنیتی، معماری، بازبینی کد، کارایی و تست؛ پیروی از روش استدلال اسکیل؛ ثبت خطای مراجعه‌نکردن؛ و اشاره به اسکیل استفاده‌شده در گزارش نهایی.
- **فایل‌های تغییرکرده:** `docs/AI_PROMPT.md` و همین `HANDOFF.md`؛ هیچ فایل کد، تست یا خروجی بیلد تغییر نکرد.
- **اسکیلِ استفاده‌شده:** طبق `payesh-standards`، وضعیت واقعی ریپو پیش از تغییر بررسی شد، تغییر مستنداتی محدود نگه داشته شد و تحویل با تست/کامیت/پوش/راستی‌آزمایی ثبت می‌شود.
- **تست و تحویل:** بررسی ساختاری قانون **11/11**، `git diff --check` و `tests/handoff-integrity.js` **4/4** سبز شدند؛ `tests/secret-scan.js` نیز **12/12** سبز و سلامت لینک‌ها **9/9** بود. `npm test` با کد **1** متوقف شد چون گیت پایهٔ `authz/write-perms.json` نسبت به مولد کهنه است (**34/35** در `tests/run.js`؛ بدون تغییر در این مأموریت). `node tests/docs-freeze-marker.js` با **13/14** ماند چون تغییر صریح `docs/AI_PROMPT.md` اثر ثبت‌شدهٔ rc44 را عوض کرده و بامپ قفل لازم است. تست دودی مستقیم به‌دلیل نبود `jsdom` اجرا نشد (کد **2**).
- **مشکلات باز:** فایل‌های نام‌بردهٔ پنج‌گانهٔ قانون در شاخهٔ پایهٔ این سشن هنوز در فهرست مهارت‌های موجود دیده نمی‌شوند؛ همچنین برای سبز شدن freeze باید نسخهٔ بعدی قفل مستندات تولید شود. این مأموریت فقط ثبت قانون در سند مرجع بود و تاریخچهٔ نامرتبط `origin/main` به‌صورت کامل وارد نشد.
- **تحویل نهایی:** کامیت `9da3a1f3fd8fddc9e1e586a48fd1b2b2e603a244` روی `arena/01a0b00d-p2` پوش شد و `git ls-remote origin refs/heads/arena/01a0b00d-p2` همان SHA را برگرداند؛ `origin/main@889445d` عمداً دست‌نخورده ماند و این کامیت هنوز روی main نیست.
- **قدم بعدی:** بامپ قفل مستندات و افزودن فایل‌های اسکیلِ غایب، کارهای پیگیری جدا هستند.
