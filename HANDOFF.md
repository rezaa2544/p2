...
## چت ۴: قاعدهٔ دائمی «قانون استفاده از اسکیل‌ها» در §۰ — ۲۸/۰۶/۱۴۰۵ (2026-09-19)

**تغییر:** به دستور ناظر، قاعدهٔ ۱۴ به بلوکِ قواعدِ الزامیِ §۰ در `docs/AI_PROMPT.md` افزوده شد: پیش از شروعِ هر کار در پنج حوزهٔ امنیت، معماری، بازبینی کد، کارایی و تست، خواندنِ `SKILL.md` مربوطه در `.claude/skills/` و پیش‌رفتن **طبق روش استدلالِ همان اسکیل** الزامی و خودکار است. دو قانونِ اجرایی: عدمِ مراجعه = خطای فرآیندی با ثبتِ صریح «⚠️ به اسکیل [نام] مراجعه نشد» در گزارش/هندآف؛ استفاده = ذکرِ کوتاهِ خروجیِ اسکیل در گزارشِ نهایی.
**نگاشتِ نام‌ها (مهم):** نام‌های دستور با دایرکتوری‌های واقعی فرق داشت — در قاعده با مسیرِ واقعی ثبت شد: `security-review`→`security-review-payesh`، `architect`→`software-architect`، `performance-audit`→`scalability-performance`، `testing-strategy`→`qa-testing`. اسکیلِ `code-review` وجود ندارد؛ `payesh-standards` جانشینِ رسمی تا ساختِ اسکیلِ اختصاصی.
**وضعیت شاخه:** `feat/wave20-chat4` — سه کامیتِ آرنا ۴/رفع باگ پنجشنبه (`226d04e`, `ead9de4`, `219e778`) روی ریموت تأیید شده‌اند؛ گیت‌ها سبز: دودی ۵۴۷/۵۴۷ · `check-authz` صفر · نشت‌یاب ۱۱/۱۱.
