...
**پیگیری‌های همان دور (سه کامیتِ بعدی):**
- `930349b` — `/api/health` از دروازه‌ی ابطال مستثنا شد (پایش‌گری که
  کلوچه‌ی کهنه بفرستد نباید سامانه را خراب ببیند) + سنجه‌ی R18 و جهشِ M13.
- `ddaf28f` — رفعِ قرمزِ `authz-model`: سنجه‌ی X6 فیلدهایی می‌ساخت که در
  مدل نیستند (`schools.county` غلط است — `county_id` درست؛ `office_id`
  مالِ users است). این قرمز از **جلسه‌ی قبل** جا مانده بود چون خروجیِ
  رگرسیون را با الگویِ دستی پالایش می‌کردم و خلاصه‌هایی مثل «۲۳۳ سبز /
  ۲ قرمز ❌» را نمی‌گرفت ⇒ درس: خروجی را خط‌به‌خط بخوان، نه با الگو.
- `0d2fa71` — جهشِ M1 در server-mutations به‌خاطرِ دفاعِ دوگانه زنده
  مانده بود؛ به‌جای حذف/سست‌کردنِ جهش، سنجه قوی شد (S11b: ابطال باید در
  بررسیِ توکن رخ دهد ⇒ کدِ پاسخ `no_session`، نه `revoked`). حالا ۲۰/۲۰.
  ⚠️ این کامیت **پوش نشد**: احراز هویتِ GitHub در این محیط منقضی شد
  (`gh auth status` «token no longer valid»)؛ با وصل شدنِ دوباره، یک
  `git push` کافی است.

## دورِ ۱۰۵ — پارتیشن‌بندیِ وزن‌دار برای مدارسِ شلوغ (فاز ۲.۴) — ۱۷/۰۶/۱۴۰۵ — انجام شد ✅

**کامیت:** `3e9ee96` · **سند:** `docs/WEIGHTED_PARTITIONING.md`
**ماژولِ تازه:** `server/partitioning.js` · **تست:** ۳۲ بررسی + ۹/۹ جهش

**یافته‌ای که همه‌چیز را عوض کرد:** هیچ شارد و رپلیکایی نساختم، چون در این
استقرار **هیچ پایگاهی در مسیر نیست** — تعدادِ فراخوانیِ `db.query` در کلِ
سرور **صفر** است و همه چیز روی storeِ JSON می‌چرخد. و بزرگ‌ترین مدرسهٔ دادهٔ
نمونه ۱۳۵ دانش‌آموز دارد، نه ۱۰۰۰. پس گزینه‌های A/B/C امروز قابلِ اجرا
نبودند؛ گلوگاهِ واقعی را پیدا کردم: `filterByScope` در شش مسیر، هر بار کلِ
مجموعه را پویش می‌کرد.

**چه شد:**
- `server/partitioning.js`: وزنِ هر مدرسه (ردیف‌های سنگین + دانش‌آموز)،
  تقسیمِ وزن‌دارِ LPT (قطعی)، شناساییِ مدارسِ شلوغ، تصمیمِ مسیریابی
  (shard/tier/replica)، نمایهٔ تفکیک‌شده، متریک‌های پایش
- نمایه **حافظِ ترتیب** (ادغامِ مرتّب، نه چسباندن) — وگرنه صفحه‌بندیِ مسیرها
  عوض می‌شد. هم‌ارزی با رفتارِ قدیمی روی دادهٔ واقعی: ۳۲/۳۲
- بی‌اعتبارسازی: خودکار با تغییرِ طول/جایگزینی، و از `sync.js` پس از هر op
  (برای انتقالِ بینِ دو مدرسه با طولِ ثابت ⇒ جداسازیِ مستأجر، سنجه‌ی W15)
- `GET /api/admin/partition-report` — فقط مدیرِ کل، فقط دادهٔ تجمیعی
- چهار مسیر (attendance/grades/users/classes) به مسیرِ تفکیک‌شده رفتند؛
  مسیرِ آرایه‌ایِ `filterByScope` دست‌نخورده ماند و در نبودِ نمایه به همان
  پویشِ قدیمی برمی‌گردد (هرگز داده پنهان نمی‌شود)

**اندازه‌گیری:** توازن از ضریبِ تغییراتِ ۰٫۴۰۸ به ۰٫۰۰۴ · خوانشِ attendance
۷٫۴× و grades ۷٫۳× سریع‌تر · روی ۱۲۰هزار ردیف، بازپرداخت از ۱۳اُمین خوانش.

**دو خطایِ خودم که در مسیر گرفتم:** دو بار اسکریپتِ ویرایش را بدونِ ذخیره
بستم (⇒ `ReferenceError` در sync.js و `apiPartitionReport is not a function`
در admin.js)؛ `node --check` هیچ‌کدام را نمی‌گیرد. از این به بعد پس از هر
ویرایش، **جدولِ تأییدِ نشستنِ تغییر در فایل** اجرا می‌شود (در این دور ۱۱/۱۱).

**رگرسیونِ کامل پس از کار: ۱۶۴ سوئیت، صفر قرمز.**

## دورِ ۱۰۴ — ابطالِ نشست (P4) — ۱۷/۰۶/۱۴۰۵ (2026-09-09) — انجام شد ✅

**کامیت:** `feat(security): implement session revocation denylist`
**سند:** `docs/SESSION_REVOCATION.md` · **ماژولِ تازه:** `server/revocation.js`

**نکتهٔ اول (صداقت):** ابطالِ نشست از قبل بود و درست کار می‌کرد
(`store.__revoked_jti`)؛ چیزی که نبود **توزیع‌شدگی** بود. پس این کار
«افزودنِ یک لایه‌ی Redis + Pub/Sub روی همان تصمیم» بود، نه ساختن از صفر —
و مراقب بودیم رفتارِ تک‌نمونه‌ای ذره‌ای تغییر نکند (L1 همان کلید، همان
شکلِ عددی، همان GC، همان حفظ در restore).

**سه لایه:** L1 همان نقشه (همزمان، بی‌I/O) · L2 کلیدِ Redis با EX=۲۸۸۰۰ ·
L3 Pub/Sub برای پخشِ فوری. دروازه‌ی ناهمزمان در `onRequest` پیش از هر مسیرِ
احراز‌شده — **به‌جز** `send-code`/`login`/`logout` که باید با کلوچه‌ی مرده
هم باز بمانند، وگرنه کاربر برای همیشه قفل می‌شد (R12).

**سه انحراف از متنِ درخواست (مستند در سند):**
1. `server/gdpr.js` در این مخزن **وجود ندارد** (نه این شاخه، نه main)؛
   مسیرِ حقِ فراموشی همان `POST /api/auth/delete-account` است.
2. کدِ مرجع از `redis.setex` و `redis.smembers` استفاده کرده بود که
   **هیچ‌کدام در `server/redis.js` نیستند** ⇒ با همان کد، نخستین خروج
   TypeError می‌داد. با `set(...,'EX',ttl)` و آرایه‌ی JSON جایگزین شد.
3. `revokeAllUserSessions` در کدِ مرجع مجموعه‌ای را می‌خواند که **هیچ‌کس
   پُرش نمی‌کرد** ⇒ «ابطالِ همه» همیشه تهی بود (خطایِ خاموش).
   `trackSession()` هنگامِ صدورِ توکن افزوده شد.

**باگِ پنهان که حینِ کار یافت و رفع شد:** `enumStage` ابطال را به شکلِ
آبجکت می‌نوشت `{at,reason}` و GC حساب می‌کرد `now - value` ⇒ NaN ⇒ آن
کلیدها **هرگز** زباله‌روبی نمی‌شدند ⇒ رشدِ بی‌پایانِ store با هر حمله‌ی
شمردنِ شناسه. همه‌جا عدد شد + GC هر دو شکل را پاک می‌کند.

**یک متدِ فقط‌افزودنی به redis.js:** `getStrict` (خطا را نمی‌بلعد) — بی‌آن،
«نشد تشخیص داد» و «نبود» یکی می‌شدند و fail-closed ناممکن بود.

**آزمون:** ۳۱ + ۱۶ بررسی، ۱۲/۱۲ جهش کشته. دروازه‌ها: smoke ۵۴۷/۵۴۷،
run.js ۳۵/۳۵، check-authz ۰، secret-scan ۱۱/۱۱، server1 ۳۱/۳۱،
server15/16/17/18 و security2 و xss-guard همگی سبز.
هزینه: ۰٫۷۱۶ms → ۰٫۷۴۰ms میانه (حدود ۲۴ میکروثانیه برای هر درخواست).

## دورِ ۱۰۳ — رفعِ قرمزِ نگهبانِ XSSِ دسته‌ی D (پیگیریِ رگرسیون) — ۱۷/۰۶/۱۴۰۵ (2026-09-09) — انجام شد ✅

**علت:** رگرسیونِ کاملِ پس از D.4 نشان داد `tests/xss-guard.js` قرمز است
(`X2b document.write فقط در نقاطِ ممهورِ چاپِ رسید`) — دو نقطهٔ `document.write`
که D.2 (کارتِ امتیازی) و D.4 (کمبودِ نیرو) برای «چاپ» افزوده بودند، عمداً توسط
این نگهبان متوقف می‌شوند تا **بازبینی** شوند. این قرمز واقعی بود، نه نویز.

**کاری که شد (بدون تضعیفِ نگهبان):**
1. بازبینیِ دستیِ هر دو نقطه: همان الگویِ ممهور — `window.open('','_blank')` و
   نوشتنِ یک رشته که تک‌تکِ مقادیرش از `esc()` گذشته است.
2. افزودنِ آن‌ها به فهرستِ مجازِ `DW_ALLOW` **با توضیحِ مستند در خودِ تست**.
3. افزودنِ سنجه‌ی تازه‌ی **X6**: تزریقِ payload واقعی (`</title><img onerror…>`)
   به نامِ مدرسه، نامِ درس، یادداشت و عنوان، سپس **پارس کردنِ خروجیِ چاپ در jsdom**
   و سنجش اینکه هیچ عنصرِ `img`/`svg` ساخته نشده و `window.__px` اجرا نشده است.
   معیارِ نخست (regex روی رشته) غلط بود: `onerror="…"` درونِ **متنِ escaped**
   بی‌ضرر است و regex آن را خطا می‌گرفت.

**جهش‌کُشیِ X6 (اثباتِ اینکه سنجه کور نیست):** سه جهش، هر سه کشته شدند —
حذفِ `esc` از (۱) یادداشتِ ردیف، (۲) `<title>`ِ کارتِ امتیازی، (۳) نامِ درس.
نکتهٔ فنی: `xss-guard` روی `index.html` ساخته‌شده اجرا می‌شود، پس پیش از هر
جهش باید `node build.js` زده شود.

**دروازه‌ها پس از اصلاح:** `xss-guard` ۲۹/۲۹ · `smoke` ۵۴۷/۵۴۷ · `run.js` ۳۵/۳۵ ·
`build.js --check` تطبیق کامل · `check-authz` ۰ · suitesِ D: ۱۰/۱۰، ۱۲/۱۲، ۱۰/۱۰
(+ جهش‌ها ۶ و ۷ و ۷).

**پرونده‌های تغییریافته:** `tests/xss-guard.js` (فقط تست — هیچ کدِ تولیدی تغییر نکرد).

## دورِ ۱۰۲ — دسته‌ی D از «دستورِ جامعِ فرناز» (اداره) — ۱۷/۰۶/۱۴۰۵ (2026-09-08) — در جریان 🔄

**وضعیت:** شاخه‌ی `arena/01a08256-p2` (از `main@4ca531c`) — یک کامیت برای هر بند.
سندِ طراحی: `docs/D1_OFFICE_LEVEL_DESIGN.md`.

- **D.1 — تفکیکِ دو سطحِ رئیس اداره (فقط سند، بدون کد):** یافته با مدرک:
  نقش یکی است (`30-authz.js:24`) ولی **سه سطحِ واقعی** روی رکوردِ اداره نشسته
  (`offices.level` — `24-edu-office.js:4`)، نه روی حساب؛ خانهٔ نقش یک داشبوردِ
  مشترک است (`07-shell.js:161`)؛ و **سمت سرور این نقش دامنهٔ نامحدود دارد**
  (`server/sync.js:447` ← `if(u.role==='edu_office') return true;`) — یعنی
  تفکیکِ سطح بدون اجرایِ سروری فقط تزئین است. پیشنهاد: `users.office_level`
  (`district`/`province`) + نگاشتِ صریحِ `county ⇒ district` + نگاشتِ سطح
  با پیش‌فرضِ **fail-closed = `district`** + `effectiveOfficeLevel()`؛
  نقشِ تازه‌ای ساخته نمی‌شود (پیشگیری از انفجارِ مجوزها).
- **D.2 — کارتِ امتیازیِ منطقه (کد):** ماژولِ تازه‌ی
  `src/js/69-office-scorecard.js` (فقط خواندن) + روتِ `officescore`
  (در منوی اداره؛ برای سوپرادمین در `EXTRA_ROUTES`) + دکمه در داشبورد.
  پنج بُعد: مالی (فقط نرخِ وصول — **هیچ مبلغی در خروجی نیست**)،
  فرایندهای داخلی (ثبتِ حضورِ ۳۰روزه + نرخِ حضور + ثبتِ نمره)،
  رشد (بزرگ‌ترین نوبت در برابرِ نوبتِ پیشین؛ باند از **جهتِ تغییر**،
  نه از خودِ نمره)، رضایتِ اولیا و مدیریتِ مدرسه‌داری.
  🔴 قاعدهٔ ثابت: **بُعدِ بی‌داده `null` است و در میانگین نمی‌آید** —
  نمرهٔ ساختگی از بی‌داده‌بودن بدتر است. دو قلابِ زنده
  (`parentSatisfactionScore`, `safetyDrillRate`) برای منابعی که هنوز
  در سامانه نیستند (آزمون با تزریقِ مجموعه، زنده‌بودنشان را می‌سنجد).
  خروجیِ چاپیِ مستقل (RTL، print CSS، بدون `<script>`).
  آزمون: `tests/officescore2.js` **۱۰/۱۰** + جهش‌ها **۵/۵** کشته.
  سرعت (کلاسِ ۱۳نفره، میانهٔ ۳ اجرا): پایه ۲۹.۶ms ← بعد ۲۹.۲ms (بی‌تغییر).
  smoke همچنان **۵۴۷/۵۴۷** (فقط «انتظارِ منو»یِ اداره در smoke به‌روز شد).
- **D.3 — پیامِ «فوری / بحرانی» (کد):** ماژولِ تازه‌ی
  `src/js/70-urgent-notice.js` + فیلدِ `announcements.urgent`
  (مدل + جدولِ تولیدشده + شِمای سرور). سه جا، یک معنا:
  اطلاعیه/بخشنامه (`urgent=۱`)، اعلانِ درون‌سامانه‌ای (`type='urgent'`
  — بی‌نیاز از فیلدِ تازه)، صفِ پیامک (`kind='urgent'`).
  رفتار: فوری همیشه **بالای فهرست** است (حتی از اصلاحیه بالاتر)،
  قرمز + 🚨، و **در صف اولویت می‌گیرد**.
  🔴 «بدون تأییدِ دستی» را بی‌قید نگذاشتیم: ارسالِ بی‌درنگ فقط وقتی
  است که خودِ مدرسه پیامک را روشن کرده باشد **و** تنظیمِ تازه‌ی
  `urgentAutoSend` را نبسته باشد (پیش‌فرض روشن، قابلِ خاموش‌کردن در
  تنظیمات + بنرِ هشدار در صف). سقفِ روزانه همچنان دیوار است و
  **مهلتِ اصلاح فقط وقتی کوتاه می‌شود (۲ دقیقه) که پیام واقعاً خودکار
  برود** — وگرنه اشتباهِ دبیر با همان مهلتِ همیشگی قابلِ جمع‌کردن است.
  آزمون: `tests/urgent2.js` **۱۲/۱۲** + جهش‌ها **۶/۶** کشته (از جمله
  جهشِ «همیشه بی‌درنگ بفرست» و «گاردِ نقشِ ناشر را بردار»).
  دروازه‌ها: smoke ۵۴۷/۵۴۷ · run.js ۳۵/۳۵ · check-authz ۰ ·
  build --check سبز · server1/15/16/18/11-sms سبز ·
  سرعتِ حضور (کلاس ۱۳نفره): پایه ۲۹.۶ms ← ۲۹.۱–۳۲.۰ms (بدون افتِ معنا‌دار).
- **D.4 — کمبودِ نیروی انسانی به تفکیکِ درس (کد):** مجموعهٔ تازهٔ
  `staff_needs` (درس + تعدادِ دبیر + توضیح + وضعیت) و روتِ `staffneeds`
  با **یک نما و دو نقش**: مدیر اعلام می‌کند (مدرسهٔ خودش)، اداره
  تجمیعِ مدارسِ محدوده را می‌بیند (خلاصه به تفکیکِ درس + جدولِ مدرسه،
  با نسخهٔ چاپی).
  🔴 سه قاعده: **اداره فقط می‌خواند** (در مدل و در جدولِ تولیدشدهٔ سرور
  هم `ins/upd/del` برای `edu_office` بسته است) · **درس باید از همان
  مدرسه باشد** (دو بار سنجیده می‌شود: انتخابگر و هنگامِ ذخیره) ·
  **تأمین‌شده حذف نمی‌شود** — فقط `filled` می‌گیرد تا سابقه بماند.
  یافتهٔ حینِ کار و رفع: اکشن‌هایِ تغییرِ وضعیت/حذف، مالکیتِ مدرسه را
  نمی‌سنجیدند ⇒ `needWritable()` افزوده شد (سمتِ سرور `inScope` از قبل
  می‌سنجید، ولی رابط نباید اصلاً پیشنهادِ غلط بدهد).
  آزمون: `tests/staffneeds2.js` **۱۰/۱۰** + جهش‌ها **۶/۶** کشته.
  دروازه‌ها: smoke ۵۴۷/۵۴۷ (دو «انتظارِ منو» به‌روز شد) · run.js ۳۵/۳۵ ·
  check-authz ۰ · build --check سبز · server1/15/16/18/11-sms سبز ·
  سرعتِ حضور (کلاس ۱۳نفره): پایه ۲۹.۶ms ← ۲۷.۸ms (بدون افت).
- **انحرافِ شاخه (گزارش‌شده به کارفرما):** دستور خواسته بود شاخه‌ی
  `feat/farnaz-phase1-chat2`؛ این سشن به `arena/01a08256-p2` قفل است و ساخت/
  پوشِ شاخه‌ی دیگر ممکن نیست ⇒ همهٔ بندها با همان نام‌گذاری و ترتیب اینجا
  پیاده و در پایان PR از همین شاخه باز می‌شود.
