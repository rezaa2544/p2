...
- **ران‌بوک تولید (سند اجباری §30):** هشت بخش — معماری (دیاگرام + چهار مسیر حیاتی + تفکیک منابع مشترک/محلی + هشدار صادقانهٔ تک‌نمونه‌ای تا ادغام موج ۱) · بوت/خاموشی (پیش‌پرواز ۷بندی + ترتیب بوت پستگرس→ردیس→اپی‌آی→لبه و خاموشی برعکس + راستی‌آزمایی + محدودیت تخلیهٔ درحال‌پرواز تا موج ۱۵) · پایش (وضعیت واقعی `/api/health` و `/metrics` + قرارداد موج ۱۵؛ هفت آلارم با آستانه و پلی‌بوک مقصد؛ ماتریس ارجاع) · عملیات روتین (بکاپ/دریل ماهانه، چرخش لاگ، حافظهٔ ردیس، تورم/وکیوم، گواهی) · مقیاس (افقی/عمودی/ردیس/رپلیکا) · پیکربندی (جدول متغیرها + چرخش رمز نشست با `PAYESH_JWT_SECRET_PREV`) · بازگشت (ارجاع به `DEPLOY.md`/`DR_RUNBOOK`) · تماس‌های اضطراری.
