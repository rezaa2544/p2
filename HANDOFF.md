...
- **محتوای قانون:**
  ۱. پیش از شروع هر کار مربوط به یکی از ۵ حوزه (امنیت، معماری، بازبینی کد، کارایی/سرعت، تست)، ابتدا فایل `SKILL.md` مربوطه در `.claude/skills/` خوانده شده و **طبق روش استدلال همان اسکیل** عمل می‌شود.
  ۲. **خودکار:** بدون نیاز به یادآوری صریح در پرامپت‌ها.
  ۳. **ردیابی خطای فرآیندی:** در صورت عدم مراجعه به اسکیل مرتبط، صریحاً عبارت «⚠️ به اسکیل [نام اسکیل] مراجعه نشد» در پاسخ و `HANDOFF.md` ثبت می‌شود.
  ۴. **شواهد:** در گزارش نهایی به اسکیل استفاده‌شده اشاره می‌گردد (مانند: «طبق security-review-payesh...»).
- **وضعیت:** ثبت در `docs/AI_PROMPT.md` و `HANDOFF.md` انجام شد، کامیت و پوش روی شاخه `main` گردید.
