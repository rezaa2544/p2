...
- **نقش:** معمار ارشد مستقل + ممیز رد تیم امنیت (Phase 8 Initiation — Independent Principal Architect + Red Team Security Auditor).
- **کامیت بازرسی‌شده:** `4530d753a46b402712acea9087877ffa3cd11a33` (HEAD فعلی `main`؛ یک کامیتِ صرفاً مستنداتی/اسکیل جلوتر از `DOC_COMMIT` ادعاشده `d32915f3`).
- **محیط ممیزی:** Debian 13، Node 20.20.2 + 22.14.0، PostgreSQL 17.11، Redis 8.0.2 — بازاجرا و بازتولید تمام گیت‌های فاز ۷.۶ بدون پذیرش هیچ ادعای مستندی.
- **شواهد بازتولیدشده (E3/E4):**
  - زنجیره مایگریشن ۰۰۱→۰۲۰ روی PostgreSQL 17.11: ۲۰/۲۰ بدون خطا.
  - `tools/production-truth-gate.js`: **۴۴/۴۴ — VERDICT: VERIFIED** روی HEAD فعلی (شامل توقف زنده PG و اثبات fail-closed نوشتن).
  - `tests/canary-atomic-postgres-live-runtime.js`: ۱۰/۱۰ (ROLLBACK اتمیک با تریگر شکست واقعی روی درج آدیت).
  - `tests/server17.js` ۷۰/۷۰ · `migration-sequence` ۱۹/۱۹ · `migrate-pg-constraints` ۱۴/۱۴ · `run.js` ۳۵/۳۵ · `smoke.js` ۵۴۷/۵۴۷ (Node 22) · مجموع ادعاشده ۱۲۷ = ۱۴+۷۰+۱۹+۱۴+۱۰ — بازتولید شد.
  - دریل‌های زنده پروداکشن: ایزولاسیون تننت (کراس‌مدرسه/کراس‌استان ⇒ 403)، کشتن ردیس ⇒ 503 fail-closed مسیرهای auth/OTP، قطع PG ⇒ 401/503 بدون فال‌بک RAM/JSON، `/metrics` در تولید بدون توکن ⇒ 404.
- **یافته‌های کلیدی (۲۵ مورد R1–R25؛ ۳ مورد قرمز):**
  - 🔴 **R5:** `DATABASE_URL` تنظیم‌شده بدون `REDIS_URL` ⇒ سرور بوت می‌شود، listen می‌کند و برای همیشه 503 می‌دهد ([FATAL] لاگ می‌شود اما fail-fast نیست) — بازتولید زنده.
  - 🔴 **R15:** حالت پیش‌فرض WAF فقط report است نه enforce.
  - 🔴 **R20:** سوئیت رگرسیون ۱۲۷تایی در CI خودکار نشده (اجرا دستی است).
  - 🟠 نمونه‌ها: ۵ ماژول کنترل‌پلن هنوز RAM-authoritative (بدون invalidation بین‌نمونه‌ای)، ناهمگونی گیت‌های NODE_ENV/PAYESH_ENV، fail-open بودن revocation در قطعی ردیس (مستند و عمدی)، بوم تست verifier وابسته به env (بدون DATABASE_URL=۱۴/۱۴، با آن=۱۳/۱۴)، عدم وجود ledger ماشینی مایگریشن.
  - 🟡 **R25:** ادعای «R.7.3 بسته شده / HEAD=d32915f3» با مخزن منطبق نیست — هیچ گزارش R.7.3 وجود ندارد؛ آخرین بستِ مستند همان R.7.2 است (اسکیل‌کامیتِ پس از آن فقط مستنداتی بود؛ کد پروداکشن دست‌نخورده ماند).
- **roadmap فاز ۸:** 8.1 سخت‌سازی عملیاتی (بستن R5/R15/R20 + CI کامل) → 8.2 مشاهده‌پذیری و پاسخ به حادثه → 8.3 اعتبارسنجی کارایی و مقیاس → 8.4 بازبینی امنیتی پیشرفته (متمرکزسازی گارد تننت) → 8.5 گیت گواهی تولید.
...
## ۲۰۲۶-۰۹-۲۱ — چت ۴: ممیزی CI / Release Governance روی current HEAD (+ یک اصلاح کد)

- **مبنا:** `ce990836` (نه SHA تاریخی `c29e3073`). ۲۳ کامیت دریافتی همگی docs-only بودند؛ هیچ drift کدی رخ نداده بود. Node v22.23.2 در `/tmp` نصب شد تا اجرای واقعی ممکن شود.
- **artifact:** `docs/audit/CHAT4_CI_RELEASE_GOVERNANCE_CURRENT_HEAD.md` (بخش‌های A–E).
- **کامیت:** `0d245b9c` — شامل یک اصلاح کد + یک اصلاح تست + بازنویسی گزارش تأیید.
- **F-QA-08 (P1) — بازتولید شد و رفع شد:** `tools/redis-backup.sh` هنگام تداخل flock با `exit 0` خارج می‌شد و هیچ بکاپی نمی‌گرفت؛ cron مستند (`0 */6 * * *`) هرگز متوجه نمی‌شد. بازتولید محلی: قفل در پروسهٔ دیگر نگه داشته شد ⇒ **exit 0 با صفر فایل بکاپ**. اصلاح حداقلی: خروج با کد **۷۵ (EX_TEMPFAIL)** روی stderr، عمداً جدا از کد ۱ (خطای واقعی). تست رگرسیون B7/B7b/B7c افزوده شد: علیه اسکریپت **قبل از اصلاح ۹/۱۱ با exit 1** و بعد از اصلاح **۱۱/۱۱ با exit 0**. پس از تغییر: `disaster-recovery-coverage` ۴۷/۴۷ و `npm test` ۳۵/۳۵ + ۵۴۷/۵۴۷ همچنان سبز، `bash -n` تمیز.
- **F-QA-02 (P0) — بازتولید شد و اصلاح شد:** `docs/audit/PHASE_8_2_FINAL_VERIFICATION_REPORT.md` دست‌نخورده با ۲۲ خط و حکم `VERIFIED` به کامیت `b0b55a13` استناد می‌کرد که **Node.js CI #1074 آن FAILURE** بوده و اکنون **۷۶ کامیت عقب** است؛ صفر run_id، صفر شمارش تست، صفر کد خروج. ادعا **پس گرفته شد** و با شواهد قابل ردیابی جایگزین شد. سند صراحتاً می‌گوید `Phase 8.2 Exit = NOT VERIFIED`، `Phase 8.3 = BLOCKED`، `Production GO = NOT DECLARED` — هیچ ارتقایی داده نشد.
- **F-QA-03 — بازتولید نشد، وضعیت CURRENTLY GREEN:** ۱۷ اجرای کامل‌شدهٔ اخیر Node.js CI همگی موفق (نرخ ۱۰۰٪، در برابر ۶۵٪ تاریخی). **#1155 (id `35579534157`) روی current HEAD: ۲۸/۲۸ مرحله، صفر skip، صفر failure.** واگرایی تاریخی PR-vs-push روی `fbe178be` و boot timeout تکرار نشد. تاریخ حفظ شد، پاک نشد.
- **F-QA-01 (P1) — همچنان OPEN، تگ جابه‌جا نشد:** `phase8.2-verified` روی `401d02b2` است که **هیچ اجرای Node.js CI ندارد** (فقط دو Fortify شکست‌خورده). طبق دستور مأموریت فقط audit شد، نه جابه‌جایی cosmetic.
- **F-QA-04 (P2) — governance weakness نه vulnerability:** جدیدترین Codacy (#316 روی همین HEAD) همان علت را دارد — `No rules found` ×۱۲، `ConfigurationNotFoundError` ×۳، **صفر یافتهٔ امنیتی**. نکتهٔ جدید: `.codacy.yml` وجود دارد و `pmd`/`pmd-legacy` را disable کرده اما آن‌ها **همچنان اجرا می‌شوند** ⇒ config توسط action رعایت نمی‌شود. `max-allowed-issues: 2147483647` هم اگر ابزار روزی موفق شود، گیت شمارش را خنثی می‌کند. Fortify به دلیل نبود `FOD_*` قرمز است.
- **F-QA-05 (P2) — عدد تاریخی تصحیح شد:** زیر `tests/` **۹۳۱ فایل `.js`** وجود دارد (۵۲۰ سطح بالا + ۴۱۱ در زیرپوشه‌ها)، نه ۵۲۰. `npm test` فقط ۲ سوئیت را سیم‌کشی می‌کند در حالی که `node.js.yml` **۲۸ ورودی مجزای `node tests/*.js`** را صدا می‌زند. **TEST/CI PARITY GAP** ثبت شد؛ عمداً اصلاح نشد چون راه‌حل درست یک قرارداد صریح CI است، نه ویرایش یک‌جانبه.
- **F-QA-06 — CLOSED:** نام `Live-Redis suite` **دقیق است**؛ سرویس واقعی `redis:8` با health-check (`redis-cli ping`، interval 5s، retries 20) و `REDIS_URL` سراسری در job تنظیم شده. تنها ظرافت: خارج از CI و بدون `REDIS_URL` سوئیت روی مسیر حافظه exit 0 می‌دهد.
- **F-QA-07 (P2) — release hygiene:** `package.json` نسخهٔ ۱٫۰٫۰، جدیدترین تگ semver `v1.0.1`، **۱۹۱۸ کامیت** از آن فاصله، `git describe` = `phase8.2-final-main-60-gce990836`، CHANGELOG **موجود نیست**. هیچ release کاذبی تولید نشده و هیچ گیتی به آن وابسته نیست.
- **F-QA-09 (INFO) — بار سوم تکرار شد:** نشست با سه artifact آغاز شد — `monitoring/alert-rules.yml` (symlink، مود 120000) حذف‌شده و `tools/migrate-ledger.js` + `tools/production-verifier.sh` از 100755 به 100644 تنزل‌یافته. مجموع تغییر محتوایی = **۱ خط** (هدف symlink). این خرابی ابزار snapshot است نه نقص مخزن. با `git checkout --` بازگردانده و روی remote تأیید شد. **هرگز `git add -A` نزنید.**
- **F-QA-10 — WITHDRAWN تأیید شد:** `monitoring/alert-rules.yml` همچنان symlink است و `infra/observability/prometheus.yml:20-22` **هر دو** فایل قواعد را بار می‌کند. هیچ config تغییر نکرد. باقی‌مانده: `monitoring/alert-rules.yaml` (۱۲ آلارم) بدون ارجاع (P3).
- **یافتهٔ جانبی ثبت‌شده (خارج از دامنه، تغییر نکرد):** `tools/redis-backup.sh` در HEAD مود `100644` دارد در حالی که cron مستند آن را به‌عنوان مسیر اجرایی صدا می‌زند.
- **جمع‌بندی زنجیره:** `code → tests → CI` سالم است؛ `release/tag` و `security workflows` شکسته‌اند — تگ شواهد CI ندارد و دو workflow امنیتی هیچ سیگنال قابل استفاده‌ای در هیچ جهتی تولید نمی‌کنند.

---

## ۲۰۲۶-۰۹-۲۱ — چت ۴ (ادامه): تعیین تکلیف چهار یافتهٔ باز — F-QA-01 / 04 / 05 / 07

**مبنا:** `6b627a8f` → پس از rebase روی دو کامیت واردشدهٔ چت‌های دیگر، پوش شد به `7afad24e`.
**کامیت‌ها:** `8e4d2c8b` (CODE) و `7afad24e` (DOC) — تفکیک کامل، بدون هم‌پوشانی.

### F-QA-04 — دو علتِ ریشه‌ایِ جداگانه کشف شد (نه یکی)

**الف) Fortify اصلاً یک workflow نامعتبر بود — نه مشکل credential.**
`secrets` در `if:` سطحِ step **مجاز نیست** (قواعد context-availability گیت‌هاب). هر دو شرط
(خطوط ۴۵ و ۵۷) مستقیم به `secrets.*` ارجاع می‌دادند ⇒ فایل **INVALID** بود و هر اجرا
بلافاصله با **صفر job** شکست می‌خورد (`created_at == updated_at`). یعنی شاخهٔ «اگر
credential نبود skip کن» **هرگز اجرا نمی‌شد** و Fortify در هیچ جهتی سیگنال نمی‌داد.
اثبات ماشینی: `actionlint 1.7.7` چهار خطای `[expression]` در ۴۵:۱۸، ۴۵:۴۹، ۵۷:۱۸، ۵۷:۴۹
گزارش کرد؛ پس از اصلاح **کاملاً تمیز**. چون `secrets` در `jobs.<id>.env` مجاز است،
حضور credential یک‌بار آنجا به رشتهٔ ساده تبدیل شد و stepها روی `env.*` شاخه می‌زنند.
**نتیجهٔ واقعی روی CI:** «Fortify AST Scan» **#351 = success** — اولین موفقیت پس از
۳۰ شکست پیاپی؛ step «Run Fortify Scan» = **skipped** و پیام صریح نبودِ credential در لاگ.
یعنی دقیقاً رفتار صادقانه: اجرا می‌شود، صریح می‌گوید credential ندارم، و **وانمود به
اسکن نمی‌کند**. نبودِ `FOD_*` عمداً به‌عنوان tooling state دست‌نخورده باقی ماند.

**ب) `.codacy.yml` — اصلاح لازم بود ولی کافی نبود (صادقانه ثبت شد).**
فایل با `---` شروع نمی‌شد و طبق مستندات Codacy بدون آن اصلاً به‌عنوان فایل پیکربندی
شناخته نمی‌شود. اضافه شد. **اما اندازه‌گیری پس از اصلاح (اجرای #319 روی `7afad24e`)
نشان داد نتیجه ذره‌ای عوض نشده** — نشانگرها بایت‌به‌بایت مثل قبل: `No rules found` ×۱۲،
`ConfigurationNotFoundError` ×۳، شکست eslint/pmd/pmd-legacy، و در نهایت
`MalformedInputException` که CLI را می‌کُشد. پس نتیجهٔ صریح: **این فایل اصلاً توسط
action خوانده نمی‌شود**؛ پیکربندی از Codacy Cloud با `project-token` می‌آید و
`secrets.CODACY_PROJECT_TOKEN` خالی است (در `with:` لاگ غایب است). این واقعیت داخل
خود فایل مستند شد تا کسی اصلاح فرمت را «رفع مشکل» نخواند.
همچنین طبق مستندات خودِ Codacy، `engines.<tool>.enabled` **هرگز** توسط فایل پیکربندی
اعمال نمی‌شود (فقط در UI) — به همین دلیل pmd با وجود disable بودن اجرا می‌شد؛ آن بلوک
اکنون صراحتاً «inert» علامت خورده. Codacy تا امروز **۳۱/۳۱ شکست** و **هرگز SARIF آپلود
نکرده**؛ مرحلهٔ Upload همیشه skipped است. **صفر یافتهٔ امنیتی** در کل لاگ.
`max-allowed-issues: 2147483647` عمداً دست‌نخورده ماند: ضعف حاکمیتی است نه آسیب‌پذیری،
و پایین‌آوردنش تصمیم مالک است. **BLOCKED — OWNER DECISION REQUIRED** برای تنظیم
`CODACY_PROJECT_TOKEN` یا بازنشسته‌کردن این workflow.

### F-QA-05 — قرارداد صریح + گیت ماشینی (بدون حذف/skip هیچ تستی)

اعداد **اندازه‌گیری‌شده**: ۹۳۱ فایل `.js` زیر `tests/`، ۵۲۱ در سطح اول، `npm test` فقط
**۲** ورودی، `node.js.yml` **۳۰** و `security.yml` **۶** ⇒ مجموع اجراشونده **۳۶**، و
**۴۸۵ فایل تستِ سطح‌بالا که هیچ اتوماسیونی اجرایشان نمی‌کند**. عدد تاریخی «۵۲۰» شمارش
فایل بود و زیرپوشه‌ها را نمی‌دید. تأیید شد `tests/run.js` **orchestrator نیست** (هیچ
spawn/require تست دیگری ندارد) — سوئیت ساختاری است.
نمونهٔ تصادفی ۱۴‌تایی از فایل‌های اجرانشده: **۹ موفق / ۴ ناموفق / ۱ timeout** ⇒ این‌ها
تست واقعی‌اند و بعضی همین حالا قرمزند (مثلاً `tests/ha-config.js` دربارهٔ بیتِ اجرایی
۱۰ اسکریپت که در HEAD واقعاً `100644`اند — نه artifact سندباکس؛ و `canary-persistence.js`
که `DATABASE_URL` می‌خواهد). قرارداد **آن‌ها را سبز اعلام نمی‌کند**.
`tests/ci-test-parity-contract.js` (۱۰ بررسی) + `docs/TEST_CI_PARITY_CONTRACT.md` ساخته و
به‌عنوان مرحلهٔ CI پیش از `npm test` سیم‌کشی شد. سقف drift **دقیقاً ۴۸۵، بدون حاشیه**.
**اثبات سه‌مرحله‌ای:** پایه سبز (exit 0) → افزودن یک فایل تستِ یتیم **قرمز** (exit 1،
`اجرانشده=486 > سقف=485`) → حذف آن، **دوباره سبز**. روی CI: **Node.js CI #1158 = success،
۲۹/۲۹ مرحله** (از ۲۸ به ۲۹ رسید) و گیت parity در مرحلهٔ ۲۳ = success.
بستنِ خودِ شکاف **BLOCKED — OWNER DECISION REQUIRED** (lane شبانه یا آرشیو رسمی).

### F-QA-01 — پرونده کامل شد، تگ عمداً جابه‌جا نشد

`docs/audit/F-QA-01_TAG_INTEGRITY_DOSSIER.md`. یافتهٔ تازه: `phase8.2-verified` یک تگ
**lightweight** است (نه annotated) — بدون tagger، بدون پیام، بدون امضا. یعنی حتی ثبت
نشده چه کسی و چرا «verified» اعلام کرده. روی `401d02b2` که **۷۷ کامیت عقب** است و
diffش فقط **یک سند ۲۲ خطی** است (هیچ کدی عوض نشده). روی آن SHA: **صفر اجرای تست**،
تنها شواهد = **دو Fortify شکست‌خورده**. سندی که تگ می‌کند به `b0b55a13` استناد می‌کرد
که **Node.js CI #1074 آن failure بود**؛ و همان سند در HEAD **بازنویسی و باطل شده**
(`0d245b9c`) ⇒ **تگ اکنون یک ادعای پس‌گرفته‌شده را منجمد کرده است**.
`grep` در `tools/ tests/ .github/ scripts/` ⇒ **هیچ اتوماسیونی این تگ را نمی‌خواند**، پس
حذف/تغییرنامش چیزی را نمی‌شکند؛ تصمیم حاکمیتی است نه فنی.
**جابه‌جایی به یک SHA سبز صراحتاً توصیه نشد** چون کلمهٔ verified را بدون شواهد E4
مشروع جلوه می‌دهد = همان fake-green. توصیه: **گزینهٔ A** — تگ annotated تازه
`phase8.2-report-published` روی همان SHA و حذف تگ قدیمی؛ دستورها ثبت شد ولی **اجرا نشد**.
یافتهٔ جانبی: تگ انتشار **`v1.0.1` هم صفر اجرای CI دارد** و تنها تگ annotated مخزن
`national-baseline-start` است. علت شکست #1095 هم دقیق شد: مرحلهٔ ۱۶ «Production Truth
Gate» شکست و **۹ مرحله از جمله خودِ `npm test` skip شد**.

### F-QA-07 — قرارداد نسخه ثبت شد، عدد نسخه تغییر نکرد

`docs/RELEASE_VERSION_CONTRACT.md`. **چهار** مرجع ناهمخوان (نه سه): `package.json` و
`package-lock.json` = `1.0.0`، جدیدترین تگ semver = `v1.0.1`، و
`docs/RELEASE_NOTES.md:3` = **`v1.0.0-rc17`**. فاصله **۱۹۲۰ کامیت** از آخرین تگ و
**هیچ CHANGELOGی وجود ندارد**. `package.json` حتی از تگ خودش عقب‌تر است.
هیچ release یا verification کاذبی تولید نکرده (هیچ گیتی نسخه را نمی‌خواند) ⇒ **P2**.
پنج قاعدهٔ الزام‌آور ثبت شد (از جمله: تگ `vX.Y.Z` فقط روی کامیتی با همان نسخه در
`package.json` **و** یک Node.js CI موفق؛ تگ انتشار جابه‌جا نمی‌شود؛ تگ‌های فازی تگ
انتشار نیستند). **version و تگ‌ها طبق دستور صریح تغییر داده نشدند** —
**BLOCKED — OWNER DECISION REQUIRED** بین سه گزینهٔ A/B/C.
