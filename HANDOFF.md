...
- ۲ **آرشیوِ HANDOFF فسادِ سیستماتیکِ ک→گ داشت** (از قبلِ این سشن): گلاس (۱۰۱)، گد (۵۷)، گدام (۵)، گارت (۱۱) — همه «کلاس/کد/کدام/کارت» بودند؛ در بند ۱۱ با جایگزینیِ بااحتیاط درست شد (کلماتِ سالمِ گ-مثلِ گزارش/دگمه/نگهبان/وگرنه دست‌نخورده ماندند).### ۳) چی موند
- بندهای ۸ تا ۱۳ دور ۷۸ (راهنمایِ پایلوت، مقایسهٔ ذخیره‌گاه، بازبینیِ مستندات/آرشیو/سایمن، گزارشِ پایانی).
- تعمیرِ `pre-confirm`/`pre-reject`/`bus-follow-open` (بازدهیِ پارامترهایِ سای‌شده — بندِ بعدی).
- خطِ «اتصال به سرور» — بدونِ تغییر.

### ۴) وضعیتِ فنی
- HEAD محلی: کامیتِ بند ۷ (روی `f6d8462`). origin = `e990503` (پوشِ دور ۷۸ در پایانِ کلِ ۱۳ بند).
- smoke **545/545** · run.js 33/33 · server1 30/30 · server2 25/25 · server3 19/19 · server4 16/16 · server5 14/14 · server6 9/9 · server7 15/15 · server8 9/9 · server9 10/10 · server10 7/7.
- مستنداتِ هم‌چرخه: USER_GUIDE.html (کالآوتِ اسکان) · AI_PROMPT ۰.۵.۱۸ بند ۷ · AD.md ۷۸.۳.
