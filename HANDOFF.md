...
## دورِ ۱۰۰ — اجرایِ برنامهٔ رفعِ ۶ یافتهٔ رفتاری (P0–P2) — ۱۸/۰۶/۱۴۰۵ (2026-09-08) — در حالِ اجرا

**وضعیت:** برنچِ `fix/behavior-findings` از `main` (`078c6bc`) + طرحِ مرجع: `docs/FIXES_ACTION_PLAN.md`. خطِ مبنا: `build --check` ✅ + smoke واقعی (jsdom نصب شد) **547/547** ✅. گزارش‌هایِ گام: `R100_STEP0..7_*_REPORT.md` (ریشه) + تجمیعی: `R100_BEHAVIOR_FIXES_AGGREGATE_REPORT.md`. (این ورودی در گامِ ۷ نهایی می‌شود.)

- **گامِ ۰ (آماده‌سازی):** برنچ + راستی‌آزماییِ خطِ مبنا + این ورودی ✅
- **گامِ ۱ →** نقصِ ۲ (P0 ماندگاری) · **۲ →** نقصِ ۱ (P0 chunking) · **۳ →** نقصِ ۳ (P1 طرحِ شهریه) · **۴ →** نقصِ ۴ (P1 سرریزِ چیدمان) · **۵ →** نقصِ ۵ (P2 بکاپ/snapshot) · **۶ →** نقصِ ۶ (P2 تداخلِ generateP12) · **۷ →** دروازهٔ نهایی + رجیسیونِ کامل.
