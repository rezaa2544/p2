...
## چت ۱ (جانشین): ادغامِ شاخهٔ زیرساخت/امنیت (`arena/01a0827b-p2`) با شاخهٔ فعال — ۱۸/۰۶/۱۴۰۵ (2026-09-09)

**درخواستِ کارفرما:** «۳۹ کامیتِ باقی‌مانده را مرج کن — کارهای
زیرساختی و امنیتی مهمی هستند که باید روی main قرار بگیرند.»
محدودیتِ نشست اجازهٔ کار روی `main` یا `arena/01a0827b-p2` را
نمی‌دهد ⇒ مسیرِ تحویل، ادغام در `arena/01a08543-p2` و رساندنش با
پی‌آر ۳۷ به `main` است.

**وضعیتِ پیش از ادغام (اندازه‌گیری، نه حدس):**
`gh api repos/rezaa2544/p2/compare/main...arena/01a0827b-p2` ⇒
`diverged`، ۵۱ جلوتر / **۷۷ عقب‌تر**، پایهٔ مشترک `54b1820`، ۱۳۴ فایل.
`git fetch --unshallow` در ۱.۳ ثانیه تمام شد (آبجکت‌ها از قبل بودند) و
`git merge-base` دقیقاً همان `54b1820` را داد.

🔴 **یافتهٔ بازدارنده:** لایهٔ سرورِ آن شاخه ۷۷ کامیت عقب‌تر بود و
merge خام، امنیتِ main را **پایین** می‌آورد (حذفِ `splice` خام به‌جای
سنگ‌قبر، `max+1` به‌جای `ids.nextId`، نبودِ `occ`/`revocation`/`gdpr`،
`/api/health` همیشه ۲۰۰). پس هیچ هانکی با «یک طرف را بردار» حل نشد؛
جدولِ تصمیم‌ها در بند **۰.۵.۲۵** `docs/AI_PROMPT.md` ثبت شده است.

**`git merge --no-ff --no-commit origin/arena/01a0827b-p2` ⇒ ۱۵ تعارض.**
نحوهٔ حل:

| فایل | حل |
|---|---|
| ۵ روت (`attendance/classes/grades/students/users`) | imports از main (`scope`+`occ`) · گیت‌های `policy.authorize/validate` از شاخه · `ids.nextId` از main · حذف از main (`softDelete`) |
| `server/auth.js` | ۴ هانک: union · **هر دو** claim (`sv`+`csrf`) · امضای ناهمگام · **OTP شاخه سپس `gdpr` main** |
| `server/index.js` | `createOtpStore({redis,cache})` + `trustedProxies` · healthِ P0-13 **و** نگهبانِ CSRF (با `await` — `sessionFrom` در main ناهمگام است) |
| `package.json` | union وابستگی‌های توسعه |
| `authz/model.json` | union (نگهداریِ `public_goals`) |
| `docs/DEPLOY.md` | هر دو سطرِ چک‌لیست (موردِ شاخه به ۱۲ شماره خورد) |
| `HANDOFF.md` | union |
| `USER_GUIDE.html` | ours + `node build.js` |
| `authz/write-perms.json` | `node tools/generate-write-perms.js` |
| `package-lock.json` | `npm install --package-lock-only` |
| `tests/smoke.js` | theirs (فقط تفاوتِ سبکِ نقل‌قول) |

**دو تلهٔ واقعی که ادغام بیرون ریخت (و رفع شد):**
1. 🔴 `policy.validate` فیلدِ پروتکلیِ `base_version` را «ناشناخته»
   می‌دانست ⇒ هر PATCH با پایهٔ نسخه ۴۰۰ می‌گرفت و P0-18 (همزمانیِ
   خوش‌بینانه) عملاً مرده بود. رفع: `TRANSPORT_FIELDS` در
   `server/policy.js`. پیش از رفع: `occ.js` **۱۰/۱۸** · پس از رفع **۱۸/۱۸**.
2. 🔴 نگهبانِ CSRF دوازده سئوتِ سرورِ main را ۴۰۳ می‌کرد. کنترلِ
   امنیتی **ضعیف نشد**؛ سئوت‌ها مثلِ مرورگر شدند (`jarOf` + `csrfHdr`):
   `occ`, `tombstone`, `preapp3`, `reexam3`, `scholarship3`, `summer3`,
   `server16`, `teacher-eval-server`, `user-self-update`,
   `session-revocation`, `dorm-kind`, `entry-gpa`. در
   `session-revocation.js` حذفِ حساب علاوه بر آن رمزِ یک‌بارمصرفِ تازه
   هم می‌خواهد (پیامدِ مستقیمِ احرازِ مجدّدِ OTP).

**اصلاحِ جانبی:** `server/tracing.js` در main دو بار `shutdownTracing`
و `__resetForTests` را تعریف کرده بود (بماندهٔ PR #22)؛ تا آمدنِ
eslint دیده نمی‌شد. `npm run lint` پیش از رفع **۲ خطا** و پس از رفع
**۰ خطا / ۶۹ هشدار** (exit 0).

**دروازه‌ها روی درختِ ادغام‌شده:**
`build --check` بیت‌به‌بیت ✅ · `run.js` **۳۵/۳۵** ✅ · smoke
**۵۴۷/۵۴۷** ✅ · `check-authz` exit 0 (۳۷۶ اکشن) ✅ · `secret-scan`
✅ · `lint` exit 0 ✅ · `occ` ۱۸/۱۸ · `tombstone` ۲۵/۲۵ · `policy`
۱۰/۱۰ + جهش‌ها · `csrf` ۱۲/۱۲ · `ip-trust` ۱۵/۱۵ · `id-collision`
۱۱/۱۱ · `lock-atomic` ۱۲/۱۲ · `teacher-eval-server` ۱۳/۱۳ ·
`user-self-update` ۸/۸ · هر ۸ سئوتِ `tests/api/*` ✅

**محدودیتِ CI:** ماتریسِ Node از `[18.x, 20.x, 22.x]` به `[22.x]`
تغییر کرد و `scan:secrets` + `lint` اضافه شد (تصمیمِ شاخهٔ زیرساخت؛
روی ۱۸/۲۰ خودِ smoke بی‌سروصدا صفر تست اجرا می‌کرد).

**بازمانده:** اصلِ هشتم — ادغامِ `main` به تأییدِ صریحِ سرپرست نیاز
دارد؛ پی‌آر ۳۷ آماده است ولی مرج نشده.
...
- **سند:** `docs/GO_LIVE_CHECKLIST.md` (تازه: §۱.۱ فنی ۶ ردیف + §۱.۲ امنیتی ۶ ردیف +
  §۱.۳ عملیاتی ۶ ردیف + §۱.۴ تصمیم‌هایِ باز + §۱.۵ ترتیبِ اجرا/ریسک‌ها/زمان‌بندی؛ هر ردیف با ستونِ راستی‌آزمایی).
- **صداقتِ سندی:** هر سندِ ارجاعیِ تمپلیت با `ls` راستی‌آزمایی شد — ۸ سندِ ناموجود با
  ارجاعِ واقعیِ جایگزین (MULTI_INSTANCE_READINESS، RELIABILITY_DR_PLAN، PILOT_ONBOARDING/…
  ، CACHE_STRATEGY_DESIGN) یا وضعیتِ ◐/⏳/⏸️ ثبت شدند؛ چت ۳ ⏸️ متوقف در ردیفِ P5 منعکس شد.
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، `tools/check-authz` خروجیِ ۰، secret-scan ‏۱۱/۱۱‏ ✅


## چت ۱ — طرحِ پاسخ به حوادثِ امنیتی (Incident Response Plan) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ کاملاً مستنداتی (تک‌فایلِ تازه + ورودیِ HANDOFF) — صفر تغییر در کد.
- **سند:** `docs/SECURITY_INCIDENT_RESPONSE.md` (تازه: §۱.۱ تیمِ IRT با جانشین + §۱.۲ جدولِ
  ۴ سناریویِ P0/P1/P2 با نشانه و اقدام + §۱.۳ playbook پنج‌فازی با دستورِ واقعی + §۱.۴
  ماتریس و قالبِ اطلاع‌رسانی + مهلتِ ۷۲ ساعته).
- **زمین‌سازی (نه وانمود):** شدت‌ها و مسیرها از `RUNBOOKS.md` §۰ ارث‌بری شد؛ بازیابی/rollback
  به §۱-۱/§۱-۳ همان سند ارجاع خورد؛ الگوهایِ `grep` نامِ واقعیِ ایونت‌هایِ `audit()` است؛
  نداشته‌ها (WAF، بلاک‌لیستِ IP، هشداردهندهٔ خودکار، Breach Log) صریحاً «نداریم + توصیه» ثبت شد.
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، `tools/check-authz` خروجیِ ۰ ✅


## چت ۱ — جمع‌بندیِ نهاییِ Security Action Plan — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ کاملاً مستنداتی (تک‌فایلِ تازه + ورودیِ HANDOFF) — صفر تغییر در کد.
- **سند:** `docs/SECURITY_SUMMARY.md` (تازه: §۱.۱ جدولِ ۱۳ حوزه با شاهدِ درختی + §۱.۲ جدولِ
  ۱۲ پایینِ باز + ۱ اطلاعاتی + ۲ توصیه/کارِ در حالِ اجرا + §۱.۳ توصیه‌هایِ Go-Live و نگهداری).
- **صداقتِ سندی (نه وانمود):** هر سطر با شاهدِ `فایل:خط`/تست تأیید شد — ۳۴۹ اکشن (نه ۳۴۳ِ
  تمپلیت)، «شاهکار» فقط شبیه‌سازیِ دمو، AES-256-GCM در این شاخه ⬜ نشده (با جبران‌کننده‌ها)،
  سقف‌هایِ OTP فایل‌محور + `checkRateLimit` رویِ Redis، ابطالِ نشست فایل‌محور + ارتقایِ P4/چت۲ ⏳.
- **رأیِ نهایی:** ۰ بحرانی/بالا/متوسطِ باز؛ ۱۲ پایینِ باز + ۱ اطلاعاتی (اولویتِ بستن:
  F-FILE-01، F-SQL-01، F-XSS-01 — هر سه یک‌خطی).
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، `tools/check-authz` خروجیِ ۰ ✅


## چت ۱ — رفعِ ۲ یافتهٔ متوسطِ ممیزی (F-AUTH-01 اعتمادِ XFF + F-CSRF-01 توکنِ CSRF و احرازِ مجدّدِ حذف) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ کدنویسی + تست (تک‌کامیتِ `fix(security)`)؛ بیلد تازه (مهرِ `1aa9f08d0384`).
- **F-AUTH-01:** `server/client-ip.js` تازه (`normalizeIp`/`parseTrustedProxies`/`getClientIp` — پیمایشِ
  راست‌به‌چپ تا اولینِ غیرقابل‌اعتماد؛ پیش‌فرض فقط loopback؛ `PAYESH_TRUSTED_PROXIES` از env)؛
  `auth.js`/`audit.js` به آن وصل شدند؛ `index.js` یک‌بار در بوت می‌سازد و تزریق می‌کند.
- **F-CSRF-01:** توکنِ ۶۴hex در claim نشست + کوکیِ `csrf_token` در login؛ نگهبانِ مرکزی در `index.js`
  (هر جهشِ ‎/api‎ با نشست، به‌جز send-code/login، باید `X-CSRF-Token` برابر داشته باشد وگرنه 403؛
  `csrf_denied` در آدیت)؛ کلاینت در `Api.request` می‌نشاند؛ logout/delete هر دو کوکی را پاک می‌کنند؛
  `delete-account` بدنهٔ `{code}` می‌خواهد (OTP همان شمارهٔ نشست، یک‌بارمصرف، tries محدود)؛ صفحهٔ
  `/account-deletion` هم کد را می‌فرستد و سرآیند را می‌نشاند؛ مودالِ کد در `58-privacy.js` بدونِ
  data-act تازه (نقشهٔ مجوزها دست‌نخورده).
- **سازگاری:** پیش‌فرضِ loopback یعنی پراکسیِ همان‌ماشین و تست‌هایِ XFFمحور (otp-ratelimit) بی‌تغییر
  سبزند؛ نشست‌هایِ قدیمیِ بی‌claim تا ورودِ دوباره رویِ جهش‌ها 403 می‌گیرند (fail-closed مستند).
- **تستِ جدید:** `tests/ip-trust.js` ‏15/15‏ (جعل از نامعتبر نادیده، زنجیره، env)؛ `tests/csrf.js`
  ‏12/12‏ (403 بدون/نادرست، معافیتِ GET و پیش‌احراز، جریانِ کاملِ حذف + purge).
- **رگرسیون:** هر ۷ سوئیتِ API + ‏24‏ سوئیتِ قدیمی (server1..18، scopeها، security2، integration،
  server3، ...) با CSRF_JAR در هارنس سبز شدند؛ server7/security2 به جریانِ کددارِ حذف مهاجرت کردند.
- **نکتهٔ محیطی:** server3 وسطِ اجرایِ پشت‌سرهم S2 را از دست داد (sleep ثابت + لود) ولی تکی سبز است؛
  خرابیِ A0b در server9 اثرِ سرورِ یتیمِ security2 رویِ 8995 بود (پس از kill سبز).
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏، secret-scan ‏۱۱/۱۱‏، lint ‏۰ خطا (59 هشدارِ پایه، بدونِ افزایش)‏ ✅

## چت ۱ — ممیزیِ امنیتیِ کد (اقدامِ ۱ از Security Action Plan) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ کاملاً مستنداتی (تک‌فایلِ تازه) — صفر تغییر در کد.
- **سند:** `docs/SECURITY_AUDIT_REPORT.md` (تازه: §۱ ممیزیِ بک‌اند + §۲ ممیزیِ فرانت‌اند +
  §۳ تعیین‌تکلیفِ W1/W2/W3 + §۴ محدودیتِ روش + §۵ گیت‌ها).
- **رأی:** ۰ بحرانی، ۰ بالا، **۲ متوسط** (اعتمادِ بی‌قیدِ XFF در `clientIp`؛ نبودِ توکنِ CSRF
  با مهارکننده‌هایِ مستند)، ۱۳ پایین، ۱ اطلاعاتی. W3 (ابهامِ `password`) **بسته شد**:
  محصول passwordless است (هیچ خوانشِ `.password` در کلِ درخت).
- **زمین‌سازی:** بازبینیِ خط‌به‌خطِ `index.js`/`auth.js`/`sync.js`(گیت‌ها)/هر ۶ روت/`pull.js`/`policy.js`/
  `admin.js`(restore بدونِ traversal)/`idor.js`؛ پیمایشِ taint رویِ ۹۷ محلِ `openModal` (۱۴۹ پرچم،
  همه دستی بازبینی و باطل شدند به‌جز ۵ نقطهٔ خامِ مهارشده)؛ دسته‌بندیِ هر ۳۲ `innerHTML`.
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏، secret-scan ‏۱۱/۱۱‏، lint ‏۰ خطا‏ ✅

## چت ۱ — برنامهٔ عملیاتی امنیتی (مبتنی بر GPT-5.6 Sol) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ کاملاً مستنداتی (تک‌فایلِ تازه) — صفر تغییر در کد.
- **سند:** `docs/SECURITY_ACTION_PLAN.md` (تازه: §۱.۱ خلاصهٔ Sol + راستی‌آزماییِ درختی + §۱.۲ چهار اقدامِ
  گام‌به‌گام با معیارِ خروج + §۱.۳ ابزارها + §۱.۴ زمان‌بندیِ ۳هفته + §۱.۵ معیارهایِ موفقیت با خطِ پایه + §۱.۶ نگاشتِ اسناد).
- **زمین‌سازی (نه وانمود):** فایلِ مستقلی از تحلیلِ Sol در ریپو نبود (جست‌وجو منفی) — اولویت‌ها از brief
  و همهٔ ادعاها از درخت: JWT/OTP/RateLimit ✅ تأیید شدند، ولی **pgcrypto در درخت نیست** (فقط digestهایِ
  Node) و به‌جایِ «موجود»، «یافت نشد» ثبت شد؛ ۷ ضعفِ آدرس‌دار (W1 تا W7) با فایل:سطر استخراج شد —
  مهم‌ترین: نبودِ توکنِ CSRF (فقط SameSite=Lax)، درون‌یابیِ `${col}` در `db.js` (مهارشده با گیتِ
  `sync.js:119-120` ولی بدونِ دفاع‌درعمق)، و نگهداشتِ ردیابی‌نشدهٔ `password` رویِ users (هدفِ رأیِ ممیزی).
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏، secret-scan (هر دو مسیر) ‏۱۱/۱۱‏، lint ‏۰ خطا‏ ✅

## چت ۱ — بررسی و تأییدِ نهاییِ وضعیتِ گیت (پوشِ همه‌چیز) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ فقط خوانش + یک گزارشِ ریشه (`GIT_VERIFICATION_FINAL_REPORT.md`) — صفر تغییر در کد.
- **یافته‌ها:** درخت تمیز، ۰ untracked/modified، هر دو شاخهٔ محلی ahead/behind صفر؛ هر ۱۷ پی‌آر (#۱–#۱۷)
  روی `origin/main` مرج‌اند (هم از API هم از گرافِ ۳۸۳ کامیت اثبات شد)؛ تنها پی‌آرِ باز #۱۸ (کارِ جاریِ چت ۳).
- **تنها استثنا (مستند):** شاخهٔ `feat/farnaz-phase1-chat2` هرگز روی origin ساخته نشد (انحرافِ شناخته‌شدهٔ
  ۰۸-۰۹، کامیتِ `9a70944`)؛ بندهایش بازتوزیع و روی main نشستند، به‌جز **ب.۳** که بی‌کامیت ماند و در بستنِ
  فاز عملاً حذف شد — نیازمندِ تأییدِ مالکِ چت ۲.
- **اقدام‌هایِ غیرمسدودکننده:** تأییدِ ب.۳ از چت ۲؛ تعیین تکلیفِ `arena/01a08256-p2`؛ بازنشستگیِ
  `fix/arch-review-wave2` (یادداشتِ superseded موجود است).
- **روش:** بدونِ `checkout` (قفلِ سشن) — همه‌چیز با `fetch` + خوانشِ read-only؛ کلونِ shallow با
  `--deepen=200` برایِ راستی‌آزماییِ گراف عمیق شد.
- **توصیهٔ صریح:** ادامهٔ کار ✅
- **گیت‌ها (تازه‌اجرا):** lint ‏۰ خطا‏، ‏`build --check` سبز‏، secret-scan ‏۱۱/۱۱‏، smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏ ✅

## چت ۱ — تحلیلِ ایستا با ESLint (Tier 3، اولویت ۱۲) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ POC (کانفیگ + اسکریپت + قدمِ CI + سند) — تنها تغییرِ
سورس: ۲ کامنتِ مجوزِ eval (صفر تغییرِ رفتار) + بازتولیدِ مشتقاتِ بیلد.
- **کانفیگ:** `eslint.config.js` تازه (فرمتِ flat — تنها فرمتِ معتبرِ ESLint 10)؛ ‏`lint: eslint .`‏ در
  `package.json`؛ قدمِ `npm run lint` در ورک‌فلو پس از `scan:secrets`؛ سندِ `docs/STATIC_ANALYSIS_SETUP.md`
  (§۰ چهار انحراف + §۱.۱/۱.۲/۱.۳/۱.۴ + §۲ شواهد + §۳ نگهداشت).
- **انحراف‌هایِ آگاهانه:** ‏`.eslintrc` → flat‏ (حذف‌شده در v10)، ‏`ignores` درون‌کانفیگی‏،
  `eslint-plugin-node` → فورکِ زندهٔ `n`، حذفِ `eslint-plugin-import` (ریپویِ CJS بدونِ گرافِ ESM).
- **زمین‌سازی (نه وانمود):** ۴ دور پیمایشِ تمام‌درختی (۳۳۶ فایل): ۹۸۷۶ خطا → **۰ خطا، ۵۹ هشدارِ
  مستند**؛ هر قانونِ error/warn/off با شمارِ برخورد و دلیل ثبت است؛ شکافِ شناخته‌شده:
  `detect-child-process` الگویِ `*Sync` را نمی‌بیند (با فیکسچر اثبات شد).
- **اثباتِ `--fix`:** فیکسچرِ موقتِ `if (!!x)` → خطایِ `no-extra-boolean-cast` → پس از
  `npm run lint -- --fix` به `if (x)` تبدیل شد (فیکسچر پاک شد، هیچ فایلِ دیگری دست نخورد).
- **نکتهٔ ابزاری:** دو ویرایشِ موازی رویِ یک فایل مسابقه می‌دهند (یکی گم شد و دوباره اعمال شد) —
  ویرایش‌هایِ یک فایل همیشه ترتیبی.
- **گیت‌ها:** lint ‏۰ خطا/خروجِ ۰‏، ‏`--fix` اثبات‌شده‏، ‏`build --check` سبز‏، smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏ ✅

## چت ۱ — Runbooks عملیات و بازیابی (Tier 3، اولویت ۱۴) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ کاملاً مستنداتی (تک‌فایلِ تازه) — صفر تغییر در کد.
- **سند:** `docs/RUNBOOKS.md` (تازه: §۰ شدت/مسیرها + §۱-۱ استقرار/رولبک + §۱-۲ چهار عیب‌یابی + §۱-۳ سه بازیابی +
  §۱-۴ مانیتورینگ + §۱-۵ سه رویهٔ امنیتی + §۲ تمرین‌ها + قاعدهٔ «واقعیت بر ران‌بوک مقدم است»).
- **زمین‌سازی (نه وانمود):** هر رویه با مکانیکِ واقعی گره خورد — ایونت‌هایِ `audit.log` (‏`sync_*`‏)، پولِ PG
  (`server/db.js`: ‏min2/max20‏)، fallback ردیس، dead-letter (`sync-del`)، بکاپ/ری‌استورِ superadmin،
  چرخشِ `jwt.key` با اثرِ «مرگِ همهٔ نشست‌ها»؛ شکاف‌ها صریح ثبت شدند (نبودِ revoke مدیریتی، نبودِ `/metrics`،
  Tracing/Logging در دستِ چتِ ۳/۲، نبودِ فایلِ alert در ریپو).
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏، secret-scan ‏۱۱/۱۱‏ ✅

## چت ۱ — CDN Integration (Tier 2، اولویت ۲) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ POC (ورکر + Ingress + هش‌گذاریِ بیلد + سند + گارد) — صفر تغییر در `src/` و `server/`؛
`index.html` بایت‌به‌بایت دست‌نخورده (تغییرِ `build.js` خروجی را عوض نکرد — `git status` گواه).
- **سازگاریِ معماری (یافتهٔ ممیزی):** بیلد تک‌فایل + فونتِ data-URI است (صفر ساب‌ریسورس) ← «هش در نامِ فایل» موضوعیت نداشت؛
  معادلِ درست: `cdn-manifest.json` قطعی (`sha256` دوتکه + `seal12`) که `node build.js` می‌نویسد و `--check` راستی‌آزمایی می‌کند.
- **دامِ اسکنر:** هشِ ۶۴تاییِ یک‌تکه، secret-scan را به ۱ هیت انداخت (گاردِ `sha256` خطِ قبلی را چک می‌کند، نه خطِ هش)؛
  راه‌حل بدونِ دست‌زدن به اسکنر: دو تکهٔ ۳۲تایی + join در راستی‌آزمایی (مستند در کد و سند).
- **فایل‌ها:** `cloudflare/worker.js` (تازه: immutable یک‌سالهٔ رزرو + ۵دقیقه‌ایِ `/` + passthrough صریحِ `/api/*`)،
  `k8s/ingress-cdn.yaml` (تازه: Ingress مبدأ با هشدارِ «کش فقط در ورکر»)، `docs/CDN_INTEGRATION_SETUP.md` (تازه: §۰ تا §۴)،
  `DEPLOY.md` (§۹ + راهِ «ج» در §۴)، `tests/cdn.js` ‏۵/۵‏ + `tests/cdn-mutations.js` ‏۲/۲‏ کشته.
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏، ‏`build --check`‏ سبز (۵ خطِ ✅/OK با خطِ تازهٔ مانیفست)، secret-scan ‏۱۱/۱۱‏ ✅

## چت ۱ — Kubernetes/ECS Auto-Scaling (Tier 2، اولویت ۱) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ POC (مانیفست + سند + گارد) — صفر تغییر در `src/` و `server/`؛ بدونِ کلاسترِ واقعی.
- **سند:** `docs/AUTO_SCALING_SETUP.md` (تازه: §۰ پنج پیش‌نیازِ اجباری + §۱-۱ انتخابِ k8s + §۱-۲ جدولِ ۴ متریکه با ستونِ وضعیت +
  §۱-۳ HPA + §۱-۴ VPA + §۱-۵ ران‌بوکِ ۳ تستِ کلاستری + چک‌لیست + §۴ «آنچه ادعا نمی‌کند»).
- **مانیفست‌ها (`k8s/`، هر ۳ ‏`autoscaling/v2`‏):** ‏`hpa-payesh-api.yaml`‏ واقعی (‏min3/max20‏، ‏cpu70‏، ‏mem80‏، صف/۱۰۰۰،
  ‏scaleDown.stabilization 300s‏ = cooldown پنج‌دقیقه) + دو HPAیِ stateful عمداً قفل‌شدهٔ ‏۱/۱‏ با کامنتِ «چرا اسکیل نمی‌شود».
- **صادق‌سازی‌ها:** متریکِ `redis_queue_depth` فقط رزروِ جدول است (صفِ job نداریم — وگرنه HPA خطایِ متریکِ ناموجود می‌گیرد)؛
  متریکِ سفارشی تا اندپوینتِ `/metrics` (P4، کدِ آینده) degraded است؛ VPA و HPA هم‌زمان رویِ یک متریک ممنوع؛
  تا P1+P2+P3 (استورِ PG، ‏REDIS_URL‏ مشترک، خروجِ OTP/ریت‌لیمیت/jti از فایل) `maxReplicas` عملاً ۱ است.
- **گارد:** `tests/hpa.js` ‏۶/۶‏ (آستانه‌ها/قفل‌ها/ارجاعِ سند) + `tests/hpa-mutations.js` ‏۲/۲‏ کشته؛ YAML هر ۳ فایل با js-yaml پارس شد.
- **ارجاع‌ها:** `DEPLOY.md` (§۹ + §۶-ب) و یک خط در `CANARY_DEPLOYMENT.md` §۱-۵ (رفعِ تناقضِ «HPA نداریم» ← POC).
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏، secret-scan ‏۱۱/۱۱‏ ✅

## اصلاحیهٔ چت ۱ — ادعایِ Redis در سندِ Canary (کشف در ممیزیِ Load Plan) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

- **ریشه:** نسخهٔ اولِ `CANARY_DEPLOYMENT.md` (وظیفهٔ ۳) می‌گفت «در این معماری Redis نیست» —
  ممیزیِ Load Plan ثابت کرد اشتباه است: `server/redis.js` + ‏`server/cache.js`‏ واقعی و سیم‌کشی‌شده‌اند
  (ioredis + ‏`REDIS_URL`‏ با fallback حافظه؛ کش در مسیرِ bootstrap و idempotency/ابطالِ sync).
- **رفع (فقط سند، ۴ نقطه):** سطرِ C4 + اسنیپتِ readiness (افزودنِ `redis-cli ping` مشروط) + پاراگرافِ §۱-۲
  (با برچسبِ «اصلاحیه») + بولتِ §۴. قاعدهٔ جدید: `REDIS_URL` ست = ping جزوِ readiness؛ نیست = fallback اعلامی.
- **راستی‌آزماییِ نمایش:** شمارشِ `chr(92)` رویِ خطِ `[ -z "$REDIS_URL" ]` = صفر (بک‌اسلشِ دیده‌شده در خروجی، نمایشی بود).
- secret-scan ‏۱۱/۱۱‏ ✅؛ کامیتِ جدا: `docs: correct redis claims in canary plan`.

## چت ۱ — Load Testing Plan (Tier 1، اولویت ۳) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ مستنداتی + سیم‌کشی — صفر تغییر در `src/` و `server/` (با گارد راستی‌آزمایی شد).
- **یافتهٔ اصلی (ممیزی پیش از کار):** ادعایِ «k6 قبلاً استفاده شده» درست بود ولی نه آن‌طور که پرامپت می‌گفت —
  `docs/LOAD_TESTING_PLAN.md` (۳۲۵ خط: مقایسهٔ ابزار، ۶ سناریو، نردبان، SLO) و کلِ `tests/performance/`
  (۶ سناریو + ۴ سوئیت + هلپر/کانفیگ/فیکسچر + رانرِ `run-benchmarks.sh`) از main (فاز ۵) موجود بود؛
  به‌جایِ دوباره‌سازی: راستی‌آزمایی + سیم‌کشیِ نقطهٔ ورود + §۱۰.
- **راستی‌آزماییِ کدِ موجود:** هر ۱۳ فایلِ `.js` پارسِ ESM ‏(k6 فقط ESM)‏؛ هر ۶۸ ایمپورتِ نسبی به اکسپورتِ واقعی
  رسید (۰ گمشده)؛ همهٔ مسیرهایِ `/api/*` (شاملِ `/api/v1/*` فاز ۳) در `server/index.js` موجودند.
  درسِ ابزار: پارامترهایِ ابزار escapeهایِ JSON را تفسیر می‌کنند — regex/بک‌اسلش فقط در فایل (write_file)، هرگز در `bash -c`.
- **دلتایِ واقعی:** ۳ اسکریپتِ `perf:login` / `perf:attendance` / `perf:full` در package.json (لفافِ رانر) +
  §۱۰ در `LOAD_TESTING_PLAN.md` (مرجعِ دستورها، پیش‌نیازها/محیط‌ها، قاعدهٔ «k6 در CI نیست»، تطبیقِ نردبانِ ۱۰M، وضعیتِ راستی‌آزمایی).
- **سیم‌کشی تست شد:** `npm run perf:login` بدونِ k6 → گاردِ تمیزِ «k6 یافت نشد» (fail-closed) ✅.
- **گیت‌ها:** smoke ‏۵۴۷/۵۴۷‏، authz ‏۰‏، secret-scan ‏۱۱/۱۱‏ ✅
- **بدهیِ یافته‌شده:** راهنمایِ نصبِ داخلِ `run-benchmarks.sh` گزینهٔ غیرواقعیِ `npm install -g k6` را پیشنهاد می‌دهد
  (نصبِ درست در README §۲ هست) — عمداً دست نخورد (خارج از دامنه)؛ follow-up ثبت شد.

## چت ۱ — وظیفهٔ ۳: طرحِ استقرارِ مرحله‌ای (Canary/Blue-Green) — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ کاملاً مستنداتی — صفر تغییر در `src/` و `server/` و `tests/` (با گاردِ `git status` راستی‌آزمایی شد).
- **سند:** `docs/CANARY_DEPLOYMENT.md` (تازه: §۰ قیدها + §۱-۱ استراتژی + §۱-۲ هلث + §۱-۳ SLO + §۱-۴ رولبک + §۱-۵ اسکیل + §۱-۶ ران‌بوک + §۲ مرجعِ اسکریپت‌ها + چک‌لیست + §۴ «آنچه ادعا نمی‌کند») + اشاره در `DEPLOY.md` §۷.
- **صادق‌سازی‌هایِ معماری (انحرافِ عمدی از پرامپت به نفعِ واقعیت):** هلث فقط `GET /api/health` است (liveness/readiness/startup رویش نگاشت شد، نه مسیرِ جدا)؛ استورِ JSON تک‌نویسنده است → شکافِ درصدی فقط در مسیرِ PG، رویِ JSON فقط cutover با ترتیبِ دقیق؛ Redis در معماری نیست (در هیچ گیتی نیامد)؛ آروان در مسیرِ درخواست نیست (هشدار، نه گیت)؛ Rollback هرگز `migrate:down` نمی‌زند؛ HPA نداریم (هشدار + scale-out دستی + نگهبان‌هایِ systemd).
- **اسکریپت‌ها (توصیه‌شده، هر دو `+x` و `bash -n` سبز):** `scripts/canary-deploy.sh` (canary مرحله‌ای در PG با گیتِ SLO از لاگِ nginx + cutover امن در JSON با symlinkِ `.env.active` + اسنپ‌شات + abort خودکار) و `scripts/rollback.sh` (= ‏`npm run rollback`‏ — اسکریپتِ تازه در package.json). هر دو: `set -euo pipefail`، گیتِ `nginx -t` پیش از reload، حالتِ `DRY_RUN=1`.
- **راستی‌آزماییِ اسکریپت‌ها:** `rollback.sh` در DRY_RUN کلِ مسیرِ echo را با exit ‏۰‏ طی کرد؛ `canary-deploy.sh --cutover` در DRY_RUN رویِ غیبتِ Green با abortِ fail-closed و exit غیرصفر ایستاد (دو بار، پس از رفعِ آلودگیِ stdout در `snapshot()` که مسیرِ اسنپ‌شات را کثیف می‌کرد).
- **گیت‌ها (فقط راستی‌آزمایی):** smoke ‏547/547‏، authz ‏0‏ (تطبیقِ کامل)، secret-scan ‏11/11‏ ✅

## چت ۱ — وظیفهٔ ۲: تحلیلِ ایستا — اسکنِ راز در CI — ۱۹/۰۶/۱۴۰۵ (2026-09-09) — ✅

**وضعیت:** رویِ `arena/01a0827b-p2`؛ منطقِ اسکن دست‌نخورده ماند (`tests/secret-scan.js` همان ۱۱ چکِ R96 است).
- **یافته:** مسیرِ `tools/secret-scan.js` در پرامپت وجود نداشت — منطق فقط در `tests/secret-scan.js` بود و CI اصلاً اسکن نمی‌کرد. به‌جایِ تکثیرِ منطق: لفافِ نازکِ `tools/secret-scan.js` (واگذاری + پاسِ دست‌نخوردهٔ کدِ خروج) + اسکریپتِ `npm run scan:secrets` + گامِ تازه در `.github/workflows/node.js.yml` پس از `npm test`.
- **تست:** `tests/scanwrap.js` ‏2/2‏ (SW1 هم‌ارزیِ خروجِ ۰ رویِ درختِ تمیز؛ SW2 انتشارِ شکست با فیکسچرِ `ghp_` ساخته‌شدهٔ برنامه‌ای + پاک‌سازیِ تضمینی در finally) + `tests/scanwrap-mutations.js` ‏2/2‏ کشته (خروجِ همیشه-۰ → SW2؛ مسیرِ خراب → SW1). سئوتِ تازه خودکار در `run-all-tests.sh` کشف می‌شود.
- **گیت‌ها:** smoke ‏547/547‏، authz ‏0‏ (تطبیقِ کامل)، scan:secrets سبز، جهش‌ها سبز ✅
...
- **رگرسیون:** ۱۷۶/۱۷۷ سبز (۱۷۷۰s)؛ تنها قرمز `xss-guard` X2b = نقطهٔ ۷ `document.write` (چاپ C.3) که مرور امنیتی و مُهر شد (فقط-تست) ⇒ **مؤثر ۱۷۷/۱۷۷** · دودی ۵۴۷/۵۴۷ · مجوزها ۰ (۳۴۹ اکشن) · ساخت ✅ · راز ۱۱/۱۱.
- **چت ۲:** همچنان هیچ شاخه‌ای روی origin نیست؛ اما ب.۴/ب.۵ در PR #8 (آمادهٔ مرج، CI سبز)، ج روی مین، د.۱ در PR #8، د.۲–د.۴ پیش‌نویس روی مین، گ.۳ گزارش روی مین — فقط **ب.۳ مفقود** (متن نزد چت ۲). حکم پیشنهادی در گزارش؛ هیچ PRای مرج/بسته نشد.
- **کارایی:** رندر جزئی `att-set` از قبل روی مین بود (دور ۱۰۲)؛ راستی‌آزمایی مستقل: میانهٔ **۴۶۸ms** ✅ + `attpartial` ۱۰/۱۰ + جهش ۴/۴ — کدی زده نشد.
